...
class shopFrontendApiOrderCalculateController extends shopFrontApiJsonController
{
    public function post()
    {
        ...
        $items = array_map(function($item) use ($formatter) {
            unset($item['product']);
    ...
    protected function getShippingData(shopCheckoutConfig $config, shopOrder $order) 
    {
        ...
        if (!is_array($address) || empty($address['country']) || empty($address['region']) || empty($address['city'])) {
            throw new waAPIException('required_param', 'When shipping_variant_id is provided, address is also required with at least country, region and city', 400);
        }
        ...
        if (!$plugin) {
            throw new waAPIException('bad_param', 'unknown shipping_variant_id: plugin not found', 400);
        }
        ...
        if ($missing_field_ids) {
            throw new waAPIException('required_param', 'Missing address fields required by shipping plugin: '.join(', ', $missing_field_ids), 400);
        }
        ...
        if (!$selected_variant) {
            ...
            throw new waAPIException('bad_param', 'unknown shipping_variant_id: unable to calculate shipping cost', 400);
        }
