...
class shopHelper
{
    ...
    public static function getGravatarPic($email, $options = array())
    {
        ...
        if ($default !== 'custom') {
            ...
            $url = wa()->getConfig()->getRootUrl().'wa-content/img/userpic.svg';
        }
        ...
        if ($count === null) {
            ...
            if ($count !== null && $include_text) {
                ...
                if ($stock_id && $all_balance_stocks) {
                    $all_balance_stocks_text =
                        "<span class='gray'>" .
                            sprintf_wp('(On all stocks: %s)', shopFrac::discardZeros($all_balance_stocks)) .
                        "</span>";
                }
                $icon .=
    ...
    public static function workupOrderItems($order_items, $options)
    {
        ...
        foreach ($order_items as $item) {
            ...
            $items[] = array(
                ...
                'discount'             => ($item_quantity > 0 ? floatval($item['total_discount'] / $item_quantity) : 0.0),
