...
class shopPresentation
{
    ...
    public function __construct($id = null, $transient = false, $params = [])
    {
        ...
        if (empty($presentation)) {
            if ($transient) {
                $presentation = $this->getModel()->getTransientByTemplate($template_id, wa()->getUser()->getId(), [
                    ...
                    'template_filter_id' => $params['template_filter_id'],
                ]);
            } else {
        ...
        if ($transient) {
            ...
            if (!empty($presentation_id)) {
                $this->getModel()->updateLastUsedData($presentation_id, true, $browser);
            }
        }
    ...
    {
        $type = null;
        $id = waRequest::request('category_id', null, waRequest::TYPE_INT);
        if ($id) {
            $type = 'categories';
        } else {
            $id = waRequest::request('set_id', null, waRequest::TYPE_STRING_TRIM);
            if ($id) {
                $type = 'sets';
            }
        }
        if ($type) {
            ...
            if ($value) {
                ...
                if ($type == 'categories') {
                    ...
                    $row = $category_model->getById($value[0]);
                } elseif ($type == 'sets') {
                    $set_model = new shopSetModel();
                    $row = $set_model->getById($value[0]);
                }
                if ($row) {
    ...
    {
        if ($this->isTransient() && $this->presentation['filter_id'] != $filter_id) {
    ...
    public function getEnabledColumns()
    {
        ...
        $required_columns = self::getRequiredColumns($this->getView());
    ...
    {
        return $this->presentation['sort_order'];
    }
    ...
    public function getProducts($collection, $options = [])
    {
        ...
        $limit = ifempty($options, 'limit', 50);
        $offset = ifempty($options, 'offset', 0);
        $columns_list = $this->getColumnsList();
        $products = $collection->getProducts($this->getCollectionFields($additional_fields), $offset, $limit, false);
                        ...
                        switch ($active_column['type']) {
                            ...
                            case 'decimal':
                                if (in_array($active_column['id'], ['price', 'compare_price', 'purchase_price', 'total_sales'])) {
                                    ...
                                    if ($this->getView() == self::VIEW_TABLE_EXTENDED) {
                                        $skus_data = [];
                                        foreach ($p['skus'] as $sku) {
                                            $sku_stock_value = '';
                                            if ($sku['stock']) {
    ...
    public static function getNearestProducts($product_id, $presentation_id, $with_name = false)
    {
        ...
        if ($filter_id && $product_id) {
            ...
            $nearest_product_ids = $collection->getPrevNextProductId($product_id);
            $result = [
                'prev' => [
                    'id' => $nearest_product_ids[0],
                ],
                'next' => [
                    'id' => $nearest_product_ids[1],
                ],
                'position' => null,
                'count' => null,
            ];
            if ($with_name) {
                $product_model = new shopProductModel();
                $products = $product_model->getById($nearest_product_ids);
                $result['prev']['name'] = ifset($products, $nearest_product_ids[0], 'name', '');
                $result['next']['name'] = ifset($products, $nearest_product_ids[1], 'name', '');
            }
        }
