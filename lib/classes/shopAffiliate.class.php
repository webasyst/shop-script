...
class shopAffiliate
{
    ...
    public static function calculateBonus($order_or_id, $credit_rate = null)
    {
        ...
        $storefront_cur = wa('shop')->getConfig()->getCurrency(false);
        $affiliatable_total = $curm->convert(ifset($order['total'], 0) - ifset($order['shipping'], 0), ifempty($order_currency, $storefront_cur), $def_cur);
