...
confirm(a.data("confirm-text")||a.attr("title")||"Are you sure?"))e.params.push(a),scope[e.name].apply(scope,e.params)}else $.shop.error("Not found js handler for link",[e,b,a])}catch(g){$.shop.error("Exception "+g.message,g)}return!1},profileAdd:function(a,b){b=b||"";b=b.match(/^hash\//)?b.replace(/^hash\//,""):null;$.ajax({url:"?module=importexport&action=add",type:"POST",dataType:"json",data:{plugin:a,hash:b},success:function(b){$.shop.trace("addAction",b.data);b&&"ok"==b.status&&(window.location.hash=
window.location.hash.replace(RegExp("\\/("+a+"):?\\d*\\/","g"),"/$1:"+b.data+"/"))}})},profileUpdateName:function(a){var b=(""+this.profiles.key()+":"+this.path.profile).replace(/([:#])/g,"\\\\$1");$("#s-importexport-profile").find('a[href="#/'+b+'/"]:first').text(a)},profileDelete:function(a,b){var d=this.path.profile,c=(""+a+":"+d).replace(/([:#])/g,"\\\\$1");$.shop.trace("profileDelete",[a,d,c]);var e=$("#s-importexport-profile").find('a[href^="#/'+c+'/"]:first').parents("li:first"),g=b.find("i.icon16.delete");
g.removeClass("delete").addClass("loading");var f=this;$.ajax({url:"?module=importexport&action=delete",type:"POST",dataType:"json",data:{plugin:a,profile:d},success:function(c){$.shop.trace("profileDelete",c.data);if(c&&"ok"==c.status&&(g.removeClass("loading").addClass("delete"),b.parents("li:first").hide(),e.remove(),$.shop.trace("profileDelete",[f.path,d]),f.profiles.list[a][d]&&(f.profiles.list[a][d]=null),f.path.profile==d)){c=null;for(d in f.profiles.list[a])if(f.profiles.list[a].hasOwnProperty(d)&&
