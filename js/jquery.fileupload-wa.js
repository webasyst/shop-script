    ...
    $.widget('wa.fileupload', $.blueimp.fileupload, {
        ...
        options: {
            ...
            send: function (e, data) {
                ...
                data.context.find('.s-upload-oneimage-progress').addClass('current');
                if (data.context && data.dataType &&
                        data.dataType.substr(0, 6) === 'iframe') {
                    // Iframe Transport does not support progress events.
                    // In lack of an indeterminate progress bar, we set
                    // the progress to 100%, showing the full animated bar:
                    data.context.find('.s-upload-oneimage-progress').css(
                        'width',
                        parseInt(100, 10) + '%'
                    );
                }
            ...
            // Callback for successful uploads:
                ...
                if (typeof that.options.onDone === 'function') {
                    that.options.onDone(data);
                }
            },
...
//                $('#s-upload-step2').hide();
//                $('#s-upload-step2-buttons').hide();
//                $('#s-upload-step3').show();
//                $('#s-upload-step3-buttons').show();
//                $('#s-upload-step3-buttons .cancel').show();
//                $(this).find('.fileupload-progressbar')
//                    .css('width', '0%');
//                $("#s-upload-step3-buttons .progressbar").show();
//                $('#s-upload-step3-buttons .cancel').text($_('Stop upload'));
//                $("#upload-error").hide();
                var that = $(this);
            ...
            stop: function (e) {
                ...
                that.find('#s-upload-progressbar').animate({
                    'width': '100%'
                });
                if (that.data('is_error')) {
                    $("#s-upload-progressbar").parents('.progressbar:first').hide();
                    $('#s-upload-filescount').hide();
                    $("#s-upload-error").show();
                }
                if (!that.data('is_error') && !that.data('is_aborted')) {
                    fileupload.dialog.trigger('close');
                    $('#s-upload-step1-buttons .cancel').text($_('Stop upload'));
                }
                that.data('is_error', false).data('is_aborted', false);
                $('#s-upload-step1-buttons .cancel').text($_('Close'));
            },
        ...
        _initDialog: function() {
            ...
            that.dialog.bind('close', function() {
                that._files.empty();
            });
        },
