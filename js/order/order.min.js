...
$jscomp.makeIterator=function(b){var a="undefined"!=typeof Symbol&&Symbol.iterator&&b[Symbol.iterator];if(a)return a.call(b);if("number"==typeof b.length)return $jscomp.arrayIterator(b);throw Error(String(b)+" is not an iterable or ArrayLike");};$jscomp.arrayFromIterator=function(b){for(var a,c=[];!(a=b.next()).done;)c.push(a.value);return c};$jscomp.arrayFromIterable=function(b){return b instanceof Array?b:$jscomp.arrayFromIterator($jscomp.makeIterator(b))};
$jscomp.findInternal=function(b,a,c){b instanceof String&&(b=String(b));for(var e=b.length,d=0;d<e;d++){var g=b[d];if(a.call(c,g,d,b))return{i:d,v:g}}return{i:-1,v:void 0}};$jscomp.ASSUME_ES5=!1;$jscomp.ASSUME_NO_NATIVE_MAP=!1;$jscomp.ASSUME_NO_NATIVE_SET=!1;$jscomp.SIMPLE_FROUND_POLYFILL=!1;$jscomp.ISOLATE_POLYFILLS=!1;$jscomp.FORCE_POLYFILL_PROMISE=!1;$jscomp.FORCE_POLYFILL_PROMISE_WHEN_NO_UNHANDLED_REJECTION=!1;
...
$jscomp.IS_SYMBOL_NATIVE="function"===typeof Symbol&&"symbol"===typeof Symbol("x");$jscomp.TRUST_ES6_POLYFILLS=!$jscomp.ISOLATE_POLYFILLS||$jscomp.IS_SYMBOL_NATIVE;$jscomp.polyfills={};$jscomp.propertyToPolyfillSymbol={};$jscomp.POLYFILL_PREFIX="$jscp$";var $jscomp$lookupPolyfilledValue=function(b,a,c){if(!c||null!=b){c=$jscomp.propertyToPolyfillSymbol[a];if(null==c)return b[a];c=b[c];return void 0!==c?c:b[a]}};
$jscomp.polyfill=function(b,a,c,e){a&&($jscomp.ISOLATE_POLYFILLS?$jscomp.polyfillIsolated(b,a,c,e):$jscomp.polyfillUnisolated(b,a,c,e))};$jscomp.polyfillUnisolated=function(b,a,c,e){c=$jscomp.global;b=b.split(".");for(e=0;e<b.length-1;e++){var d=b[e];if(!(d in c))return;c=c[d]}b=b[b.length-1];e=c[b];a=a(e);a!=e&&null!=a&&$jscomp.defineProperty(c,b,{configurable:!0,writable:!0,value:a})};
...
(function(b){function a(v){return b.data(v,"tooltip")}function c(v){a(this).delay?q=setTimeout(d,a(this).delay):d();t=!!a(this).track;b(document.body).bind("mousemove",g);g(v)}function e(){if(!b.tooltip.blocked&&this!=k&&(this.tooltipText||a(this).bodyHandler)){k=this;p=this.tooltipText;if(a(this).bodyHandler){h.title.hide();var v=a(this).bodyHandler.call(this);v.nodeType||v.jquery?h.body.empty().append(v):h.body.html(v);h.body.show()}else if(a(this).showBody){v=p.split(a(this).showBody);h.title.html(v.shift()).show();
h.body.empty();for(var u=0,w;w=v[u];u++)0<u&&h.body.append("<br/>"),h.body.append(w);h.body.hideWhenEmpty()}else h.title.html(p).show(),h.body.hide();a(this).showURL&&b(this).url()?h.url.html(b(this).url().replace("http://","")).show():h.url.hide();h.parent.addClass(a(this).extraClass);a(this).fixPNG&&h.parent.fixPNG();c.apply(this,arguments)}}function d(){q=null;y&&b.fn.bgiframe||!a(k).fade?h.parent.show():h.parent.is(":animated")?h.parent.stop().show().fadeTo(a(k).fade,k.tOpacity):h.parent.is(":visible")?
h.parent.fadeTo(a(k).fade,k.tOpacity):h.parent.fadeIn(a(k).fade);g()}function g(v){if(!(b.tooltip.blocked||v&&"OPTION"==v.target.tagName))if(!t&&h.parent.is(":visible")&&b(document.body).unbind("mousemove",g),null==k)b(document.body).unbind("mousemove",g);else{h.parent.removeClass("viewport-right").removeClass("viewport-bottom");var u=h.parent[0].offsetLeft,w=h.parent[0].offsetTop;v&&(u=v.pageX+a(k).left,w=v.pageY+a(k).top,v="auto",a(k).positionLeft&&(v=b(window).width()-u,u="auto"),h.parent.css({left:u,
right:v,top:w}));v=b(window).scrollLeft();var B=b(window).scrollTop(),F=b(window).width(),D=b(window).height(),z=h.parent[0];v+F<z.offsetLeft+z.offsetWidth&&(u-=z.offsetWidth+20+a(k).left,h.parent.css({left:u+"px"}).addClass("viewport-right"));B+D<z.offsetTop+z.offsetHeight&&(w-=z.offsetHeight+20+a(k).top,h.parent.css({top:w+"px"}).addClass("viewport-bottom"))}}function m(v){function u(){h.parent.removeClass(w.extraClass).hide().css("opacity","")}if(!b.tooltip.blocked){q&&clearTimeout(q);k=null;var w=
a(this);y&&b.fn.bgiframe||!w.fade?u():h.parent.is(":animated")?h.parent.stop().fadeTo(w.fade,0,u):h.parent.stop().fadeOut(w.fade,u);a(this).fixPNG&&h.parent.unfixPNG()}}var h={},k,p,q,y=b.browser.msie&&/MSIE\s(5\.5|6\.)/.test(navigator.userAgent),t=!1;b.tooltip={blocked:!1,defaults:{delay:200,fade:!1,showURL:!0,extraClass:"",noHideOnClick:!1,top:15,left:15,id:"tooltip"},block:function(){b.tooltip.blocked=!b.tooltip.blocked}};b.fn.extend({tooltip:function(v){v=b.extend({},b.tooltip.defaults,v);h.parent||
(h.parent=b('<div id="'+v.id+'"><h3></h3><div class="body"></div><div class="url"></div></div>').appendTo(document.body).hide(),b.fn.bgiframe&&h.parent.bgiframe(),h.title=b("h3",h.parent),h.body=b("div.body",h.parent),h.url=b("div.url",h.parent));var u=this.each(function(){b.data(this,"tooltip",v);this.tOpacity=h.parent.css("opacity");this.tooltipText=this.title;b(this).removeAttr("title");this.alt=""}).mouseover(e).mouseout(m);v.noHideOnClick||u.click(m);return u},fixPNG:y?function(){return this.each(function(){var v=
b(this).css("backgroundImage");v.match(/^url\(["']?(.*\.png)["']?\)$/i)&&(v=RegExp.$1,b(this).css({backgroundImage:"none",filter:"progid:DXImageTransform.Microsoft.AlphaImageLoader(enabled=true, sizingMethod=crop, src='"+v+"')"}).each(function(){var u=b(this).css("position");"absolute"!=u&&"relative"!=u&&b(this).css("position","relative")}))})}:function(){return this},unfixPNG:y?function(){return this.each(function(){b(this).css({filter:"",backgroundImage:""})})}:function(){return this},hideWhenEmpty:function(){return this.each(function(){b(this)[b(this).html()?
...
[z(f.disableTimeRanges[n][0]),z(f.disableTimeRanges[n][1])];f.disableTimeRanges=f.disableTimeRanges.sort(function(r,x){return r[0]-x[0]});for(n=f.disableTimeRanges.length-1;0<n;n--)f.disableTimeRanges[n][0]<=f.disableTimeRanges[n-1][1]&&(f.disableTimeRanges[n-1]=[Math.min(f.disableTimeRanges[n][0],f.disableTimeRanges[n-1][0]),Math.max(f.disableTimeRanges[n][1],f.disableTimeRanges[n-1][1])],f.disableTimeRanges.splice(n,1))}return f}function e(f){var l=f.data("timepicker-settings"),n=f.data("timepicker-list");
if(n&&n.length&&(n.remove(),f.data("timepicker-list",!1)),l.useSelect)var r=n=b("<select />",{"class":"ui-timepicker-select"});else n=b("<ul />",{"class":"ui-timepicker-list"}),r=b("<div />",{"class":"ui-timepicker-wrapper",tabindex:-1}),r.css({display:"none",position:"absolute"}).append(n);if(l.noneOption)if(!0===l.noneOption&&(l.noneOption=l.useSelect?"Time...":"None"),b.isArray(l.noneOption))for(var x in l.noneOption){if(parseInt(x,10)==x){var C=d(l.noneOption[x],l.useSelect);n.append(C)}}else C=
d(l.noneOption,l.useSelect),n.append(C);(l.className&&r.addClass(l.className),null===l.minTime&&null===l.durationTime||!l.showDuration)||("function"==typeof l.step?"function":l.step,r.addClass("ui-timepicker-with-duration"),r.addClass("ui-timepicker-step-"+l.step));C=l.minTime;"function"==typeof l.durationTime?C=z(l.durationTime()):null!==l.durationTime&&(C=l.durationTime);x=null!==l.minTime?l.minTime:0;var H=null!==l.maxTime?l.maxTime:x+G-1;x>H&&(H+=G);H===G-1&&"string"===b.type(l.timeFormat)&&l.show2400&&
(H=G);var L=l.disableTimeRanges,I=0,Q=L.length,N=l.step;"function"!=typeof N&&(N=function(){return l.step});for(var O=0;H>=x;O++,x+=60*N(O)){var K=x,M=D(K,l);if(l.useSelect)var J=b("<option />",{value:M});else J=b("<li />"),J.addClass(43200>K%86400?"ui-timepicker-am":"ui-timepicker-pm"),J.data("time",86400>=K?K:K%86400);J.text(M);if((null!==l.minTime||null!==l.durationTime)&&l.showDuration)if(M=F(x-C,l.step),l.useSelect)J.text(J.text()+" ("+M+")");else{var P=b("<span />",{"class":"ui-timepicker-duration"});
P.text(" ("+M+")");J.append(P)}Q>I&&(K>=L[I][1]&&(I+=1),L[I]&&K>=L[I][0]&&K<L[I][1]&&(l.useSelect?J.prop("disabled",!0):J.addClass("ui-timepicker-disabled")));n.append(J)}(r.data("timepicker-input",f),f.data("timepicker-list",r),l.useSelect)?(f.val()&&n.val(g(z(f.val()),l)),n.on("focus",function(){b(this).data("timepicker-input").trigger("showTimepicker")}),n.on("blur",function(){b(this).data("timepicker-input").trigger("hideTimepicker")}),n.on("change",function(){t(f,b(this).val(),"select")}),t(f,
n.val(),"initial"),f.hide().after(n)):(x=l.appendTo,"string"==typeof x?x=b(x):"function"==typeof x&&(x=x(f)),x.append(r),p(f,n),n.on("mousedown click","li",function(R){f.off("focus.timepicker");f.on("focus.timepicker-ie-hack",function(){f.off("focus.timepicker-ie-hack");f.on("focus.timepicker",A.show)});h(f)||f[0].focus();n.find("li").removeClass("ui-timepicker-selected");b(this).addClass("ui-timepicker-selected");B(f)&&(f.trigger("hideTimepicker"),n.on("mouseup.timepicker click.timepicker","li",
function(S){n.off("mouseup.timepicker click.timepicker");r.hide()}))}))}function d(f,l){var n,r,x;return"object"==typeof f?(n=f.label,r=f.className,x=f.value):"string"==typeof f?n=f:b.error("Invalid noneOption value"),l?b("<option />",{value:x,"class":r,text:n}):b("<li />",{"class":r,text:n}).data("time",String(x))}function g(f,l){return f=l.roundingFunction(f,l),null!==f?D(f,l):void 0}function m(f){f.target!=window&&(f=b(f.target),f.closest(".ui-timepicker-input").length||f.closest(".ui-timepicker-wrapper").length||
(A.hide(),b(document).unbind(".ui-timepicker"),b(window).unbind(".ui-timepicker")))}function h(f){f=f.data("timepicker-settings");return(window.navigator.msMaxTouchPoints||"ontouchstart"in document)&&f.disableTouchKeyboard}function k(f,l,n){if(!n&&0!==n)return!1;f=f.data("timepicker-settings");var r=!1;n=f.roundingFunction(n,f);return l.find("li").each(function(x,C){x=b(C);if("number"==typeof x.data("time"))return x.data("time")==n?(r=x,!1):void 0}),r}function p(f,l){l.find("li").removeClass("ui-timepicker-selected");
var n=z(y(f),f.data("timepicker-settings"));null!==n&&(f=k(f,l,n))&&(n=f.offset().top-l.offset().top,(n+f.outerHeight()>l.outerHeight()||0>n)&&l.scrollTop(l.scrollTop()+f.position().top-f.outerHeight()),f.addClass("ui-timepicker-selected"))}function q(f,l){if(""!==this.value&&"timepicker"!=l){var n=b(this);if(!n.is(":focus")||f&&"change"==f.type){f=n.data("timepicker-settings");var r=z(this.value,f);if(null===r)return void n.trigger("timeFormatError");var x=!1;if(null!==f.minTime&&null!==f.maxTime&&
(r<f.minTime||r>f.maxTime)&&(x=!0),b.each(f.disableTimeRanges,function(){return r>=this[0]&&r<this[1]?(x=!0,!1):void 0}),f.forceRoundTime){var C=f.roundingFunction(r,f);C!=r&&(r=C,l=null)}f=D(r,f);x?t(n,f,"error")&&n.trigger("timeRangeError"):t(n,f,l)}}}function y(f){return f.is("input")?f.val():f.data("ui-timepicker-value")}function t(f,l,n){if(f.is("input")){f.val(l);var r=f.data("timepicker-settings");r.useSelect&&"select"!=n&&"initial"!=n&&f.data("timepicker-list").val(g(z(l),r))}return f.data("ui-timepicker-value")!=
l?(f.data("ui-timepicker-value",l),"select"==n?f.trigger("selectTime").trigger("changeTime").trigger("change","timepicker"):-1==["error","initial"].indexOf(n)&&f.trigger("changeTime"),!0):(f.trigger("selectTime"),!1)}function v(f){switch(f.keyCode){case 13:case 9:break;default:f.preventDefault()}}function u(f){var l=b(this),n=l.data("timepicker-list");if(!n||!a(n)){if(40!=f.keyCode)return!0;A.show.call(l.get(0));n=l.data("timepicker-list");h(l)||l.focus()}switch(f.keyCode){case 13:return B(l)&&(q.call(l.get(0),
{type:"change"}),A.hide.apply(this)),f.preventDefault(),!1;case 38:var r=n.find(".ui-timepicker-selected");return r.length?r.is(":first-child")||(r.removeClass("ui-timepicker-selected"),r.prev().addClass("ui-timepicker-selected"),r.prev().position().top<r.outerHeight()&&n.scrollTop(n.scrollTop()-r.outerHeight())):(n.find("li").each(function(x,C){return 0<b(C).position().top?(r=b(C),!1):void 0}),r.addClass("ui-timepicker-selected")),!1;case 40:return r=n.find(".ui-timepicker-selected"),0===r.length?
(n.find("li").each(function(x,C){return 0<b(C).position().top?(r=b(C),!1):void 0}),r.addClass("ui-timepicker-selected")):r.is(":last-child")||(r.removeClass("ui-timepicker-selected"),r.next().addClass("ui-timepicker-selected"),r.next().position().top+2*r.outerHeight()>n.outerHeight()&&n.scrollTop(n.scrollTop()+r.outerHeight())),!1;case 27:n.find("li").removeClass("ui-timepicker-selected");A.hide();break;case 9:A.hide();break;default:return!0}}function w(f){var l=b(this),n=l.data("timepicker-list"),
r=l.data("timepicker-settings");if(!n||!a(n)||r.disableTextInput)return!0;switch(f.keyCode){case 96:case 97:case 98:case 99:case 100:case 101:case 102:case 103:case 104:case 105:case 48:case 49:case 50:case 51:case 52:case 53:case 54:case 55:case 56:case 57:case 65:case 77:case 80:case 186:case 8:case 46:r.typeaheadHighlight?p(l,n):n.hide()}}function B(f){var l=f.data("timepicker-settings"),n=null,r=f.data("timepicker-list").find(".ui-timepicker-selected");return r.hasClass("ui-timepicker-disabled")?
!1:(r.length&&(n=r.data("time")),null!==n&&("string"!=typeof n&&(n=D(n,l)),t(f,n,"select")),!0)}function F(f,l){f=Math.abs(f);var n,r;f=Math.round(f/60);var x=[];return 60>f?x=[f,E.mins]:(n=Math.floor(f/60),r=f%60,30==l&&30==r&&(n+=E.decimal+5),x.push(n),x.push(1==n?E.hr:E.hrs),30!=l&&r&&(x.push(r),x.push(E.mins))),x.join(" ")}function D(f,l){if("number"!=typeof f)return null;var n=parseInt(f%60),r=parseInt(f/60%60),x=parseInt(f/3600%24);x=new Date(1970,0,2,x,r,n,0);if(isNaN(x.getTime()))return null;
if("function"===b.type(l.timeFormat))return l.timeFormat(x);for(var C="",H=0;H<l.timeFormat.length;H++)switch(n=l.timeFormat.charAt(H)){case "a":C+=11<x.getHours()?E.pm:E.am;break;case "A":C+=11<x.getHours()?E.PM:E.AM;break;case "g":n=x.getHours()%12;C+=0===n?"12":n;break;case "G":n=x.getHours();f===G&&(n=l.show2400?24:0);C+=n;break;case "h":n=x.getHours()%12;0!==n&&10>n&&(n="0"+n);C+=0===n?"12":n;break;case "H":n=x.getHours();f===G&&(n=l.show2400?24:0);C+=9<n?n:"0"+n;break;case "i":r=x.getMinutes();
C+=9<r?r:"0"+r;break;case "s":n=x.getSeconds();C+=9<n?n:"0"+n;break;case "\\":H++;C+=l.timeFormat.charAt(H);break;default:C+=n}return C}function z(f,l){if(""===f||null===f)return null;if("object"==typeof f)return 3600*f.getHours()+60*f.getMinutes()+f.getSeconds();if("string"!=typeof f)return f;f=f.toLowerCase().replace(/[\s\.]/g,"");"a"!=f.slice(-1)&&"p"!=f.slice(-1)||(f+="m");var n="("+E.am.replace(".","")+"|"+E.pm.replace(".","")+"|"+E.AM.replace(".","")+"|"+E.PM.replace(".","")+")?",r=f.match(new RegExp("^"+
n+"([0-9]?[0-9])\\W?([0-5][0-9])?\\W?([0-5][0-9])?"+n+"$"));if(!r)return null;f=parseInt(1*r[2],10);if(24<f){if(l&&!1===l.wrapHours)return null;f%=24}n=r[1]||r[5];var x=f;12>=f&&n&&(x=n==E.pm||n==E.PM,x=12==f?x?12:0:f+(x?12:0));r=3600*x+60*(1*r[3]||0)+(1*r[4]||0);12>f&&!n&&l&&l._twelveHourTime&&l.scrollDefault&&(l=r-l.scrollDefault(),0>l&&l>=G/-2&&(r=(r+G/2)%G));return r}var G=86400,E={am:"am",pm:"pm",AM:"AM",PM:"PM",decimal:".",mins:"mins",hr:"hr",hrs:"hrs"},A={init:function(f){return this.each(function(){var l=
b(this),n=[],r;for(r in b.fn.timepicker.defaults)l.data(r)&&(n[r]=l.data(r));n=b.extend({},b.fn.timepicker.defaults,n,f);if(n.lang&&(E=b.extend(E,n.lang)),n=c(n),l.data("timepicker-settings",n),l.addClass("ui-timepicker-input"),n.useSelect)e(l);else{if(l.prop("autocomplete","off"),n.showOn)for(var x in n.showOn)l.on(n.showOn[x]+".timepicker",A.show);l.on("change.timepicker",q);l.on("keydown.timepicker",u);l.on("keyup.timepicker",w);n.disableTextInput&&l.on("keydown.timepicker",v);q.call(l.get(0),
null,"initial")}})},show:function(f){var l=b(this),n=l.data("timepicker-settings");if(f&&f.preventDefault(),n.useSelect)return void l.data("timepicker-list").focus();h(l)&&l.blur();f=l.data("timepicker-list");if(!l.prop("readonly")&&(f&&0!==f.length&&"function"!=typeof n.durationTime||(e(l),f=l.data("timepicker-list")),!a(f))){l.data("ui-timepicker-value",l.val());p(l,f);A.hide();f.show();var r={};n.orientation.match(/r/)?r.left=l.offset().left+l.outerWidth()-f.outerWidth()+parseInt(f.css("marginLeft").replace("px",
""),10):r.left=l.offset().left+parseInt(f.css("marginLeft").replace("px",""),10);"t"==(n.orientation.match(/t/)?"t":n.orientation.match(/b/)?"b":l.offset().top+l.outerHeight(!0)+f.outerHeight()>b(window).height()+b(window).scrollTop()?"t":"b")?(f.addClass("ui-timepicker-positioned-top"),r.top=l.offset().top-f.outerHeight()+parseInt(f.css("marginTop").replace("px",""),10)):(f.removeClass("ui-timepicker-positioned-top"),r.top=l.offset().top+l.outerHeight()+parseInt(f.css("marginTop").replace("px",""),
10));f.offset(r);r=f.find(".ui-timepicker-selected");if(!r.length){var x=z(y(l));null!==x?r=k(l,f,x):n.scrollDefault&&(r=k(l,f,n.scrollDefault()))}r&&r.length?(r=f.scrollTop()+r.position().top-r.outerHeight(),f.scrollTop(r)):f.scrollTop(0);return n.stopScrollPropagation&&b(document).on("wheel.ui-timepicker",".ui-timepicker-wrapper",function(C){C.preventDefault();var H=b(this).scrollTop();b(this).scrollTop(H+C.originalEvent.deltaY)}),b(document).on("touchstart.ui-timepicker mousedown.ui-timepicker",
m),b(window).on("resize.ui-timepicker",m),n.closeOnWindowScroll&&b(document).on("scroll.ui-timepicker",m),l.trigger("showTimepicker"),this}},hide:function(f){f=b(this);var l=f.data("timepicker-settings");return l&&l.useSelect&&f.blur(),b(".ui-timepicker-wrapper").each(function(){var n=b(this);if(a(n)){var r=n.data("timepicker-input"),x=r.data("timepicker-settings");x&&x.selectOnBlur&&B(r);n.hide();r.trigger("hideTimepicker")}}),this},option:function(f,l){return"string"==typeof f&&"undefined"==typeof l?
b(this).data("timepicker-settings")[f]:this.each(function(){var n=b(this),r=n.data("timepicker-settings"),x=n.data("timepicker-list");"object"==typeof f?r=b.extend(r,f):"string"==typeof f&&(r[f]=l);r=c(r);n.data("timepicker-settings",r);x&&(x.remove(),n.data("timepicker-list",!1));r.useSelect&&e(n)})},getSecondsFromMidnight:function(){return z(y(this))},getTime:function(f){var l=y(this);if(!l)return null;l=z(l);if(null===l)return null;f||(f=new Date);f=new Date(f);return f.setHours(l/3600),f.setMinutes(l%
3600/60),f.setSeconds(l%60),f.setMilliseconds(0),f},isVisible:function(){var f=this.data("timepicker-list");return!(!f||!a(f))},setTime:function(f){var l=this.data("timepicker-settings"),n=l.forceRoundTime?g(z(f),l):D(z(f),l);return f&&null===n&&l.noneOption&&(n=f),t(this,n),this.data("timepicker-list")&&p(this,this.data("timepicker-list")),this},remove:function(){if(this.hasClass("ui-timepicker-input")){var f=this.data("timepicker-settings");return this.removeAttr("autocomplete","off"),this.removeClass("ui-timepicker-input"),
...
className:null,closeOnWindowScroll:!1,disableTextInput:!1,disableTimeRanges:[],disableTouchKeyboard:!1,durationTime:null,forceRoundTime:!1,maxTime:null,minTime:null,noneOption:!1,orientation:"l",roundingFunction:function(f,l){if(null===f)return null;if("number"!=typeof l.step)return f;var n=f%(60*l.step);return n>=30*l.step?f+=60*l.step-n:f-=n,f==G&&l.show2400?f:f%G},scrollDefault:null,selectOnBlur:!1,show2400:!1,showDuration:!1,showOn:["click","focus"],showOnFocus:!0,step:30,stopScrollPropagation:!1,
...
this;b(document).on("click",".js-orders-show-filters",function(){a.visibilityFilters(!0);a.clearFilters()});b(document).on("click",".js-orders-hide-filters",function(){a.visibilityFilters(!1);a.clearFilters()})},initSearch:function(a){var c=this,e=b("#s-orders-search"),d=[],g=null,m=function(t){c.pre_search_hash||(c.pre_search_hash=window.location.hash,b.order_list.options&&"split"===b.order_list.options.view&&c.clearFilters());switch(t.autocomplete_item_type){case "order":c.search_item_param=null;
b.wa.setHash("#/order/"+t.id+"/");e.val(t.value);break;case "contact":c.search_item_param="contact_id="+t.id;b.wa.setHash("#/orders/"+c.search_item_param+"/");e.val(t.value);break;case "product":c.search_item_param="product_id="+t.id;b.wa.setHash("#/orders/"+c.search_item_param+"/");e.val(t.value);break;case "coupon":c.search_item_param="coupon_id="+t.id;b.wa.setHash("#/orders/"+c.search_item_param+"/");break;case "tracking_number":c.search_item_param="tracking_number="+t.value;b.wa.setHash("#/orders/"+
c.search_item_param+"/");break;case "shipping":c.search_item_param="shipping_id="+t.id;b.wa.setHash("#/orders/"+c.search_item_param+"/");break;case "payment":c.search_item_param="payment_id="+t.id;b.wa.setHash("#/orders/"+c.search_item_param+"/");break;case "city":c.search_item_param="city="+t.value;b.wa.setHash("#/orders/"+c.search_item_param+"/");break;case "region":c.search_item_param="region="+((t.value||"").split(":")[1]||"");b.wa.setHash("#/orders/"+c.search_item_param+"/");break;case "country":c.search_item_param=
"country="+t.value;b.wa.setHash("#/orders/"+c.search_item_param+"/");break;case "item_code":c.search_item_param="item_code="+t.value;b.wa.setHash("#/orders/"+c.search_item_param+"/");break;default:c.pre_search_hash=null}};e.unbind("keydown").bind("keydown",function(t){if(13==t.keyCode||10==t.keyCode){if(g)return!1;var v=b(this);if(!b(this).val())return location.hash="#/orders/all/",v.autocomplete("close"),!1;if(d&&b.isArray(d)&&d.length)return m(d[0]),setTimeout(function(){v.autocomplete("close")},
150),!1}});e.autocomplete({minLength:1,delay:300,html:!0,select:function(t,v){m(v.item);return!1},source:function(t,v){g&&g.abort();b.orders.waLoadingBeforeLoad();g=b.getJSON("?action=autocomplete&type=order",t,function(u){d=u;v(u);b.orders.waLoadingLoaded()}).always(function(){g=null})}});var h=b(".js-orders-search"),k=function(){h.hasClass("js-orders-search-split-and-mobile")?b(".js-orders-search-show").removeClass("hidden"):b(".js-orders-search-show").addClass("hidden")},p=function(){b(".js-orders-search-show").removeClass("hidden");
h.removeClass("show");h.removeClass("hide");h.addClass("hidden")};if(a&&"object"===typeof a)if(Array.isArray(a))e.val(""),p();else{var q=c.pre_search_hash||window.location.hash,y;for(y in a){a=a[y];c.search_item_param=y+"="+a.id;c.pre_search_hash=q.replace("/"+c.search_item_param,"");k();e.val(a.name);h.addClass("show").removeClass("hidden");break}}b(document).on("click",".js-orders-search-show",function(t){t.preventDefault();k();h.removeClass("hidden");setTimeout(function(){h.addClass("show");e.focus()},
5)});b(document).on("click",".js-orders-search-hide",function(t){t.preventDefault();if(!h.hasClass("show"))return!1;h.addClass("hide");e.val("");setTimeout(function(){return p()},180);c.pre_search_hash&&c.search_item_param&&(t=c.pre_search_hash,b.order_list.options.view&&t.includes("view=")&&(t=t.replace(RegExp("view=(split|table|kanban)"),"view="+b.order_list.options.view)),t.includes(c.search_item_param)&&(t="#/orders/"),b.wa.setHash(t),c.search_item_param=null,c.pre_search_hash=null)})},initDropdown:function(){var a=
this,c=this,e=["params.","state_id=","courier_contact_id="],d=function(k){k.$button.css({background:"var(--highlighted-blue)"})},g=function(k){return k&&"/"===k.slice(-1)?k.slice(0,-1):k},m=function(k){return k.every(function(p){return!e.some(function(q){return p.startsWith(q)})})},h=!1;b(".js-orders-dropdown").waDropdown({hover:!1,items:".menu > li > a",ready:function(k){var p=g(window.location.hash);if(!p||!p.includes("hash=search")){var q=decodeURIComponent(c.getFiltersHashStorage());q&&(p=q)}if(p){var y=
decodeURIComponent(p).replace(RegExp("#(\\/?)orders(\\/?)|hash=search\\/$","g"),"").split("&").filter(Boolean);if(y.length){if(h||m(y))return h=!0,!1;q=k.$menu.find(".js-orders-link");q.length&&q.each(function(){var t=b(this);y.some(function(v){return v==t.data("param")})&&(k.$button.html(t.html()),d(k));y.some(function(v){return decodeURIComponent(t.attr("href")).includes(v)})&&(k.$button.html(t.html()),d(k))})}b(".js-remove-filters-link").toggleClass("hidden",!p.includes("hash=search"))}p=k.$menu.find('[href="'+
p+'"]');p.length&&k.$button.html(p.html())},change:function(k,p,q){k=b(p);var y=["params.storefront","params.sales_channel"],t=g(window.location.hash),v=k.data("param");p=!1;t.includes("hash=search")&&!v&&(v=decodeURIComponent(k.attr("href").replace(/#\/orders\/withoutSearch|\/$/g,"")));if(v){"string"===typeof t&&t.includes(c.search_item_param)&&(t=t.replace(c.search_item_param,""),c.pre_search_hash=c.search_item_param=null,b(".js-orders-search-hide").trigger("click"));var u=decodeURIComponent(t).replace(RegExp("#(\\/?)orders(\\/?)|hash=search\\/|&id=\\d+|$",
"g"),"").split("&").filter(Boolean);p=u.findIndex(function(z){return"state_id=new|processing|auth|paid"===z});-1!==p&&u.splice(p,1);p=$jscomp.makeIterator(v.split("="));var w=p.next().value;t=p.next().value;var B=u.findIndex(function(z){return z.includes(w)});if(p="all"===w){var F=!1;t.split("|").forEach(function(z){u.forEach(function(G,E){G.startsWith(z)&&(F=!0,u.splice(E,1))})});c.clearFilter(q.$wrapper);q.$before&&q.$before.parent().removeClass("selected");if(!F)return!1}else-1!==B?u[B]=w+"="+
t:u.push(v),b(".js-remove-filters-link").toggleClass("hidden",!1),b(".js-order-nav-brick").each(function(){b(this).removeClass("selected")});v=-1;"courier_contact_id"===w?v=u.findIndex(function(z){return z.includes("params.courier_id")}):"params.courier_id"===w&&(v=u.findIndex(function(z){return z.includes("courier_contact_id")}));-1!==v&&u.splice(v,1);u.reduceRight(function(z,G,E){y.some(function(A){return G.includes(A)})&&(null!==z?u.splice(E,1):z=E);return z},null);var D=[];B=u.filter(function(z){if(RegExp("^sort\\[\\d+\\]").test(z))return!0;
e.some(function(G){return z.startsWith(G)})&&D.push(z);return!1});v=D.join("&");D.length&&(v="&"+v);v=encodeURIComponent(v);t="";B.length&&(t="&"+B.join("&"));B="";m(D)?(B+="all/",c.clearFilters()):B+="hash=search/"+v+t+"/";c.setFiltersHashStorage(B);b.wa.setHash("/orders/"+B)}else location.replace(k.attr("href"));p||(d(q),q.$button.html(k.html()))}});b(".js-remove-filters-link").on("click",function(){a.clearFilters()})},visibilityFilters:function(a){var c=b(".js-order-nav-block-secondary");a?c.show():
...
a.filter_params_str||"";this.container=b("#order-list");this.templates=a.templates||{};"table"==a.view&&(this.container=b("#order-list").find("tbody:first"),this.$selectionMenu=this.options.$selectionMenu)&&(this.select_all_input=this.$selectionMenu.find(".s-order-list-select-all"),this.select_all_input.attr("checked",!1),b(".js-selection-dropdown").waDropdown({hide:!1}));if("split"==a.view){var c=b("#s-orders"),e=b("#s-order"),d=function(m){m=e.find(".back.order-list");m.addClass("js-order-hide mobile-only").show().attr("href",
"javascript:void(0);");m.off().on("click",function(){e.addClass("desktop-and-tablet-only");c.removeClass("desktop-and-tablet-only");b(window).off("hashchange")});setTimeout(function(){});b(window).unbind("hashchange");b(window).on("hashchange",function(){})};this.container.off().on("wa_order_mobile_loaded",function(m,h){b.order_list.isMobile()&&h&&b.order_list.id&&d(h)});this.container.on("click","li[data-order-id]",function(m){b("#skeleton_split").removeClass("desktop-and-tablet-only");e.removeClass("desktop-and-tablet-only");
...
e.options.id+"]").addClass("selected"))}catch(q){console&&console.log(q.stack);return}h&&m.unobserve(h);b(".lazyloading-progress-string").text(p.data.progress.loaded+" "+p.data.progress.of);b(".lazyloading-progress").hide();b(".lazyloading-chunk").text(p.data.progress.chunk);p.data.loaded>=p.data.total_count?b(".lazyloading-link").hide():(b(".lazyloading-link").show(),(h=e.container.find(".order:last").get(0))&&m.observe(h))}else console&&console.log("Error when loading orders: "+p.errors)}).error(function(p){console&&
...
return!1});(function(){m=new IntersectionObserver(function(k){0>=k[0].intersectionRatio||d<g&&c()});(h=e.container.find(".order:last").get(0))&&m.observe(h)})()}},initView:function(){function a(m){var h=b.order_list.getSelectedOrders(),k=function(){c.$selectionMenu.addClass("disabled");c.$selectionMenu.find(".js-selection-menu-loading").show();c.select_all_input.attr("disabled",!0);g(m,h,function(){c.$selectionMenu.removeClass("disabled");c.$selectionMenu.find(".js-selection-menu-loading").hide();
...
'"]').addClass("selected");d.attr("data-view-mode",this.options.view);e.find("li a").each(function(){var m=b(this),h=b(this).parents("li:first");m.attr("href","#/orders/view="+h.attr("data-view")+(b.order_list.options.filter_params_str?"&"+b.order_list.options.filter_params_str:"")+"/")});var g=function(m,h,k){var p=function(){var u=b(window),w=b.Deferred();if(!h.all||!c.options.lazy_loading||u.lazyLoad("get","stopped"))return w.resolve(),w;c.container.one("append_order_list.ensureOrderListChunkLoaded",
function(){w.resolve()});setTimeout(function(){c.container.off("append_order_list.ensureOrderListChunkLoaded");w.resolve()},2E3);u.lazyLoad("force");return w},q=function(u){var w=b.Deferred();if(b.isEmptyObject(u.data.orders))return w.resolve(),w;c.container.one("append_order_list.updateOrderListChunk",function(){w.resolve()});setTimeout(function(){c.container.off("append_order_list.updateOrderListChunk");w.resolve()},1E3);c.updateListItems(u.data.orders);return w},y=function(u){var w=b.Deferred(),
B=function(){c.container.find(".order:not(:hidden):first").length||b("#s-content").html(c.templates.dummy_no_orders_body)},F=[],D=c.filter_params;if(!b.isEmptyObject(u.data.orders))for(var z=0,G=u.data.orders.length;z<G;z++)!b.isEmptyObject(D)&&D.state_id&&("string"===b.type(D.state_id)&&D.state_id!==u.data.orders[z].state_id?F.push(u.data.orders[z].id):"array"===b.type(D.state_id)&&-1===D.state_id.indexOf(u.data.orders[z].state_id)&&F.push(u.data.orders[z].id));b.isEmptyObject(F)?(B(),w.resolve()):
(c.hideListItems(F).done(function(){B();w.resolve()}),setTimeout(function(){w.resolve()},1E3));return w},t=function(u){var w=b.Deferred();p().done(function(){q(u).done(function(){y(u).done(function(){b.order_list.updateCounters({state_counters:u.data.state_counters,common_counters:{pending:u.data.pending_count}});w.resolve()})})});return w},v=function(u,w){b.shop.jsonPost("?module=orders&action=performAction&id="+m+"&offset="+(u||0)+"&"+c.options.filter_params_str,h,function(B){"ok"===B.status?t(B).done(function(){B.data.offset<
B.data.total_count?v(B.data.offset,w):w(B)}):w(B)})};v(0,k)};if(c.$selectionMenu)c.$selectionMenu.off("click.order_list",".js-wf-action-item").on("click.order_list",".js-wf-action-item",function(){var m=b(this),h=m.data("action-id");if(!m.hasClass("disabled"))return h&&a(h),!1})},loadOrder:function(a){this.container.find(".selected").removeClass("selected");this.container.find("[data-order-id="+a+"]").addClass("selected");b.orders.load("?module=order&id= "+a+"&"+this.filter_params_str,{content:b("#s-order")},
function(){this.id=a;b(window).trigger("wa_loaded",[["split"]]);b("#order-list").trigger("wa_order_mobile_loaded",[a])})},getSelectedOrders:function(a){a=a||!1;var c={count:0,all:!1};if(this.select_all_input.attr("checked")){c.all=!0;var e=b.order_list.filter_params;a?(c.serialized=[],b.isEmptyObject(e)?c.serialized.push({name:"filter_params",value:""}):b.each(e,function(d,g){b.isArray(g)?b.each(g,function(m,h){c.serialized.push({name:"filter_params["+d+"]["+m+"]",value:h})}):c.serialized.push({name:"filter_params["+
d+"]",value:g})})):c.filter_params=b.isEmptyObject(e)?"":e;c.count=this.options.total_count}else a?c.serialized=b.order_list.container.find(".order.selected").map(function(){c.count+=1;return{name:"order_id[]",value:b(this).attr("data-order-id")}}).toArray():c.order_id=b.order_list.container.find(".order.selected").map(function(){c.count+=1;return b(this).attr("data-order-id")}).toArray();return c},initSortMenu:function(){var a=this,c=b(".js-orders-sort"),e=c.find(".js-sort-dropdown-skeleton");c=
c.find(".js-sort-dropdown");var d;d||c.waDropdown({hover:!1,items:".menu > li > a",ready:function(g){d=g;var m=a.sort[0]?a.sort[0]:"create_datetime",h=a.sort[1]?a.sort[1]:"desc";d.$button.data("sort")||(g.$wrapper.find(".count").addClass("hidden"),g.$button.find("."+h).removeClass("hidden"),g.$menu.find('a[data-sort="'+m+'"]').find("."+h).removeClass("hidden"));d.$button.data("sort",m);d.$button.data("order",h)},change:function(g,m,h){g=b(m);var k=h.$button.data();m=k.sort;k=k.order;g.data("sort")===
m?k="desc"===k?"asc":"desc":(m=g.data("sort"),k="desc");h.$menu.find(".count").addClass("hidden");g.find("."+k).removeClass("hidden");h.$button.find("."+k).removeClass("hidden").siblings(".count").addClass("hidden");g=$jscomp.makeIterator([m,k]);h=g.next().value;g=g.next().value;d.$button.data("sort",h).data("order",g);m=window.location.hash||"";m=m.replace(/(^[^#]*#\/*|\/$)/g,"");m.split("/")[1]||(m="#/orders/state_id=new|processing|auth|paid");m=m.replace(/(&*sort\[[01]\]=.*?[&\/]|&*sort\[[01]\]=.*?$)/g,
"");(k=0<m.indexOf("=")||m.includes("all"))&&(m=m.replace(/[\/&]$/,""));b.wa.setHash(m+((k?"&":"")+"sort[0]="+h+"&sort[1]="+g+"/"))}});c.removeClass("hidden");e.addClass("hidden")},initSidebar:function(){var a=b("#s-order-nav"),c=a.find(".selected").removeClass("selected");this.filter_params.state_id?b.isArray(this.filter_params.state_id)&&1<this.filter_params.state_id.length?(b("#s-pending-orders").addClass("selected"),b(".js-pending-orders").parent().addClass("selected")):a.find('[data-state-id="'+
this.filter_params.state_id+'"]').addClass("selected"):this.filter_params.contact_id?a.find("[data-contact-id="+this.filter_params.contact_id+"]").addClass("selected"):this.filter_params.storefront?(c=decodeURIComponent(this.filter_params.storefront),"/"===c.slice(-1)&&(c=c.slice(0,-1)),a.find('[data-storefront="'+c+'"]').addClass("selected")):b.order_list.filter_params_str?a.find('[href="'+window.location.hash+'"]:first').closest("li").addClass("selected").length||c.addClass("selected"):b("#s-all-orders").addClass("selected");
a.find(".js-orders-link").unbind("click.order_list").bind("click.order_list",function(){a.find(".selected").removeClass("selected");b(this).hasClass("js-order-nav-brick")?b(this).addClass("selected"):b(this).closest("li").addClass("selected");if(b(this).attr("href").replace(/(^[^#]*#\/*|\/$)/g,"")==b.orders.hash){var e=b.orders.hash.replace("orders/","");e!==b.orders.hash&&b.orders.ordersAction.apply(b.orders,e.split("/"))}})},updateSelectionMenuActionItemsByStates:function(a){var c=[];if("string"===
typeof a)c.push(a);else if(b.isArray(a))c=[].concat(a);else return;a=this.$selectionMenu.find(".wf-actions .js-wf-action-item");a.addClass("disabled");var e={};a.each(function(){var g=b(this),m=g.data("actionId");g=g.data("availableForStates").split(",");for(var h=0;h<c.length;h++){var k=c[h];k=""+k;-1!==g.indexOf(k)&&(e[k]=e[k]||[],e[k].push(m))}});var d=b.shop.intersectArrays(b.shop.getValues(e),!0);0<d.length&&a.filter(function(){return-1<b.inArray(b(this).data("actionId"),d)}).removeClass("disabled")},
...
a.enableAllSelectionMenuActionItems()):(a.$selectionMenu.hide(),a.$search_form&&a.$search_form.show())};c.off("select",".order").on("select",".order",function(k,p){(void 0!==p?p:1)?b(this).addClass("selected").find("input:first").attr("checked",!0).prop("checked",!0):(b(this).removeClass("selected").find("input:first").attr("checked",!1).prop("checked",!1),e.is(":checked")&&e.attr("checked",!1).prop("checked",!1));g();return!1});var m=function(){a.xhrs.printforms&&a.xhrs.printforms.abort();d({printforms:[]});
...
(k.attr("checked",!1),c.find(".order").trigger("select",!1,!1));m()});c.off("click",".order input").on("click",".order input",function(k){var p=k.shiftKey;k=this.checked;var q=b(this).parents(".order:first");k?q.addClass("selected"):(e.is(":checked")&&e.attr("checked",!1),q.removeClass("selected"));if(p&&k){(p=c.data("last_checked"))||(p=c.find(".order:first").trigger("select",!0));var y=q.prevAll(".selected[data-order-id="+p.attr("data-order-id")+"]");if(y.length){var t=q.prev();for(p=p.get(0);t.length&&
p!=t.get(0);)t.addClass("selected").find("input").attr("checked",!0),t=t.prev()}else if(y=q.nextAll(".selected[data-order-id="+p.attr("data-order-id")+"]"),y.length)for(t=q.next(),p=p.get(0);t.length&&p!=t.get(0);)t.addClass("selected").find("input").attr("checked",!0),t=t.next();c.data("last_checked")||y.length||p.trigger("selected",!1)}k&&c.data("last_checked",q);g();m()});var h=function(k,p){if(100<p.count)alert($_("Maximum of %d orders is allowed for bulk form printing.").replace("%d",100));else{var q=
"?module=orders&action=printformsDisplay";p=b.map(p.serialized,function(y){return y.name+"="+y.value}).join("&");k=b.map(k,function(y){return"form[]="+y}).join("&");q+="&"+p+"&"+k;b.order_list.plugin_hash&&(q+="&hash="+b.order_list.plugin_hash);k=(""+Math.random()).slice(2);window.open(q,k)}};a.$selectionMenu.off("click.printforms").on("click.printforms",".s-printform-item-button",function(k){k.preventDefault();a.xhrs.printforms&&a.xhrs.printforms.abort();a.xhrs.printforms=null;k=a.$selectionMenu.find(".s-printform-item :checkbox").map(function(){var q=
b(this);return q.is(":checked")?q.data("id"):!1}).toArray();if(k.length){var p=a.getSelectedOrders(!0);if(!(0>=p.count))return h(k,p),!1}})},updateTitle:function(a,c){c=parseInt(c,10)||0;var e="";this.options.state_names&&(this.filter_params.state_id?"string"===typeof this.filter_params.state_id?e=this.options.state_names[this.filter_params.state_id].name:b.isArray(this.filter_params.state_id)&&(e=$_("Processing")):e=$_("All orders"));document.title=c?"("+c+") "+e+(a||""):e+a},updateCounters:function(a){switch(this.options.view){case "split":a.all_count&&
b(".js-all-counter").text(a.all_count);break;case "kanban":var c=b(".s-kanban__list__count");for(d in a.state_counters)a.state_counters.hasOwnProperty(d)&&c.filter('[data-status-id="'+d+'"]').text(a.state_counters[d])}a.total_processing&&this.updateTotalProcessing(a.total_processing);(c=this.sidebar)||(c=b("#s-sidebar"));for(var e in a)if(a.hasOwnProperty(e)){var d=a[e];for(var g in d)if(d.hasOwnProperty(g)){var m="common_counters"==e?b(".js-"+g+"-counter"):"storefront_counters"===e?c.find("li[data-"+
e.replace("_counters","")+'="'+g+'"] .count'):c.find("li[data-"+e.replace("_counters","")+"-id="+g+"] .count");if("new"===g||m&&m.length){var h=parseInt(m.text(),10)||0;d[g]=""+d[g];"+"==d[g].substr(0,1)?h+=parseInt(d[g].substr(1),10)||0:"-"==d[g].substr(0,1)?(h-=parseInt(d[g].substr(1),10)||0,h=0>h?0:h):h=parseInt(d[g],10)||0;m.text(h);"new"===g&&(b.shop.updateAppCounter(h),this.updateTitle(this.options.title_suffix,parseInt(h,10)),b(document).find(".js-new-order-counter").text("+"+h))}}}},updateListItems:function(a){var c=
"template-order-list-"+this.options.view;if(document.getElementById(c)){var e=this.container;b.isArray(a)||(a=[a]);c=b("<div></div>").append(tmpl(c,{orders:a,states:this.options.state_names,state_counters:this.options.state_counters||{},state_icons:this.options.state_icons||{},check_all:"table"==this.options.view?this.select_all_input.attr("checked"):!1}));var d=b(".order",e);b(".order",c).each(function(){var g=b(this);d.filter("[data-order-id="+g.attr("data-order-id")+"]").replaceWith(g)});c.remove();
this.container.trigger("append_order_list",[a]);this.getAndUpdateCounters()}},getAndUpdateCounters:function(){var a=this,c=parseInt(this.container.find(".order:first").attr("data-order-id"),10)||0;b.getJSON(this.buildLoadListUrl(c,!0,!0),function(e){var d;"ok"===e.status&&(null==e?0:null==(d=e.data)?0:d.counters)&&a.updateCounters(e.data.counters)})},updateTotalProcessing:function(a){a&&b(".js-total_processing-counter").html(a.replace(",","."))},hideListItems:function(a){var c=b.Deferred();setTimeout(function(){if(b.order_list&&
b.order_list.container&&b.order_list.container.length){if(b.isArray(a)){var e=b.order_list.container.find(".order");m=b();for(var d=0;d<a.length;d++){var g=e.filter("[data-order-id="+a[d]+"]");m=m.add(g)}}else var m=b.order_list.container.find(".order[data-order-id="+a+"]");var h=m.length,k=0;m.slideUp(450,function(){k++;k>=h&&c.resolve()})}},1E3);return c.promise()},dispatch:function(a,c){"string"===typeof a&&(a=b.shop.helper.parseParams(a));var e={list:function(){b.orders.waLoadingBeforeLoad();
b.order_list.finit();b.orders.load("?module=orders&"+b.orders.buildOrdersUrlComponent(a),function(){b.order_list.dispatch(a);b(window).trigger("wa_loaded",[["table","kanban"]])})},order:function(){b.order_list.loadOrder(a.id)}};null===this.params?(this.params=a,c&&e.list()):a.id&&(c||a.id!=this.params.id)?(e.order(),this.params=a):(c||this.diff(a,this.params)||this.diff(this.params,a))&&e.list()},diff:function(a,c){for(var e in a)if(a.hasOwnProperty(e)&&"id"!=e&&a[e]!==c[e])return!0;return!1},buildLoadListUrl:function(a,
c,e,d,g,m){d=void 0===d?null:d;g=void 0===g?null:g;m=void 0===m?null:m;d&&this.filter_params_str&&this.filter_params_str.includes("hash")&&(this.filter_params_str+=encodeURIComponent("&update_datetime>="+d),d=null);g=g||this.sort;"kanban"===this.options.view&&"state_id=new|processing|auth|paid"===this.filter_params_str&&(this.filter_params_str=null);return"?module=orders&action=loadList"+(m?"&state_id="+m:"")+"&id="+a+(this.filter_params_str?"&"+this.filter_params_str:"")+(c?"&lt=1":"")+(e?"&counters=1":
"")+(e?"&prev_pending="+b(".js-pending-counter:first").text()||"0":"")+(this.options.view?"&view="+this.options.view:"")+(d?"&search=update_datetime>="+d:"")+("&sort[0]="+g[0]+"&sort[1]="+g[1])},updateProcess:function(a,c){var e=this;a=a||"run";c=c||{};timeout=c.timeout||6E4;var d=this,g=function(){null!==d.timer_id&&(clearTimeout(d.timer_id),d.timer_id=null);null!==d.xhrs.update_load&&(d.xhrs.update_load.abort(),d.xhrs.update_load=null)};g();if("run"==a){var m=function(q,y){var t=d.container.find(".order:first");
t=parseInt(t.attr("data-order-id"),10)||0;var v=k(c.last_update_datetime),u;(null==(u=d.options)?0:u.view)&&"kanban"===d.options.view&&(t=0,u=Array.from(document.querySelectorAll("#order-list [data-order-id]")).map(function(w){return w.getAttribute("data-order-id")}),u.length&&(t=Math.max.apply(Math,$jscomp.arrayFromIterable(u))));return b.getJSON(d.buildLoadListUrl(t,!0,!0,v),function(w){if("ok"==w.status){h(w);b.isEmptyObject(w.data.counters)||d.updateCounters(w.data.counters);if(w.data.count)try{if("kanban"===
d.options.view){var B=d.container.find('[data-kanban-list-status-id="new"]');B.length&&(w.data.orders.reverse(),w.data.orders.forEach(function(F){var D,z,G,E,A,f;return B.prepend(tmpl("template-order-list-kanban-card",{order:F,state:"new",state_name:(null==(D=d.options)?void 0:null==(z=D.state_names["new"])?void 0:z.name)||"",state_color:(null==(G=d.options)?void 0:null==(E=G.state_names["new"])?void 0:null==(A=E.options)?void 0:null==(f=A.style)?void 0:f.color)||""}))}))}else d.container.prepend(tmpl("template-order-list-"+
d.options.view,{orders:w.data.orders,states:d.options.state_names||{},state_counters:d.options.state_counters||{},state_icons:d.options.state_icons||{},check_all:"table"==d.options.view?d.select_all_input.attr("checked"):!1})),b(".lazyloading-progress-string").text(w.data.progress.loaded+" "+w.data.progress.of),d.container.trigger("append_order_list",[w.data.orders])}catch(F){console&&(console.log("Error: "+F.message),y());return}"function"===typeof q&&q(w)}else console&&console.log("Error when loading new orders: "+
w.errors),"function"===typeof y&&y()}).error(function(w){console&&(w&&w.errors?console.log("Error when loading new orders: "+w.errors):console.log(["Error when loading new orders",w]));"function"===typeof y&&y()})},h=function(q){if(!b.isEmptyObject(q.data.updated_orders))try{for(var y=[],t=$jscomp.makeIterator(q.data.updated_orders),v=t.next(),u={};!v.done;u={order$jscomp$3:u.order$jscomp$3,newStateId:u.newStateId,orderElem:u.orderElem,oldStateId:u.oldStateId,hashStateIds:u.hashStateIds,targetColumn:u.targetColumn},
v=t.next()){u.order$jscomp$3=v.value;u.orderElem=d.container.find("[data-order-id="+u.order$jscomp$3.id+"]");var w=u.orderElem.hasClass("selected");if(u.orderElem.length){u.oldStateId=u.orderElem.data("state-id");u.newStateId=u.order$jscomp$3.state_id;var B=decodeURIComponent(window.location.hash),F=void 0,D=void 0,z=void 0;u.hashStateIds=(null==(F=/state_id=([\w|]+)/.exec(B))?void 0:null==(D=F[1])?void 0:null==(z=D.replace(/\|{1,2}/g,","))?void 0:z.split(","))||[];var G=function(A){return function(f){var l=
{states:d.options.state_names||{}},n="";if("kanban"===d.options.view){n="-card";A.order$jscomp$3.order_icon=A.order$jscomp$3.contact.photo_50x50;l.order=A.order$jscomp$3;l.state=A.newStateId;var r;l.state_name=null==(r=l.states[l.state])?void 0:r.name;var x;l.state_color=(null==(x=l.states[l.state])?void 0:x.options.style.color)||"#fff";l.isUpdated=!0}else l.orders=[A.order$jscomp$3],l.state_counters=d.options.state_counters||{},l.state_icons=d.options.state_icons||{},l.check_all="table"==d.options.view?
d.select_all_input.attr("checked"):!1;A.orderElem.after(tmpl("template-order-list-"+d.options.view+n,l)).remove();f&&d.loadOrder(A.order$jscomp$3.id)}}(u);if(function(A){return function(){if(A.oldStateId!==A.newStateId){if(A.hashStateIds.some(function(l){return A.newStateId.includes(l)})||"kanban"===e.options.view)return!0;var f=A.orderElem.next();f.length&&d.loadOrder(f.data("order-id"));A.orderElem.remove();return!1}return!0}}(u)()){if(G(w),u.oldStateId!==u.newStateId){var E=d.container.find('[data-order-id="'+
u.order$jscomp$3.id+'"]');u.targetColumn=d.container.find('[data-kanban-list-status-id="'+u.newStateId+'"]');E.slideToggle("normal",function(A){return function(){b(this).prependTo(A.targetColumn).slideToggle("normal")}}(u))}}else u.orderElem.remove();y.push(u.order$jscomp$3.update_datetime)}}k(y.sort()[y.length-1]);d.container.trigger("append_order_list",[q.data.updated_orders])}catch(A){console&&console.log("Error: "+A.message)}},k=function(q){function y(v){return v.toISOString().replace("T"," ").substring(0,
19)}q||(q=new Date,q.setDate(q.getDate()-1),q=y(q));q=new Date(q+"Z");var t=y(q);q.setTime(q.getTime()+timeout);c.last_update_datetime=y(q);return t},p=function(){d.timer_id=setTimeout(function(){null===d.xhrs.lazy_load?d.xhrs.update_load=m(p,g):d.xhrs.lazy_load.then(function(){d.xhrs.update_load=m(p,g)})},timeout)};p()}},finit:function(a){a=this.xhrs;var c=this.lazy_load_win,e;for(e in a)a.hasOwnProperty(e)&&null!==a[e]&&a[e].abort();this.updateProcess("kill");c&&c.lazyLoad("stop");this.params=null},
ordersNavTeleport:function(){var a=window.ordersNav;a&&b.orders&&(a.show(),"split"===this.options.view?b("#s-orders").find(".sidebar-header").prepend(a):b("#s-content").before(a),b.orders.initDropdown(),"split"===this.options.view||this.isMobile()?(a=b(".js-orders-search").detach(),a.addClass("js-orders-search-split-and-mobile"),b("#s-order-nav .js-order-nav-view").after(a)):(a=b(".js-orders-search-split-and-mobile"),a.length&&(a.removeClass("js-orders-search-split-and-mobile"),b("#s-order-nav .s-order-nav-search").append(a.detach()))))},
selectionMenuTeleport:function(){if(this.$selectionMenu&&(this.$search_form=b(".js-sidebar-search-form"),this.$search_form.length)){var a=this.$selectionMenu.detach();this.$search_form.after(a)}},isMobile:function(){return b("#js-media-mobile").is(":visible")}}})(jQuery);$.order_edit={id:0,container:null,form:null,customer_form:null,$storefront_selector:null,stocks:[],slide_on:!1,options:{},locales:{},init:function(b){this.options=b;b.id&&(this.id=b.id);this.container="string"===typeof b.container?$(b.container):b.container;this.form="string"===typeof b.form?$(b.form):b.form;this.$storefront_selector=$("#order-storefront");b.stocks.sort(function(a,c){return a.sort-c.sort});this.stocks=b.stocks;b.title&&(document.title=title);b.float_delimeter||(b.float_delimeter=
...
minLength:3,delay:300,select:function(h,k){$(".s-order-errors").empty();h="?module=orders&action=getProduct&product_id="+k.item.id+"&order_id="+$.order_edit.id+"&currency="+$.order_edit.options.currency;(k=b.getStorefront())&&(h+="&storefront="+k);$.getJSON(h,function(p){var q=$("#order-items"),y=parseInt(q.find(".s-order-item:last").attr("data-index"),10)+1||0;q=p.data.product;q.sku_id&&q.skus[q.sku_id]&&(q.skus[q.sku_id].checked=!0);1===$("#subtotal.hidden").length&&($("#productless-subtotal").addClass("hidden").prop("disabled",
!0).hide(),$("#subtotal").removeClass("hidden").show());$("#order-currency").length&&!$("#order-currency").attr("disabled")&&($('<input type="hidden" name="currency">').val($("#order-currency").val()).insertAfter($("#order-currency")),$("#order-currency").attr("disabled","disabled"));var t=$("#s-orders-add-row");t.before(tmpl("template-order",{data:p.data,options:{index:y,currency:$.order_edit.options.currency,stocks:$.order_edit.stocks,price_edit:e}}));p=t.prev();y=q.skus[q.sku_id];$.order_edit.initQuantity({$wrapper:p.find("#js-quantity-section").removeAttr("id"),
denominator:q.order_multiplicity_factor,step:y.order_count_step||q.order_count_step,min:y.order_count_min||q.order_count_min});$("#s-order-comment-edit").show();$.order_edit.updateTotal();c(p)});d.val("");return!1}});this.container.off("change",".s-orders-skus input[type=radio]").on("change",".s-orders-skus input[type=radio]",function(){var h=$(this),k=h.parents("tr:first"),p=h.closest("li"),q=this.value,y=k.attr("data-product-id"),t=k.attr("data-index"),v=null;"edit"==($.order_edit.id?"edit":"add")&&
(v=parseInt(h.attr("name").replace("sku[edit][",""),10));h="?module=orders&action=getProduct&product_id="+y+"&sku_id="+q+"&currency="+$.order_edit.options.currency;(q=b.getStorefront())&&(h+="&storefront="+q);$.getJSON(h,function(u){var w=-1!==k.find("input:first").attr("name").indexOf("add")?"add":"edit";k.find(".s-orders-services").replaceWith(tmpl("template-order-services-"+w,{services:u.data.sku.services,service_ids:u.data.service_ids,product_id:y,options:{price_edit:e,index:t,currency:$.order_edit.options.currency,
stocks:$.order_edit.stocks}}));k.find(".s-orders-product-price").find("input").val(u.data.sku.price);k.find(".s-orders-sku-stock-place").empty();p.find(".s-orders-sku-stock-place").html(tmpl("template-order-stocks-"+w,{sku:u.data.sku,index:t,stocks:$.order_edit.stocks,item_id:v}));var B=k.find(".js-order_count_min");if(B.length){w=u.data.sku;B=B.find(".js-value");var F=u.data.sku.order_count_min?parseFloat(u.data.sku.order_count_min):parseFloat(u.data.product.order_count_min);B.text(F);B=parseFloat(w.count);
F=k.find(".s-orders-sku-stock-select");if(F.length&&w.stock){var D=F.val();D&&w.stock[D]&&(B=parseFloat(w.stock[D]));F.data("stock",w.stock)}F=k.find(".js-quantity-field").data("controller");F.update({min:w.order_count_min?parseFloat(w.order_count_min):parseFloat(u.data.product.order_count_min),max:B});console.log(F)}c(k);$.order_edit.updateTotal()})});this.container.off("change",".s-orders-sku-stock-select").on("change",".s-orders-sku-stock-select",function(){var h=$(this).parents("tr.s-order-item:first"),
k=h.find(".s-orders-sku-stock-select");if(k.length){var p=k.val();k=k.data("stock");if(p&&k&&k[p]){var q=h.find(".js-quantity-field").data("controller");q.update({max:parseFloat(k[p])});console.log(q)}}c(h)});var g=function(h,k,p){var q=h.attr("data-price");h=h.attr("data-percent-price");p&&k.val(q);k.attr("data-price",q);k.attr("data-percent-price",h)};this.container.off("change",".s-orders-service-variant").on("change",".s-orders-service-variant",function(){var h=$(this),k=h.find("option:selected");
...
k.discount).data("description",k.discount_description).data("items_discount",k.items_discount);if("calculate"===e.data("discount")){c.val(e.data("description"));h=e.data("items_discount")||[];for(var p=0;p<h.length;p++)if(h[p]){var q="#order-items span.js-item-total-discount";q+='[data-discount-id="'+h[p].selector+'"]';q=$(q);q.length&&q.html(h[p].html).show()}b()}"calculate"===e.data("discount")?$.order_edit.parseFloat(k.discount)!==$.order_edit.parseFloat(a.val())&&e.click():e.show()}});var m=function(){a.show();
...
$.order_edit.updateTotal();b()});this.container.trigger("order_edit_init_discount")},initCouponControl:function(){function b(){q.show();q.css("padding-right",$(".coupon-controls").width()+4+"px")}var a=$("#coupon_id"),c=$("#coupon-code"),e=$(".coupon-code-label"),d=$(".js-no-coupon-text"),g=$(".js-coupon-icon"),m=$(".js-edit-coupon"),h=$(".js-close-coupon"),k=$(".js-delete-coupon"),p=$("#update-discount"),q=$("#js-coupon-invalid-msg");m.on("click",function(){$(this).add(d).add(g).add(e).hide();c.data("value",
c.val());c.val("").show();h.show()});h.click(function(){c.val(c.data("value"));m.show();""===a.val()?d.show():g.add(e).show();$(this).add(c).add(q).hide()});k.click(function(){$(this).add(c).add(g).add(e).add(m).add(h).add(q).hide();a.val("");p.click();d.add(m).show()});$(".disabled-link").click(function(y){y.preventDefault()});c.autocomplete({source:function(y,t){$.ajax({type:"POST",url:"?module=marketing&action=CouponsAutocomplete",data:{term:c.val(),products:$('input[name^="product["]').serializeArray()},
success:function(v){t(v);v.length?q.hide():b()},error:function(){t([])}})},minLength:1,delay:300,select:function(y,t){t.item.data.valid?(c.val(t.item.label).hide(),a.val(t.item.value),e.html(t.item.label).show(),m.add(g).add(k).show(),t.item.data.right?($(".s-order-edit-coupon").attr("href",$(".s-order-edit-coupon").data("href")+t.item.value),0!=t.item.value.length&&(p.click(),$(".s-order-edit-coupon").removeClass("disabled-link"))):0!=t.item.value.length&&p.click(),h.hide()):(c.val(t.item.label),
...
d){var g=d="string"===typeof d?d:""+d;switch(e){case "float":e=parseFloat(d);0<=e&&(g=1*e.toFixed(3));break;case "number":var m="0123456789.,".split(""),h=[],k=!1;$.each(d.split(""),function(p,q){"."===q||","===q?k||(k=!0,h.push(".")):0<=m.indexOf(q)&&h.push(q)});g=h.join("");break;case "integer":m="0123456789".split(""),h=[],$.each(d.split(""),function(p,q){0<=m.indexOf(q)&&h.push(q)}),g=h.join("")}return g}var c=function(e){c=function(d){this.$wrapper=d.$wrapper;this.$field=this.$wrapper.find(".js-quantity-field");
...
this.value=this.validate(d);this.init()};c.prototype.init=function(){function d(m){var h="0123456789.".split(""),k=[],p=!1;m=m.replace(/,/g,".");e.each(m.split(""),function(q,y){"."===y?p||(p=!0,k.push(y)):0<=h.indexOf(y)&&k.push(y)});return k.join("")}var g=this;g.$field.data("controller",g);g.$wrapper.data("controller",g);g.$field.on("input",function(){var m=g.$field.val();var h=d("string"===typeof m?m:""+m);h!==m&&g.$field.val(h)});g.$field.on("change",function(){g.set(g.$field.val())})};c.prototype.set=
...
k=e.find("select.s-orders-sku-stock-select").val(),p=e.data("item-id"),q=e.find("input[name^=sku]:not(:radio)").add(e.find("input[name^=sku]:checked")).first().val()||0,y={product_id:d,sku_id:q,services:[]};e.find(".s-orders-services").length&&e.find(".s-orders-services input:checkbox:checked").each(function(){var t=$(this).closest("li"),v=$.order_edit.parseFloat(t.find("input.s-orders-service-price").val()),u=$(this).val();t=t.find(':input[name^="variant["]').val();g.push({id:u,price:v,variant_id:t});
y.services.push(u)});y.services.sort();y.services=y.services.join("_");a.push({id:p,product_id:d,quantity:h,price:m,sku_id:q,services:g,stock_id:k});c.push(y)});c=this.reduceOrderContent(c);b.data("order-content",c);return a},reduceOrderContent:function(b){b.sort(function(a,c){var e=a.product_id-c.product_id;0===e&&(e=a.sku_id-c.sku_id);0===e&&(e=a.services.localeCompare(c.services));return e});return b.map(function(a){return"*"+a.product_id+":"+a.sku_id+":"+a.services}).join(";")},updateTotal:function(){function b(){a=
...
g.params={shipping_id:b,coupon_id:h,payment_id:m};g["customer[id]"]=g.contact_id=$("#s-customer-id").val();g.tax="calculate";b&&(b=parseInt(b.split(".")[0]),0<b&&$("#shipping-custom").find(">#shipping-custom-"+b+" :input").each(function(){g[this.name]=this.value}));$.ajax({type:"POST",url:"?module=order&action=total",data:g,success:function(p){function q(r){var x=$("#payment_methods"),C=$("#payment-info"),H=x.val();x.html("");C.html("");x.append("<option value=''></option>");x.val("");r.length&&$.each(r,
function(L,I){x.append('<option value="'+I.id+'" '+(I.is_truncated?'title="'+I.name+'"':"")+">"+I.truncated_name+"</option>");I.id===H&&(x.val(H),I.custom_html&&$("<div />",{id:"payment-custom-"+I.id,class:"fields form"}).html(I.custom_html).appendTo(C))})}if(p&&"ok"===p.status){var y=$("#shipping-rate"),t=$("#shipping_methods"),v=$.trim(t.val())||"",u=v.replace(/\W.+$/,"");p.data&&(p.data=$.order_edit.parseTotalResponse(p.data));t.empty();t.prepend('<option value=""></option>');if(0<p.data.shipping_method_ids.length){var w=
$("#shipping-custom");w.empty();var B=p.data.shipping_method_ids,F=p.data.shipping_methods,D=!1,z=!1;-1!==B.indexOf(v)&&(D=!0);D||-1===B.indexOf(Number(u))||(z=D=!0);w.find(">div.fields.form").hide();for(var G=0;G<B.length;G+=1){var E=""+B[G],A=F[E],f=E.replace(/\W.+$/,""),l=$("<option></option>");l.html(A.truncated_name).attr("value",E).data("rate",A.rate);l.data("error",A.error||void 0);l.data("est_delivery",A.est_delivery||void 0);l.data("comment",A.comment||void 0);l.data("external",A.external||
!1);A.is_truncated&&l.attr("title",A.name);t.append(l);v||0!==G||$("#shipping-rate").val($.order_edit.formatFloat(A.rate));if(u==f){if(A.custom_data)for(var n in A.custom_data)A.custom_data.hasOwnProperty(n)&&l.data(n,A.custom_data[n]);A.custom_html&&(l=w.find("#shipping-custom-"+f),l.length||(l=w.append('<div id="shipping-custom-'+f+'" class="fields form"></div>'),l=w.find("#shipping-custom-"+f)),l.html(A.custom_html))}if(!D&&(A=(""+E).split("."),f=v.split("."),""!==f[0]&&void 0!==A[0]&&void 0!==
f[0]&&A[0]==f[0])){v=E;t.val(v);$.order_edit.setShippingInfo();$.order_edit.realUpdateTotal();return}}t.val(v);z&&t.val(u);t.data("_shipping_id",u);$.shop.trace("set shipping_methods id",[u,v])}q(p.data.payment_methods);""!=k&&$("#discount").val(p.data.discount);y.val(p.data.shipping);$("#order-edit-form").trigger("order_total_updated",p.data);a.text(p.data.subtotal);c.text(p.data.total);e.text(p.data.tax);$.order_edit.showValidateErrors(p.data.errors);d.setShippingInfo()}$.order_edit.switchSubmitButton()},
...
$jscomp.makeIterator(h);for(var q=k.next();!q.done;q=k.next()){q=q.value;var y=b('[name="'+e.namespace+"[address."+q+'][region]"]');0<y.length&&""!=y.data("country")&&(g[e.namespace+"[address."+q+"][data][country]"]=y.data("country"))}}g[p.name]=p.value})}b.post("?action=customerForm",g,function(k){e.updateInnerHtml(k);m.hide();k=e.getContact(!0);c.afterReload&&c.afterReload(k);d.trigger("afterReload",[k])})};ShopBackendCustomerForm.prototype.bind=function(a,c){this.$wrapper.on(a+".custom"+this.dom_ns,
