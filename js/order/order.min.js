...
a):(c||this.diff(a,this.params)||this.diff(this.params,a))&&e.list()},diff:function(a,c){for(var e in a)if(a.hasOwnProperty(e)&&"id"!=e&&a[e]!==c[e])return!0;return!1},buildLoadListUrl:function(a,c,e,d,g,l){d=void 0===d?null:d;g=void 0===g?null:g;l=void 0===l?null:l;d&&this.filter_params_str&&this.filter_params_str.includes("hash")&&(this.filter_params_str+=encodeURIComponent("&update_datetime>="+d),d=null);g=g||this.sort;"kanban"===this.options.view&&"state_id=new|processing|auth|paid"===this.filter_params_str&&
(this.filter_params_str=null);return"?module=orders&action=loadList"+(l?"&state_id="+l:"")+"&id="+a+(this.filter_params_str?"&"+this.filter_params_str:"")+(c?"&lt=1":"")+(e?"&counters=1":"")+(this.options.view?"&view="+this.options.view:"")+(d?"&search=update_datetime>="+d:"")+("&sort[0]="+g[0]+"&sort[1]="+g[1])},updateProcess:function(a,c){var e=this;a=a||"run";c=c||{};timeout=c.timeout||6E4;var d=this,g=function(){null!==d.timer_id&&(clearTimeout(d.timer_id),d.timer_id=null);null!==d.xhrs.update_load&&
(d.xhrs.update_load.abort(),d.xhrs.update_load=null)};g();if("run"==a){var l=function(r,y){var q=d.container.find(".order:first");q=parseInt(q.attr("data-order-id"),10)||0;var u=k(c.last_update_datetime),v;(null==(v=d.options)?0:v.view)&&"kanban"===d.options.view&&(q=0,v=Array.from(document.querySelectorAll("#order-list [data-order-id]")).map(function(w){return w.getAttribute("data-order-id")}),v.length&&(q=Math.max.apply(Math,$jscomp.arrayFromIterable(v))));return b.getJSON(d.buildLoadListUrl(q,
!0,!0,u),function(w){if("ok"==w.status){h(w);b.isEmptyObject(w.data.counters)||d.updateCounters(w.data.counters);if(w.data.count)try{if("kanban"===d.options.view){var B=d.container.find('[data-kanban-list-status-id="new"]');if(B.length){w.data.orders.reverse();var G,D,z,F,E,A;w.data.orders.forEach(function(f){return B.prepend(tmpl("template-order-list-kanban-card",{order:f,state:"new",state_name:(null==(G=d.options)?void 0:null==(D=G.state_names["new"])?void 0:D.name)||"",state_color:(null==(z=d.options)?
void 0:null==(F=z.state_names["new"])?void 0:null==(E=F.options)?void 0:null==(A=E.style)?void 0:A.color)||""}))})}}else d.container.prepend(tmpl("template-order-list-"+d.options.view,{orders:w.data.orders,states:d.options.state_names||{},state_counters:d.options.state_counters||{},state_icons:d.options.state_icons||{},check_all:"table"==d.options.view?d.select_all_input.attr("checked"):!1})),b(".lazyloading-progress-string").text(w.data.progress.loaded+" "+w.data.progress.of),d.container.trigger("append_order_list",
[w.data.orders])}catch(f){console&&(console.log("Error: "+f.message),y());return}"function"===typeof r&&r(w)}else console&&console.log("Error when loading new orders: "+w.errors),"function"===typeof y&&y()}).error(function(w){console&&(w&&w.errors?console.log("Error when loading new orders: "+w.errors):console.log(["Error when loading new orders",w]));"function"===typeof y&&y()})},h=function(r){if(!b.isEmptyObject(r.data.updated_orders))try{for(var y=[],q={},u=$jscomp.makeIterator(r.data.updated_orders),
v=u.next();!v.done;q={$jscomp$loop$prop$order$5:q.$jscomp$loop$prop$order$5,$jscomp$loop$prop$newStateId$9:q.$jscomp$loop$prop$newStateId$9,$jscomp$loop$prop$orderElem$11:q.$jscomp$loop$prop$orderElem$11,$jscomp$loop$prop$oldStateId$8:q.$jscomp$loop$prop$oldStateId$8,$jscomp$loop$prop$hashStateIds$10:q.$jscomp$loop$prop$hashStateIds$10,$jscomp$loop$prop$targetColumn$12:q.$jscomp$loop$prop$targetColumn$12},v=u.next()){q.$jscomp$loop$prop$order$5=v.value;q.$jscomp$loop$prop$orderElem$11=d.container.find("[data-order-id="+
q.$jscomp$loop$prop$order$5.id+"]");var w=q.$jscomp$loop$prop$orderElem$11.hasClass("selected");if(q.$jscomp$loop$prop$orderElem$11.length){q.$jscomp$loop$prop$oldStateId$8=q.$jscomp$loop$prop$orderElem$11.data("state-id");q.$jscomp$loop$prop$newStateId$9=q.$jscomp$loop$prop$order$5.state_id;var B=decodeURIComponent(window.location.hash),G=void 0,D=void 0,z=void 0;q.$jscomp$loop$prop$hashStateIds$10=(null==(G=/state_id=([\w|]+)/.exec(B))?void 0:null==(D=G[1])?void 0:null==(z=D.replace(/\|{1,2}/g,
","))?void 0:z.split(","))||[];var F=function(A){return function(f){var m={states:d.options.state_names||{}},n="";if("kanban"===d.options.view){n="-card";A.$jscomp$loop$prop$order$5.order_icon=A.$jscomp$loop$prop$order$5.contact.photo_50x50;m.order=A.$jscomp$loop$prop$order$5;m.state=A.$jscomp$loop$prop$newStateId$9;var t;m.state_name=null==(t=m.states[m.state])?void 0:t.name;var x;m.state_color=(null==(x=m.states[m.state])?void 0:x.options.style.color)||"#fff";m.isUpdated=!0}else m.orders=[A.$jscomp$loop$prop$order$5],
m.state_counters=d.options.state_counters||{},m.state_icons=d.options.state_icons||{},m.check_all="table"==d.options.view?d.select_all_input.attr("checked"):!1;A.$jscomp$loop$prop$orderElem$11.after(tmpl("template-order-list-"+d.options.view+n,m)).remove();f&&d.loadOrder(A.$jscomp$loop$prop$order$5.id)}}(q);if(function(A){return function(){if(A.$jscomp$loop$prop$oldStateId$8!==A.$jscomp$loop$prop$newStateId$9){if(A.$jscomp$loop$prop$hashStateIds$10.some(function(m){return A.$jscomp$loop$prop$newStateId$9.includes(m)})||
"kanban"===e.options.view)return!0;var f=A.$jscomp$loop$prop$orderElem$11.next();f.length&&d.loadOrder(f.data("order-id"));A.$jscomp$loop$prop$orderElem$11.remove();return!1}return!0}}(q)()){if(F(w),q.$jscomp$loop$prop$oldStateId$8!==q.$jscomp$loop$prop$newStateId$9){var E=d.container.find('[data-order-id="'+q.$jscomp$loop$prop$order$5.id+'"]');q.$jscomp$loop$prop$targetColumn$12=d.container.find('[data-kanban-list-status-id="'+q.$jscomp$loop$prop$newStateId$9+'"]');E.slideToggle("normal",function(A){return function(){b(this).prependTo(A.$jscomp$loop$prop$targetColumn$12).slideToggle("normal")}}(q))}}else q.$jscomp$loop$prop$orderElem$11.remove();
y.push(q.$jscomp$loop$prop$order$5.update_datetime)}}k(y.sort()[y.length-1]);d.container.trigger("append_order_list",[r.data.updated_orders])}catch(A){console&&console.log("Error: "+A.message)}},k=function(r){function y(u){return u.toISOString().replace("T"," ").substring(0,19)}r||(r=new Date,r.setDate(r.getDate()-1),r=y(r));r=new Date(r+"Z");var q=y(r);r.setTime(r.getTime()+timeout);c.last_update_datetime=y(r);return q},p=function(){d.timer_id=setTimeout(function(){null===d.xhrs.lazy_load?d.xhrs.update_load=
l(p,g):d.xhrs.lazy_load.then(function(){d.xhrs.update_load=l(p,g)})},timeout)};p()}},finit:function(a){a=this.xhrs;var c=this.lazy_load_win,e;for(e in a)a.hasOwnProperty(e)&&null!==a[e]&&a[e].abort();this.updateProcess("kill");c&&c.lazyLoad("stop");this.params=null},ordersNavTeleport:function(){var a=window.ordersNav;a&&b.orders&&(a.show(),"split"===this.options.view?b("#s-orders").find(".sidebar-header").prepend(a):b("#s-content").before(a),b.orders.initDropdown(),"split"===this.options.view||this.isMobile()?
