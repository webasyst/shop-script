...
(m=u);var q=d.disableTimeRanges,v=0,C=q.length,x=d.step;"function"!=typeof x&&(x=function(){return d.step});for(var E=0;m>=h;E++,h+=60*x(E)){var B=h,D=t(B,d);if(d.useSelect)var y=b("<option />",{value:D});else y=b("<li />"),y.addClass(43200>B%86400?"ui-timepicker-am":"ui-timepicker-pm"),y.data("time",86400>=B?B:B%86400);y.text(D);if((null!==d.minTime||null!==d.durationTime)&&d.showDuration)if(D=A(h-l,d.step),d.useSelect)y.text(y.text()+" ("+D+")");else{var F=b("<span />",{"class":"ui-timepicker-duration"});
F.text(" ("+D+")");y.append(F)}C>v&&(B>=q[v][1]&&(v+=1),q[v]&&B>=q[v][0]&&B<q[v][1]&&(d.useSelect?y.prop("disabled",!0):y.addClass("ui-timepicker-disabled")));c.append(y)}(k.data("timepicker-input",a),a.data("timepicker-list",k),d.useSelect)?(a.val()&&c.val(e(n(a.val()),d)),c.on("focus",function(){b(this).data("timepicker-input").trigger("showTimepicker")}),c.on("blur",function(){b(this).data("timepicker-input").trigger("hideTimepicker")}),c.on("change",function(){p(a,b(this).val(),"select")}),p(a,
...
$("#shipping-info").empty().hide());$.shop.trace("check shipping_methods id",[c,e]);$.order_edit.updateTotal()});$("#shipping-rate").keyup(function(){$("#shipping-rate").data("shipping",$(this).val());$.order_edit.updateTotal()})},initDiscountControl:function(){function b(){0<a.val()?e.stop().show():e.stop().hide();var b=25;e.is(":visible")&&e.fadeOut(b+=50,function(){e.fadeIn(b+=50,function(){e.fadeOut(b+=50,function(){e.fadeIn(b+=50,function(){e.fadeOut(b+=50,function(){e.fadeIn(b+=50,function(){e.fadeOut(b+=
50,function(){e.fadeIn(b+=50,function(){})})})})})})})})}var a=$("#discount"),c=$("#discount-description"),d=$("#update-discount"),g=$("#edit-discount"),e=$("#discount-tooltip-icon");$(document).tooltip;e.tooltip({showURL:!1,bodyHandler:function(){return c.val()||c.data("updated-manually-msg")}});d.tooltip({showURL:!1,bodyHandler:function(){return d.data("description")||c.data("updated-manually-msg")}});g.length?e.show():e.hide();$("#order-edit-form").on("order_total_updated",function(e,h){a.show();
a.parent().find("span.js-order-discount:first").hide();g.hide();d.data("description",h.discount_description);if(h.discount){d.data("value",h.discount).data("description",h.discount_description).data("items_discount",h.items_discount);if("calculate"==d.data("discount")){c.val(d.data("description"));for(var k=d.data("items_discount")||[],l=0;l<k.length;l++)if(k[l]){var m="#order-items span.js-item-total-discount",m=m+('[data-discount-id="'+k[l].selector+'"]'),m=$(m);m.length&&m.html(k[l].html).show()}b()}"calculate"===
d.data("discount")?$.order_edit.parseFloat(h.discount)+""!=$.order_edit.parseFloat(a.val())&&d.click():d.show()}});var k=function(){a.show();a.parent().find("span.js-order-discount:first").hide();g.hide();$(".js-item-total-discount").hide()};g.click(function(){k();c.val("");return!1});d.click(function(){k();a.val(d.data("value")||0).change();c.val(d.data("description"));a.attr("title",c.data("edit-manually-msg"));d.hide().data("discount","calculate");$.order_edit.updateTotal();b();return!1});a.on("keyup",
function(){g.click();c.val("");$("#order-items .js-item-total-discount").hide();a.attr("title",null);d.show().data("discount",$(this).val());$.order_edit.updateTotal();b()})},initCustomerSourceControl:function(b){var a=$("#customer-source");a.autocomplete({delay:0,minLength:0,appendTo:"#order-edit-form",source:function(a,d){var g=b.filter(function(b){return b&&b.indexOf&&0<=b.indexOf(a.term)}).slice(0,10);1==g.length&&g[0]==a.term?d([]):d(g)}}).on("focus",function(){a.autocomplete("search")})},getOrderItems:function(b){var a=
[],c=[];b.find(".s-order-item").each(function(){var b=$(this),g=b.find('input[name^="product"]').val(),e=[],k=$.order_edit.parseFloat(b.find(".s-orders-product-price input").val()),f=$.order_edit.parseFloat(b.find("input.s-orders-quantity").val()),h=b.find("select.s-orders-sku-stock-select").val(),r=b.find("input[name^=sku]:not(:radio)").add(b.find("input[name^=sku]:checked")).first().val()||0,l={product_id:g,sku_id:r,services:[]};b.find(".s-orders-services").length&&b.find(".s-orders-services input:checkbox:checked").each(function(){var a=
$(this).closest("li"),b=$.order_edit.parseFloat(a.find("input.s-orders-service-price").val()),c=$(this).val(),a=a.find(':input[name^="variant["]').val();e.push({id:c,price:b,variant_id:a});l.services.push(c)});l.services.sort();l.services=l.services.join("_");a.push({product_id:g,quantity:f,price:k,sku_id:r,services:e,stock_id:h});c.push(l)});c=this.reduceOrderContent(c);b.data("order-content",c);b.data("order-content")!==c&&$("#discount").trigger("keyup");return a},reduceOrderContent:function(b){b.sort(function(a,
...
for(var d={},g=$("#s-order-edit-customer").find('[name^="customer["]').serializeArray(),e=0;e<g.length;e++)d[g[e].name]=g[e].value;d.items=$.order_edit.getOrderItems(b);var k=$("#update-discount").data("discount");void 0!==k&&("calculate"!==k&&(k=$.order_edit.parseFloat(k)),d.discount=k);$.order_edit.id?d.order_id=$.order_edit.id:d.currency=$("#order-edit-form input[name=currency]").val();(b=$("#shipping_methods").val())||(b=$("#shipping_methods option").eq(1).val());$("#shipping-rate").data("shipping")&&
(d.shipping=$("#shipping-rate").data("shipping"));d.params={shipping_id:b};d["customer[id]"]=d.contact_id=$("#s-customer-id").val();b&&(b=parseInt(b.split(".")[0]),0<b&&$("#shipping-custom").find(">#shipping-custom-"+b+" :input").each(function(){d[this.name]=this.value}));$.ajax({type:"POST",url:"?module=order&action=total",data:d,success:function(b){if("ok"===b.status){if(0<b.data.shipping_method_ids.length){var d=$("#shipping_methods"),e=d.val(),g=e.replace(/\W.+$/,""),m=$("#shipping-rate");d.empty();
$("#shipping-custom").empty();var p=b.data.shipping_method_ids,v=b.data.shipping_methods,C=-1!==p.indexOf(e),x=$("#shipping-custom");x.find(">div.fields.form").hide();for(var z=0;z<p.length;z+=1){var A=p[z],t=v[A];m.data("shipping")||A!=e||m.val($.order_edit.roundFloat(t.rate));var n=$("<option></option>");n.html(t.name).attr("value",A).data("rate",t.rate);n.data("error",t.error||void 0);n.data("est_delivery",t.est_delivery||void 0);n.data("comment",t.comment||void 0);n.data("external",t.external||
!1);if(t.custom_data)for(var u in t.custom_data)t.custom_data.hasOwnProperty(u)&&n.data(u,t.custom_data[u]);d.append(n);e||0!==z||$("#shipping-rate").val($.order_edit.formatFloat(t.rate));if(t.custom_html){var n=A.replace(/\W.+$/,""),q=x.find("#shipping-custom-"+n);q.length||(x.append('<div id="shipping-custom-'+n+'" style="display: none;" class="fields form"></div>'),q=x.find("#shipping-custom-"+n));q.html(t.custom_html);g==n?q.show():q.hide()}C||(t=(""+A).split("."),n=e.split("."),void 0!==t[0]&&
void 0!==n[0]&&t[0]==n[0]&&(C=!0,e=A))}C&&(d.val(e),d.data("_shipping_id",g),$.shop.trace("set shipping_methods id",[g,e]))}""!=k&&$("#discount").val($.order_edit.roundFloat(b.data.discount));$("#order-edit-form").trigger("order_total_updated",b.data);a.text($.order_edit.roundFloat(b.data.subtotal));c.text($.order_edit.roundFloat(b.data.total));$.order_edit.showValidateErrors(b.data.errors)}},dataType:"json"})}else a.text(0),c.text(0)},saveSubmit:function(b,a){var c=this.form,d=c.serializeArray();
"calculate"===$("#update-discount").data("discount")?$(d).each(function(){"discount"==this.name&&(this.value="calculate")}):void 0===$("#update-discount").data("discount")&&(d=d.filter(function(a){return"discount"!==a.name}),0<$("#discount").val()?d.discount_description=$("#edit-discount").val():d=d.filter(function(a){return"discount_description"!==a.name}));success="add"==a?function(a){$.order_edit.slide(!1,!0)&&(location.href="#/orders/state_id=new&view=split&id="+a.data.order.id+"/")}:function(a){$.order_edit.container.find(".back").click()};
$.shop.trace(a,c.serialize());return $.shop.jsonPost(c.attr("action"),d,success,function(b){$.shop.trace(a,b);b&&b.errors&&!$.isEmptyObject(b.errors)&&$.order_edit.showValidateErrors(b.errors)},b)},roundFloat:function(b){return b?this.formatFloat(Math.round(100*b)/100):0},formatFloat:function(b){return","===this.float_delimeter?(""+b).replace(".",","):""+b},parseFloat:function(b){return b?parseFloat((""+b).replace(",",".")):0},slideBack:function(){this.slide(!1,"add"==this.options.mode)},slide:function(b,
