...
(m=u);var q=d.disableTimeRanges,v=0,B=q.length,x=d.step;"function"!=typeof x&&(x=function(){return d.step});for(var E=0;m>=h;E++,h+=60*x(E)){var A=h,D=t(A,d);if(d.useSelect)var y=b("<option />",{value:D});else y=b("<li />"),y.addClass(43200>A%86400?"ui-timepicker-am":"ui-timepicker-pm"),y.data("time",86400>=A?A:A%86400);y.text(D);if((null!==d.minTime||null!==d.durationTime)&&d.showDuration)if(D=C(h-l,d.step),d.useSelect)y.text(y.text()+" ("+D+")");else{var F=b("<span />",{"class":"ui-timepicker-duration"});
F.text(" ("+D+")");y.append(F)}B>v&&(A>=q[v][1]&&(v+=1),q[v]&&A>=q[v][0]&&A<q[v][1]&&(d.useSelect?y.prop("disabled",!0):y.addClass("ui-timepicker-disabled")));c.append(y)}(k.data("timepicker-input",a),a.data("timepicker-list",k),d.useSelect)?(a.val()&&c.val(e(n(a.val()),d)),c.on("focus",function(){b(this).data("timepicker-input").trigger("showTimepicker")}),c.on("blur",function(){b(this).data("timepicker-input").trigger("hideTimepicker")}),c.on("change",function(){p(a,b(this).val(),"select")}),p(a,
...
b(this),f=[],h;for(h in b.fn.timepicker.defaults)e.data(h)&&(f[h]=e.data(h));f=b.extend({},b.fn.timepicker.defaults,f,a);if(f.lang&&(q=b.extend(q,f.lang)),f=c(f),e.data("timepicker-settings",f),e.addClass("ui-timepicker-input"),f.useSelect)d(e);else{if(e.prop("autocomplete","off"),f.showOn)for(var g in f.showOn)e.on(f.showOn[g]+".timepicker",w.show);e.on("change.timepicker",l);e.on("keydown.timepicker",B);e.on("keyup.timepicker",x);f.disableTextInput&&e.on("keydown.timepicker",v);l.call(e.get(0),
...
$("#shipping-custom").empty();var p=b.data.shipping_method_ids,v=b.data.shipping_methods,B=-1!==p.indexOf(e),x=$("#shipping-custom");x.find(">div.fields.form").hide();for(var z=0;z<p.length;z+=1){var C=p[z],t=v[C],n=$("<option></option>");n.html(t.name).attr("value",C).data("rate",t.rate);n.data("error",t.error||void 0);n.data("est_delivery",t.est_delivery||void 0);n.data("comment",t.comment||void 0);n.data("external",t.external||!1);if(t.custom_data)for(var u in t.custom_data)t.custom_data.hasOwnProperty(u)&&
n.data(u,t.custom_data[u]);d.append(n);e||0!==z||$("#shipping-rate").val($.order_edit.formatFloat(t.rate));if(t.custom_html){var n=C.replace(/\W.+$/,""),q=x.find("#shipping-custom-"+n);q.length||(x.append('<div id="shipping-custom-'+n+'" style="display: none;" class="fields form"></div>'),q=x.find("#shipping-custom-"+n));q.html(t.custom_html);g==n?q.show():q.hide()}B||(t=(""+C).split("."),n=e.split("."),void 0!==t[0]&&void 0!==n[0]&&t[0]==n[0]&&(B=!0,e=C))}B&&(d.val(e),d.data("_shipping_id",g),$.shop.trace("set shipping_methods id",
[g,e]))}""!=k&&$("#discount").val($.order_edit.roundFloat(b.data.discount));m.data("shipping")||m.val($.order_edit.roundFloat(b.data.shipping));$("#order-edit-form").trigger("order_total_updated",b.data);a.text($.order_edit.roundFloat(b.data.subtotal));c.text($.order_edit.roundFloat(b.data.total));$.order_edit.showValidateErrors(b.data.errors)}},dataType:"json"})}else a.text(0),c.text(0)},saveSubmit:function(b,a){var c=this.form,d=c.serializeArray();"calculate"===$("#update-discount").data("discount")?
$(d).each(function(){"discount"==this.name&&(this.value="calculate")}):void 0===$("#update-discount").data("discount")&&(d=d.filter(function(a){return"discount"!==a.name}),0<$("#discount").val()?d.discount_description=$("#edit-discount").val():d=d.filter(function(a){return"discount_description"!==a.name}));success="add"==a?function(a){$.order_edit.slide(!1,!0)&&(location.href="#/orders/state_id=new&view=split&id="+a.data.order.id+"/")}:function(a){$.order_edit.container.find(".back").click()};$.shop.trace(a,
c.serialize());return $.shop.jsonPost(c.attr("action"),d,success,function(b){$.shop.trace(a,b);b&&b.errors&&!$.isEmptyObject(b.errors)&&$.order_edit.showValidateErrors(b.errors)},b)},roundFloat:function(b){return b?this.formatFloat(Math.round(100*b)/100):0},formatFloat:function(b){return","===this.float_delimeter?(""+b).replace(".",","):""+b},parseFloat:function(b){return b?parseFloat((""+b).replace(",",".")):0},slideBack:function(){this.slide(!1,"add"==this.options.mode)},slide:function(b,a,c){if(0==
