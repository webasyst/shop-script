...
$jscomp.polyfill("Array.prototype.values",function(b){return b?b:function(){return $jscomp.iteratorFromArray(this,function(b,c){return c})}},"es8","es3");
...
p.hide();b(l.editLink).hide();if(!n){a=[];for(var d=[],e=0,k=l.makeReadableBy.length;e<k;++e){var u=l.makeReadableBy[e];"blur"==u&&c.blur(function(){"read"!=r&&f.call(this)});"enter"==u&&a.push(13);"esc"==u&&a.push(27)}e=0;for(k=l.updateBy.length;e<k;++e)u=l.updateBy[e],"alt+enter"==u&&d.push({ctrl:!1,alt:!0,shift:!1,key:13}),"ctrl+enter"==u&&d.push({ctrl:!0,alt:!1,shift:!1,key:13}),"ctrl+s"==u&&d.push({ctrl:!0,alt:!1,shift:!1,key:17});a.length&&function(b){c.keydown(function(a){!~b.indexOf(a.keyCode)||
...
this.value?b(this).attr("disabled",!0).addClass("js-ajax-disabled"):b(this).hasClass("js-ajax-disabled")&&b(this).removeAttr("disabled").removeClass("js-ajax-disabled")});this.refresh("submit");this.clearValidateErrors();b.ajax({url:b(f).attr("action"),data:b(f).serializeArray(),dataType:"json",type:"post",success:function(f){if("fail"==f.status)e.showValidateErrors(f.errors)||e.refresh("error",f.errors);else if(f.data.redirect)b.shop.trace("$.product.saveData redirect",f.data.redirect),window.location.href=
f.data.redirect;else{e.refresh("success",f.data.message||"");b.shop.trace("$.product.saveData updateData",[a,c]);e.updateData(f.data,a,c);d&&"function"==typeof d&&d();if(e.path.tab&&(f="editTab"+e.path.tab.substr(0,1).toUpperCase()+e.path.tab.substr(1)+"Saved",!1===e.call(f)))return!1;b.each(e.standalone_tabs,function(b,a){e.path.tab!=a&&(b="editTab"+a.substr(0,1).toUpperCase()+a.substr(1)+"Saved",e.call(b))})}e.ajax.save=!1},error:function(b,a,c){e.ajax.save=!1;e.refresh("error",a)}});return!1},
showValidateErrors:function(a){a=a||{};if(b.isEmptyObject(a))return!1;var c=this,d=!1,e=c.$edit_forms;if(a.features&&a.features.gtin){var f=e.find(".s-product-form.features").find('input[name="product[features][gtin]"]');if(f.length){f.addClass("error js-validate-error-input");var g=b('<div class="errormsg js-validate-error-msg"></div>').text(a.features.gtin);f.after(g);d=!0;c.refresh("error",$_("Save GTIN feature for product is fail"))}}if(a.skus){var h=e.find(".s-product-form.main table.s-product-skus tbody");
b.each(a.skus,function(a,e){e.features&&e.features.gtin&&(a=h.find('.js-sku-settings[data-id="'+a+'"]').find('input[name="skus['+(a+'][features][gtin]"]')),a.length&&(a.addClass("error js-validate-error-input"),e=b('<div class="errormsg js-validate-error-msg"></div>').text(e.features.gtin),a.after(e),d=!0,c.refresh("error",$_("Save GTIN feature for SKU is fail"))))})}return d},clearValidateErrors:function(){var b=this.$edit_forms,c=b.find(".s-product-form.main table.s-product-skus tbody");b=b.find(".s-product-form.features");
c.find(".js-validate-error-input").removeClass("error");c.find(".js-validate-error-msg").remove();b.find(".js-validate-error-input").removeClass("error");b.find(".js-validate-error-msg").remove()},updateData:function(a,c,d){b.shop.trace("$.product.updateData("+c+","+d+")",a);var e=b("#s-product-edit-forms .s-product-form.main"),f=this.path.id;if(this.path.id&&"new"!=this.path.id)this.path.id!=a.id&&b.shop.error("Abnormal product ID change",[this.path.id,a]);else{this.path.id=a.id;b(this.options.container_selector).data("product-id",
this.path.id);var g="/product/"+this.path.id+"/";null!==c&&(c=c||this.path.mode||"profile")&&("profile"!=c&&(g+=c+"/"),null!==d&&(d=d||this.path.tab||"")&&"main"!=d.toLowerCase()&&(g+=d+"/"));b.shop.trace("update location.hash="+g);var h=/#\/product\/new\//,k="#/product/"+this.path.id+"/";b("#shop-productprofile, #s-product-edit-menu, #s-product-edit-forms, #s-product-profile-page").find('[href*="#/product/new/"]').each(function(){b(this).attr("href",b(this).attr("href").replace(h,k))});b("#s-product-edit-forms").find(':input[name="product\\[id\\]"]').val(this.path.id);
window.location.hash=g}c=b("#shop-productprofile h1");c.find(".s-product-name:first").text(a.name);c.find(".s-product-id").show().text("id: "+a.id);document.title=a.name+b.product.options.title_suffix;b("#product-stock-stat").html(tmpl("template-product-stock-stat",a.raw));e.find(':input[name="product\\[type_id\\]"]').parents(".value").data("type",a.type_id);if(b.isEmptyObject(a.frontend_urls))b(".s-product-frontend-url-not-empty").hide(),b(".s-product-frontend-url-empty").show();else{c=b("#s-product-frontend-url");
c.text(a.url);b("#s-product-frontend-url-input").val(a.url);c.trigger("readable");d=c.parent();d.attr("href",a.frontend_urls[0].url);d.find("span:first").text(a.frontend_urls[0].base);c.closest("div.value").find(".s-product-frontend-url").each(function(c){a.frontend_urls[c+1]&&b(this).attr("href",a.frontend_urls[c+1].url).text(a.frontend_urls[c+1].url)});b(".s-product-frontend-url-not-empty").show();b(".s-product-frontend-url-empty").hide();c="";d=0;for(g=a.frontend_urls.length;d<g;d+=1)c+=' <span class="s-product-frontend-url-not-empty"><a href="'+
a.frontend_urls[d].url+'" target="_blank">'+a.frontend_urls[d].url+'</a><i class="icon10 new-window"></i></span> ';c&&b("#s-product-frontend-links").find(".s-product-frontend-url-not-empty").wrapAll("<div></div>").closest("div").replaceWith(c)}this.editTabMainUpdate(a.raw||{},a.features_selectable_strings,f);e.find("select.s-product-categories").each(function(){var c=b(this),d=c.parent(),e=c.val();if("select"==e||"---"==e){var f=b("#s-product-edit-forms .s-product-form.main select.s-product-categories");
1<f.length?d.remove():d.find(".s-product-delete-from-category").hide()}else if("create"==e){var g=d.find("input.val").val();"0"==g?(f=b("#s-product-edit-forms .s-product-form.main select.s-product-categories"),1<f.length?d.remove():(c.val(g),c.show().attr("disabled",!1),d.find("input.val").attr("disabled",!0),d.find(".s-new-category").hide(),d.find(".s-product-delete-from-category").hide())):(c.val(g),c.show().attr("disabled",!1),d.find("input.val").attr("disabled",!0),d.find(".s-new-category").hide(),
d.find(".s-product-delete-from-category").show())}e=parseInt(e,10);if(!b.isEmptyObject(a.storefront_map[e])){c="";e=a.storefront_map[e];for(f=0;f<e.length;f+=1)c+='<a class="hint" href="'+e[f]+'" target="_blank">'+e[f]+"</a> ";d.find(".s-storefront-map").html(c)}});b("#s-product-categories").html(tmpl("template-product-categories",{categories:a.categories||[]}));b("#s-product-tags").html(tmpl("template-product-tags",{tags:a.tags||[]}));b("#s-product-save table.s-product-skus > tbody > tr").each(function(){var c=
b(this).data("id");b(this).find("td.s-stock .s-product-stock-icon").each(function(){var d=b(this).data("stock");a.raw&&a.raw.skus&&a.raw.skus[c]&&a.raw.skus[c].stock_icon[d]&&b(this).html(a.raw.skus[c].stock_icon[d])})});e.find("tr.js-sku-settings").each(function(){var c=b(this),d=c.attr("data-id");(d=a.raw.skus[d])&&void 0!==d.file_name&&!d.file_name&&(c.find(".s-sku-attachment").hide(),c.find(".fileupload").show())});this.helper.checkChanges(this.get("form"),!0);b("#s-product-edit-menu li a i.icon10.status-yellow-tiny").remove();
this.helper.updateSubmitButton(!1);"0"==a.raw.sku_type&&(e=b("#s-product-feature-superposition"),e.find("input[type=checkbox]").attr("checked",!1),f=e.find("ul.features li"),f.find(".count").text(""),f.find("i").removeClass("status-blue-tiny status-gray-tiny").addClass("status-gray-tiny"),e=e.find(".superposition-count"),e.find(".options").text(""),e.find(".skus").text(""));b("#s-product-meta-title").attr("placeholder",a.default_meta_title);b("#s-product-meta-keywords").attr("placeholder",a.default_meta_keywords);
b("#s-product-meta-description").attr("placeholder",a.default_meta_description);b('#s-product-profile-tabs .s-tab-block[data-tab="stock-logs"]').trigger("refresh");this.setData("main","category_name",a.category_name)},refresh:function(a,c){b(this.options.form_selector).find(':input[name^="product\\["]:disabled.js-ajax-disabled').each(function(){b(this).removeAttr("disabled").removeClass("js-ajax-disabled")});var d=b(this.options.message_selector);d.removeClass("errormsg successmsg status").empty().show();
this.timer&&clearTimeout(this.timer);var e=null;d.append(this.message[a]||"");switch(a){case "submit":d.addClass("status");break;case "error":d.addClass("errormsg");for(a=0;a<c.length;a++)d.append(c[a][0]);e=2E4;break;case "success":c&&d.addClass("successmsg").append(c),e=3E3,this.helper.updateSubmitButton(!1)}e&&(this.timer=setTimeout(function(){d.removeClass("errormsg successmsg status").empty().show()},e))},editTabLoad:function(a,c){var d=this,e=b("#s-product-edit-forms .s-product-form."+a.tab);
!0===c||!e.length&&"new"==a.id?(b.shop.trace("product.editTabLoad: create",[a.tab+" \u2014 create",a,c]),this.saveData(this.path.mode,a.tab,function(){a.tab&&d.call(a)})):this.editTabLoadContent(a)},editTabLoadContent:function(a,c){var d=this;a=a||this.path;var e="?module=product&action="+a.tab+"&id="+a.id;a.tail&&(e+="&param[]="+a.tail.split("/").join("&param[]="));var f=Math.random();this.ajax.random=f;var g=b("#s-product-edit-forms .s-product-form."+a.tab);g.length&&g.remove();b("#s-product-edit-forms > form").append(tmpl("template-productprofile-tab",
{id:a.tab}));g=b("#s-product-edit-forms .s-product-form."+a.tab);this.ajax.target=g;this.ajax.link=b("#s-product-edit-menu li."+a.tab);b.shop.trace("$.product.editTabLoadContent",[a,e,a.params]);if(a.params&&c){var h=typeof a.params;switch(h){case "String":e+=a.params;break;case "Array":e+=a.params.serialize();break;default:b.shop.error("unexpected type "+h,a.params)}}b.ajax({url:e,type:c?"POST":"GET",data:c?c||{}:a.params||{},success:function(c,e){b.shop.trace("$.product.loadTab status="+e);if(d.ajax.random==
f){g.empty().append(c);d.ajax.target=null;d.ajax.link=null;c="#/product/"+a.id+"/edit/";a.tab&&(c+=a.tab+"/");a.tail&&(c+=a.tail+"/");if(a.params&&!b.isEmptyObject(a.params)){e=[];for(var h in a.params)a.params.hasOwnProperty(h)&&e.push(h+"="+a.params[h]);c+=e.join("&")}window.location.hash=c;d.dispatch(a)}}})},editTabInit:function(a){b("html, body").animate({scrollTop:0},200);b("#shop-productprofile").find(".s-product-name").removeClass("editable");this.$edit_forms=b("#s-product-edit-forms")},editTabFocus:function(a){b("#s-product-edit-menu li."+
a.tab).addClass("selected");this.refresh()},editTabBlur:function(a){if(a){b("#s-product-edit-forms .s-product-form").hide();b("#s-product-edit-menu li.selected").removeClass("selected");var c=a.tab;if(-1!==this.standalone_tabs.indexOf(c)){var d=this;this.helper.checkChanges(this.get("form"),!0,function(a){a&&(b.shop.trace("$.product.editTabBlur: save standalone tab ["+c+"]",d.standalone_tabs),d.saveData(d.path.mode,c,function(){c&&d.call(d.path)}))})}this.path.tab=null;this.path.tail=null}},editTabShow:function(a){var c=
b("#s-product-edit-forms .s-product-form."+a.tab);c.length?(c.show(),b.shop.trace("$.product.showTab",a),this.path.tab=a.tab):b.shop.error("$.product.showTab",a)},editClick:function(a){var c=a.attr("href").replace(/.*#\/product\/(\d+|new)\/edit\//,"").replace(/\/$/,"").split("/"),d=[];if(c.length){b.shop.trace("$.product.editClick",c);for(var e,f="editTab";e=c.shift();)if(f+=e.substr(0,1).toUpperCase()+e.substr(1),e="function"==typeof this[f],b.shop.trace("$.settings.featuresClick try",[f,e,c]),!0===
e){var g=f;d=c.slice(0)}}if(g){if(b.shop.trace("$.product.editClick",[g,d]),!a.hasClass("js-confirm")||confirm(a.attr("title")||"Are you sure?")){d.push(a);try{this[g].apply(this,d)}catch(h){b.shop.error("Error at method $.product."+g+". Original message: "+h.message,h)}}}else b.shop.error("Not found js handler for link",[g,a]);return!1},switchSubMenu:function(a){a=a||"default";b("#mainmenu .s-submenu > *").hide();b("#mainmenu .s-submenu .s-submenu-"+a).show()},profileFocus:function(){var a=this.options.duration,
c=this;b("#s-product-edit-menu, #s-product-edit-save-panel").hide();c.switchSubMenu();b("#s-sidebar").show().animate({width:b.product.options.sidebar_width});b("#s-toolbar").show().animate({width:"200px"},a).queue(function(){b(this).dequeue();b("#s-product-edit-forms").hide();b("#s-product-profile-page, h1 .s-product-edit-link, #mainmenu .s-level2, #s-product-frontend-links").show();c.get("container").find(".back").attr("href","#/"+b.products.list_hash)});b("#maincontent").animate({"margin-top":"84px"},
a);b("#s-content").animate({"margin-left":b.product.options.sidebar_width},a);b("#shop-productprofile").off("click.edit-product","a.js-action").on("click.edit-product","a.js-action",function(){return c.editClick(b(this))})},profileAction:function(){this.path.mode="profile"},disableSkus:function(a,c){var d=b(this.options.form_selector).find(".s-sku-list");a?c?d.find(".s-sku-virtual").hide().find(":input").attr("disabled",!0):d.find(".s-sku-virtual").show().find(":input").attr("disabled",!1):(c?d.hide():
d.show(),d.find(":input").attr("disabled",c))},skuTypeFeaturesSelectableLoad:function(a,c){var d=this,e="?module=product&action=featuresSelectable&id="+this.path.id,f={type_id:a,sku_type:1};b("#s-product-feature-superposition-field-group").load(e,f,function(){var b=d.featureSelectableInit();c&&(c.base_price_selectable&&b.find(':input[name="product[base_price_selectable]"]').val(c.base_price_selectable),c.currency&&b.find("select.s-product-currency").val(c.currency));d.onSkuTypeEnabled(a)})},onSkuTypeEnabled:function(a){var c=
b("#s-product-feature-superposition-field-group");a&&c.data("type",a);c.show(400,function(){b.product.checkFeatureValidation()});c.find("input").attr("disabled",!1);"new"==b.product.path.id?b.product.disableSkus(!1,!0):b.product.disableSkus(!0,!1);a=["compare_price","purchase_price"];for(var d,e,f=0;f<a.length;f++)d=c.find(':input[name="product['+a[f]+'_selectable]"]:first'),d.length&&!parseInt(d.val())&&(e=b(':input[name^="skus"][name$="['+a[f]+']"]:first'),e.length&&d.val(e.val()))},checkFeatureValidation:function(){var a=
...
defaultChangedStatus:!1},data:{url_helper:{url:"",name:"",timer:null}},init:function(){this.data.url_helper={url:"",name:"",timer:null}},onTabChanged:function(a,c){b.shop.trace("$.product.onTabChanged id="+a+" changed="+!!c,c);b("#s-product-edit-menu li."+a+" .s-product-edit-tab-status").html(c?this.options.tab_changed:"");c&&this.updateSubmitButton(!0)},type:function(a){return parseInt(a,10)||parseInt(b('#s-product-edit-forms .s-product-form.main :input[name="product\\[type_id\\]"]').val(),10)||
0},onNameChange:function(a,c,d){this.data.url_helper.timer&&clearTimeout(this.data.url_helper.timer);var e=b(b.product.options.form_selector).find(':input[name="product\\[url\\]"]'),f=e.parent();if(b.product.path.id&&"new"!=b.product.path.id||e.val()!=this.data.url_helper.url)b.shop.trace("$.product.onNameChange stop "+this.data.url_helper.url+" != "+e.val()),b(b.product.options.form_selector).off(".product",':input[name="product\\[name\\]"]'),f.find(".js-url-helper").hide();else{c&&(f.find(".js-url-helper").length?
f.find(".js-url-helper").show():f.append('<i class="icon16 loading js-url-helper"></i>'));var g=this;this.data.url_helper.timer=setTimeout(function(){g.urlHelper(a,e,d)},d||500)}},onChange:function(b){var a=this.getContainerId(b),d=this;this.checkChanges(b.parents("form"),!1,function(c){d.updateSubmitButton(c);if(c)d.checkChanges(b,!1,function(b){d.onTabChanged(a,b)});else d.onTabChanged(a,c)})},urlHelper:function(a,c){this.data.url_helper.timer&&clearTimeout(this.data.url_helper.timer);a={name:b(a).val()};
b.shop.trace("$.product.urlHelper ",a);if(a.url!=this.data.url_helper.name){var d=this;this.data.url_helper.url=a.url;b.getJSON("?module=product&action=suggestUrl",a,function(a){"ok"===a.status&&(d.data.url_helper.url=a.data.url,c.val(a.data.url),c.parent().find(".js-url-helper").hide(),b.product.informAboutUrlInUse(a.data.in_use))})}else c.parent().find(".js-url-helper").hide()},getValueByName:function(a,c){for(var d=a,e=c.replace(/\]/,"").split("[");e.shift();)if(void 0!==d[c])d=d[c];else{d="";
break}b.shop.trace("$.product.helper.getValueByName",[d,c,a]);return d},checkChangesStack:{},checkChanges:function(a,c,d){var e=a.attr("id");this.checkChangesStack[e]&&(clearTimeout(this.checkChangesStack[e]),this.checkChangesStack[e]=null);var f=this;this.checkChangesStack[e]=setTimeout(function(){var g=f.checkChangesDelayed(a,c);b.shop.trace("$.product.helper.checkChanges["+e+"]",[a,c,g,d?"callback":"none"]);d&&d(g)},c?50:500)},checkChangesDelayed:function(a,c){var d=this.options.defaultChangedStatus,
e=this,f=":input:not(.js-ignore-change)";if(a.hasClass("s-product-form")){if(a.hasClass("ajax"))return!1}else a.hasClass("s-product-form-chunk")||(f=".s-product-form:not(.ajax) "+f);a.find(f).each(function(){var f=b(this),h=(f.attr("type")||this.tagName).toLowerCase();switch(h){case "input":case "text":case "textarea":if(f.hasClass("ace_text-input"))break;this.defaultValue!=this.value&&(d=!0,c&&(this.defaultValue=this.value,e.updateInput(this.name,this.value,"textarea"==h)));break;case "radio":case "checkbox":this.defaultChecked!=
this.checked&&(d=!0,c&&(this.defaultChecked=this.checked));break;case "select":this.length&&b(this).find("option").each(function(){this.defaultSelected!=this.selected&&(d=!0,c&&(this.defaultSelected=this.selected))});break;case "file":this.value&&(d=!0,c&&(this.value=null));break;case "reset":case "button":case "submit":break;case "hidden":f.hasClass("js-check-change")&&void 0!==f.attr("data-value")&&(defaultValue=f.attr("data-value"),this.disabled?c&&(h=b.shop.helper.escape(this.name),h=a.find(':input[name="'+
h+'"]:enabled'),h.length&&f.attr("data-value",h.val())):defaultValue!=this.value&&(d=!0,c&&f.attr("data-value",this.value)));break;default:b.shop.error("$.product.checkChangesDelayed unsupported type "+h,[h,this])}!c&&d&&(b.shop.trace("$.product.helper.checkChangesDelayed",[this,d,this.defaultValue,this.value]),d=f);return c||!d});return d},getContainerId:function(a){a=(b(a).attr("class")||"").split(" ");var c=!1,d;for(d in a)if("s-product-form"!=a[d]){c=a[d];break}return c},updateInput:function(a,
c,d,e){if(a){var f=".s-"+a.replace(/\[(.+?)\]/g,"-$1")+"-input";b.shop.trace("update field: "+a+" "+f,c);var g=b(f);g.is("input,textarea")?g.val(c):d?"product[summary]"===a?g.text(c):g.html(c):g.text(c);e&&(a=b(f).attr("name"),a=a.replaceAll(/\[-[\d]+\]/,"["+e+"]"),b.shop.trace("$.product.helper.updateInput name",[a]))}},updateSubmitButton:function(a){b("#s-product-edit-save-panel :submit").removeClass(a?"green":"yellow").addClass(a?"yellow":"green")},count:function(b){var a=0,d;for(d in b)b.hasOwnProperty(d)&&
a++;return a}},multiSkus:function(a,c){c="boolean"===typeof c?c:!0;var d=b("#s-product-edit-forms .s-product-form.main table.s-product-skus:first");1<a?(d.find("thead tr th.s-sku-sort").show(),d.find(".s-name,.s-sku-sort").show("slow"),d.find(".delete").parent("a").show(),c&&(2===a?d.find("> tbody:first > tr:first .s-name :input").focus():d.find("> tbody:first > tr:last .s-name :input").focus())):(d.find("thead tr th.s-sku-sort").hide(),d.find(".s-name,.s-sku-sort").hide(),d.find(".delete").parent("a").hide(),
d.find(".s-sku-big").removeClass("s-sku-big"))},editTabMainSkuAdd:function(){try{var a=b("#s-product-edit-forms .s-product-form.main table.s-product-skus:first").find("tbody:first"),c=0,d="0";a.find(':input[name$="[price]"]').each(function(){var a=b(this),e=a.val()?a.val().replace(",","."):0;e=parseFloat(e);e>c&&(c=e,d=a.val()||"0")});var e=0;a.find(':input[name$="[purchase_price]"]').each(function(){e=Math.max(e,parseFloat(b(this).val())||0)});b.product.multiSkus(a.find("tr:not(.js-sku-settings)").length+
1);var f={id:--b.product.editTabMainData.sku_id,product_id:b('input:hidden[name="product[id]"]').val()||"new",sku:"",available:1,name:"",price:""+c,price_loc:d,purchase_price:""+e,stock_icon:{0:"<i class='icon10 status-green' ></i>"},stock:{},count:null};b.shop.trace("$.product.editTabMainSkuAdd",[b.product.editTabMainData.sku_id,f]);a.append(tmpl("template-sku-edit",{sku_id:b.product.editTabMainData.sku_id,sku:f,stocks:this.getData("main","stocks"),stock_ids:this.getData("main","stock_ids")},!0));
a.find(".s-product-currency").trigger("change");a.find("td.s-sku").addClass("s-sku-big")}catch(g){b.shop.error(g.message,g)}return!1},editTabMainSkuSort:function(a,c,d){try{b.post("?module=product&action=skuSort",{product_id:this.path.id,sku_id:a,after_id:c},function(a){b.shop.trace("$.product.editTabMainSkuSort result",a);a.error&&(b.shop.error("Error occurred while sorting product SKUs","error"),d.sortable("cancel"))},function(a){b.shop.trace("$.product.editTabMainSkuSort cancel",{data:a});d.sortable("cancel");
...
b.shop.jsonPost("?module=products&action=deleteList",{product_id:[b.product.path.id],get_lists:1},function(c){b.product_list&&b("#s-sidebar").trigger("update",c.data.lists);a.trigger("close");window.location.hash="#/products/"});return!1}})});return!1},editTabMainSkuDelete:function(a,c){var d=b("#s-product-edit-forms .s-product-form.main table.s-product-skus:first").find("tbody:first"),e=c.parents("tbody").find('> tr[data-id="'+a+'"]'),f=this;if(0<a){var g=c.parents("tbody").find('tr[data-id]:not(.js-sku-settings):not([data-id^="-"])').length;
c=c.parents("tbody").find("tr[data-id]:not(.js-sku-settings)").length;1<g?b.ajax({url:"?module=product&action=skuDelete",data:{sku_id:a,product_id:this.path.id},dataType:"json",type:"post",success:function(c){"fail"==c.status?f.refresh("error",c.errors):c.data.redirect?window.location.href=c.data.redirect:(f.refresh("success",c.data.message||"Success"),e.hide("normal",function(){e.remove();b('#s-product-view table.s-product-skus > tbody > tr[data-id="'+a+'"]').remove();b.product.multiSkus(d.find("tr:not(.js-sku-settings)").length,
!1)}),b("#s-product-edit-forms .s-product-form.main").find("input[name=product\\[sku_id\\]][value="+c.data.sku_id+"]").attr("checked",!0))},error:function(b,a,c){f.refresh("error",a)}}):1<c?(e.after('<input type="hidden" name="skus['+a+']" value="%deleted%">'),e.hide("normal",function(){e.remove();b('#s-product-view table.s-product-skus > tbody >tr[data-id="'+a+'"]').remove();b.product.multiSkus(d.find("tr:not(.js-sku-settings)").length,!1)})):f.refresh("error",$_("A product must have at least one SKU."))}else e.hide("normal",
function(){e.remove();b('#s-product-view table.s-product-skus > tbody >tr[data-id="'+a+'"]').remove();b.product.multiSkus(d.find("tr:not(.js-sku-settings)").length,!1)})},editTabMainCategoriesAdd:function(){var a=b("#s-product-edit-forms .s-product-form.main select.s-product-categories:last").parent("div"),c=a.clone(!1);c.find("select").val("select").attr("disabled",!1).show();c.find("input.val").val(0).attr("disabled",!0).parent().hide();c.find(".s-product-delete-from-category").hide();c.find(".s-storefront-map").html("");
c.show();a.after(c)},editTabMainSkuStock:function(a,c){c=b("#s-product-sku-"+a);c.hide().find(":input:enabled").attr("disabled",!0);a=b("#s-product-sku-stock-"+a);a.show().find(":input:disabled").removeAttr("disabled");a.find(":input:first").val(c.find(":input:first").val())},editTabMainSkuSettings:function(a,c){c.hide();var d=c.parents("tr");c=d.data();var e=this;b.when(d.after(tmpl("template-sku-settings",{sku_id:a,sku:c}))).done(function(){var c="?module=product&action=skuSettings&product_id="+
e.path.id;c+="&sku_id="+a;c+="&type_id="+e.helper.type();var g=b('#s-product-edit-forms .s-product-form.main tr.js-sku-settings[data-id="'+a+'"] > td:first');g.load(c,function(){d.find(':input[name$="[available]"]').remove();b.product_images&&b.product_images.options&&b.product_images.options.enable_2x&&b.fn.retina&&g.find(".s-product-image-crops img").retina();if(0<a){b.shop.trace("fileupload",[g.find(".fileupload").length,typeof g.find(".fileupload").fileupload]);try{var c=document.cookie.match(/(?:^|; )_csrf=([^;]*)/),
e=c?decodeURIComponent(c[1]):"";g.find(".fileupload:first").fileupload({dropZone:null,url:"?module=product&action=skuEproductUpload",acceptFileTypes:/(\.|\/)(gif|jpe?g|png)$/i,start:function(){g.find(".fileupload:first").hide()},progress:function(a,c){b.shop.trace("fileupload progress",c);a=g.find(".js-progressbar-container");a.show();a.find(".progressbar-inner:first").css("width",Math.round(100*c.loaded/c.total,0)+"%")},done:function(a,c){b.shop.trace("fileupload done",[c.result,typeof c.result]);
a=(c.result.files||[]).shift();g.find(".js-progressbar-container").hide();!a||a.error?(g.find(".error-message").text(a.error).show(),g.find(".fileupload:first").addClass("error").show()):(c=g.find(".s-sku-attachment"),c.find("input.s-input-file-name").val(a.name),c.find(".s-file-name").text(a.name),c.find(".s-file-size").text(a.size),c.find(".s-file-description").val(""),c.find("input[type=checkbox]").attr("checked",!0),c.show(),d.find("input.s-input-virtual").val(0))},fail:function(a,c){b.shop.trace("fileupload fail",
[c.textStatus,c.errorThrown]);g.find(".error-message").text("error").show().text(c.errorThrown||"error");g.find(".js-progressbar-container").hide();g.find(".fileupload:first").addClass("error").show()},formData:{product_id:b.product.path.id,sku_id:a,_csrf:e}})}catch(m){b.shop.error("Exception "+m.message,m)}}})})},editTabMainSkuImageSelect:function(a,c,d){a=d.parents("li:first");var e=a.hasClass("selected"),f=d.parents("div.value:first");f.find("ul.s-product-image-crops li.selected").removeClass("selected");
...
a.fileupload("start")},editTabMainSkuEproductDelete:function(b){},editTabMainLinkEdit:function(b){return!1},editTabMainTypeSelect:function(b){var a=b.parents("form").find(':input[name="product[type_id]"]').parents(".value");"main"==this.path.tab?b.hide():(a.find(".js-action").hide(),window.location.hash="/product/"+this.path.id+"/"+this.path.mode+"/");a.find(".js-type-name").hide();a.find(":input").show().focus();setTimeout(function(){a.find(":input").focus()},100)},editTabMainTypeChange:function(a){var c=
a.parents(".value"),d=a.find(":selected:first"),e=d.val();a=parseInt(d.data("sku-type"));c.find(".js-type-icon").html(d.data("icon"));c.find(".js-type-name").text(d.text());e!=c.data("type")?b("#s-product-edit-forms .s-product-form.features").length&&"undefined"!=typeof this.editTabFeaturesReload?this.editTabFeaturesReload(e):(c=b("#s-product-edit-menu > li.features > a"),e="/features/"+e+"/",c.attr("href",c.attr("href").replace(/\/features\/.*$/,e))):(c=b("#s-product-edit-menu > li.features > a"),
...
f=b("#s-product-view table.s-product-skus tbody");e.find('tr[data-id^="-"]').remove();f.find('tr[data-id^="-"]').remove();e.parents("table").find("tr:hidden").show();var g=[];for(h in a.skus||{})g.push(b.extend({id:h},a.skus[h]));g=g.sort(function(b,a){return b.sort-a.sort});var h=this.getData("main","stocks");var k=this.getData("main","stock_ids"),m="";tmpl.cache["template-sku-edit"]=tmpl(tmpl.load("template-sku-edit"));for(var n=0,l=g.length;n<l;n+=1)m+=tmpl("template-sku-edit",{sku_id:g[n].id,
sku:g[n],hide_sort_column:1>=l,stocks:h,stock_ids:k,checked:g[n].id==a.sku_id});e.html(m);e.find("select.s-product-currency").val(a.currency);e.parent().show();e.closest(".s-sku-list").show();this.multiSkus(e.find("tr:not(.js-sku-settings)").length);m="";n=0;for(l=g.length;n<l;n+=1)m+=tmpl("template-sku",{sku_id:g[n].id,sku:g[n],stocks:h,stock_ids:k,runout:a.runout||{}});f.html(m);d&&"new"!=d||e.parents("div.field-group:first").after(b("#s-sku-type-field-group"));c&&(d=b("#s-product-feature-superposition .superposition-count"),
...
10)?a.addClass("editable"):a.removeClass("editable");var c=this.get("form");a.inlineEditable({minSize:{width:150},maxSize:{width:550},inputClass:"s-title-h1-edit",beforeMakeEditable:function(){b("#s-edit-product").hide()},afterBackReadable:function(a,e){b("#s-edit-product").show();if(!e.changed)return!1;b.shop.jsonPost("?module=product&action=save&id="+b.product.path.id,{update:{name:b(a).val()}},function(d){var e=b(a).val();document.title=e+b.product.options.title_suffix;"ok"===d.status&&d.data&&
...
f.options.view,{products:a.data.products,check_all:d.find(".s-select-all:first").length?d.find(".s-select-all:first").attr("checked"):!1,sort:b.product_list.sort,primary_currency:b.product_list.options.primary_currency,stocks:b.product_list.options.stocks})),f.container.trigger("append_product_list",[a.data.products])}catch(m){console.log("Error: "+m.message);b(window).lazyLoad("stop");return}d.find(".last").removeClass("last");d.find(".product:last").addClass("last");b(".lazyloading-progress-string").text(a.data.progress.loaded+
...
else{var a=h.data("stocks");a&&"object"==typeof a?g.html(tmpl("template-list-stock-editor-many",{product_id:h.data("product-id"),values:a})):g.html(tmpl("template-list-stock-editor-one",{product_id:h.data("product-id"),value:a||""}))}g.find(".cancel").click(function(){var a=g;a.removeClass("editor-on").html(a.data("read-only-state-html"));a.data("read-only-state-html","");a.data("denied-reason","")});var c=g.find("form").submit(function(){if(c.find(".loading").length)return!1;c.find(":submit").after('<i class="icon16 loading"></i>');
b.post("?module=products&action=inlineSave",c.serialize(),function(a){c.find(".loading").remove();try{a.data&&a.data[0]&&(a.data[0].alien=h.hasClass("s-alien"));var d=b("<table>").html(tmpl("template-product-list-table",{products:a.data,check_all:b.product_list.container.find(".s-select-all:first").prop("checked"),sort:b.product_list.sort}));h.replaceWith(d.find("tr"));e.container.trigger("append_product_list",[a.data])}catch(r){console.log("Error: "+r.message)}},"json");return!1});c.on("dblclick",
...
h=b("#s-category-list-block"),k=b("#s-set-list"),m=b("#s-set-list-block"),n=function(){function c(){if(!m){var c=b("#s-category-list-handler");w=b.categories_tree.isCollapsed(c);n=g.is(":hidden");var k=b('<div id="s-category-list-dummy" class="block"></div>').css({height:h.height()});h.replaceWith(k);f.show().append(h.hide());h.find(".heading").before('<span class="hint float-right" style="margin-top: 3px;">'+$_("Drag products here")+"</span>");w&&b.categories_tree.expand(c,null,function(){b.categories_tree.collapse(c)});
...
function(a){a.pageX>d+10&&(c&&clearTimeout(c),c=null,b||(g.hide(),h.css({height:""}),h.find(".heading").prev(".hint").show(),b=!0))})})();m=!0}}function k(){if(m){var a=b("#s-category-list-dummy");a.replaceWith(h);h.css({"overflow-x":"","overflow-y":""});var c=b("#s-category-list-handler");w&&b.categories_tree.collapse(c);n||g.show();h.find(".heading").prev(".hint").remove();b("#s-category-list-handler").show();b("i.add",h).show();a.remove();h.unbind("mousemove.fixed_blocks.category");e.unbind("mouseout.fixed_blocks.category");
f.children().length||f.hide();m=!1;b.categories_tree.init()}}var l=h.offset().top,m=!1,n,w;e.unbind("scroll.fixed_blocks.category").bind("scroll.fixed_blocks.category",function(){m||(l=h.offset().top);var d=l;d=document.getElementById("s-category-list-dummy")?d+b("#s-category-list-dummy").height():d+b("#s-category-list-block").height();e.scrollTop()+a+5>=d?c():k()});return{setFixed:c,unsetFixed:k}}(),l=function(){function c(){if(!n){l=k.is(":hidden");var c=b('<div id="s-set-list-dummy" class="block"></div>').css({height:m.height()});
m.replaceWith(c);f.show().append(m);m.find(".heading").before('<span class="hint float-right" style="margin-top: 3px;">'+$_("Drag products here")+"</span>");l||k.hide();b("i.add",m).hide();b("i.collapse-handler",m).hide();m.css({"overflow-x":"hidden","overflow-y":"auto"});(function(){var b=k.is(":hidden"),c;m.bind("mousemove.fixed_blocks.set",function(d){document.getElementById("products-helper")&&b&&(c&&(clearTimeout(c),c=null),c=setTimeout(function(){if(b){m.css({height:""});k.show();var c=m.height(),
d=(e.height()-a)/2;m.css({height:Math.min(c,d)});m.find(".heading").prev(".hint").hide();b=!1}},250))});e.bind("mouseout.fixed_blocks.set",function(a){a.pageX>d+10&&(c&&clearTimeout(c),c=null,b||(k.hide(),m.css({height:""}),m.find(".heading").prev(".hint").show(),b=!0))})})();n=!0}}function g(){if(n){var a=b("#s-set-list-dummy");a.replaceWith(m);m.css({"overflow-x":"","overflow-y":""});m.find(".heading").prev(".hint").remove();l||k.show();b("i.add",m).show();b("i.collapse-handler",m).show();a.remove();
m.unbind("mousemove.fixed_blocks.category");e.unbind("mouseout.fixed_blocks.category");f.children().length||f.hide();n=!1}}var h=m.offset().top,l,n=!1;e.unbind("scroll.fixed_blocks.set").bind("scroll.fixed_blocks.set",function(){n||(h=m.offset().top);var d=h;d=document.getElementById("s-set-list-dummy")?d+b("#s-set-list-dummy").height():d+b("#s-set-list-block").height();e.scrollTop()+a+5>=d?c():g()});return{setFixed:c,unsetFixed:g}}();return{category:n,set:l}},initEditingControls:function(){var a=
...
d.data.lists);c&&c()})},g=b("#"+a.join("-")+"-handler");g.length?b.categories_tree.expand(g,function(a){a.parent().find("ul:first").hide();f()}):f();return!1}function e(a,c){var d=a[0],e=b("#"+a.join("-")),f=e.prev(".drag-newposition");a=b("#s-"+a[0]+"-list");var g=a.find("ul:first");"set"==d?(e.remove(),f.remove()):(d=e.find("ul:first"),d.length&&(d.find(">li.drag-newposition:first").remove(),e.hide(),d.find(">li").each(function(){e.after(this)})),d=e.parents("ul:first"),e.remove(),f.remove(),d.find("li.dr").length||
...
"-handler").remove()}var k=b(this),m=parseInt(b.product_list.collection_hash[1]);k.find(".dialog-buttons i.loading").show();var n="1"==k.find("input[name=s-delete-products]:checked").val();"category"==d&&k.find('input[name="s-delete-sub"]').is(":checked")?b.get("?action=categoryExpand&tree=1&recurse=1&id="+m,function(c){c=b("<div>").html(c).find('li[id^="category-"]').map(function(){return parseInt(this.id.substr(9),10)}).get().reverse();c.push(m);a(c,function(){location.href="#/"})}):c(b.product_list.collection_hash,
...
b){for(var c=[],d,g=!1;d=a.serialized.pop();)d.name&&("product_id[]"==d.name?c.push(parseInt(d.value,10)):"hash"==d.name&&(g=d.value));a="?action=importexport#/"+b;g?window.location=a+("/hash/"+g):c.length&&(a+="/hash/id/"+c.join(","),window.location=a)},createCoupon:function(a){for(var b=[],c,f=!1;c=a.serialized.pop();)c.name&&("product_id[]"==c.name?b.push(parseInt(c.value,10)):"hash"==c.name&&(f=c.value));a=this.options.marketing_url+"coupons/create/";f?window.location=a+("?products_hash="+f):
...
if("ok"===b.status){var g=d.find(".js-condition-list .js-feature-insertion-block"),h=d.find(".js-show-more").attr("data-count"),k=0,p;for(p in b.data.features)b.data.features.hasOwnProperty(p)&&(c.initShowFeatures(b.data.features[p],g,"template_feature_field",e,!1),++k);b=h-k;0<b?(g="<b><i>"+(20<b?$_("Show %d more").replace("%d",20)+" "+$_("From").toLowerCase()+" "+b:$_("Show %d more").replace("%d",b))+"</i></b>",d.find(".js-show-more").html(g).attr("data-count",b),a||(f=parseInt(f)+parseInt(k),d.find(".js-show-more").attr("data-offset",
