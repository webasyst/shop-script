...
if(f.length){f.addClass("error js-validate-error-input");var g=b('<div class="errormsg js-validate-error-msg"></div>').text(a.features.gtin);f.after(g);d=!0;c.refresh("error",$_("Cannot save the value of the \u201cGTIN\u201d feature for this product."))}}if(a.skus){var h=e.find(".s-product-form.main table.s-product-skus tbody");b.each(a.skus,function(a,e){"status"===a?(d=!0,c.refresh("error",$_(e))):e.features&&e.features.gtin&&(a=h.find('.js-sku-settings[data-id="'+a+'"]').find('input[name="'+("skus["+
a+"][features][gtin]")+'"]'),a.length&&(a.addClass("error js-validate-error-input"),e=b('<div class="errormsg js-validate-error-msg"></div>').text(e.features.gtin),a.after(e),d=!0,c.refresh("error",$_("Cannot save the value of the \u201cGTIN\u201d feature for an SKU of this product."))))})}return d},clearValidateErrors:function(){var b=this.$edit_forms,c=b.find(".s-product-form.main table.s-product-skus tbody");b=b.find(".s-product-form.features");c.find(".js-validate-error-input").removeClass("error");
c.find(".js-validate-error-msg").remove();b.find(".js-validate-error-input").removeClass("error");b.find(".js-validate-error-msg").remove()},updateData:function(a,c,d){b.shop.trace("$.product.updateData("+c+","+d+")",a);var e=b("#s-product-edit-forms .s-product-form.main"),f=this.path.id;if(this.path.id&&"new"!=this.path.id)this.path.id!=a.id&&b.shop.error("Abnormal product ID change",[this.path.id,a]);else{this.path.id=a.id;b(this.options.container_selector).data("product-id",this.path.id);var g=
"/product/"+this.path.id+"/";null!==c&&(c=c||this.path.mode||"profile")&&("profile"!=c&&(g+=c+"/"),null!==d&&(d=d||this.path.tab||"")&&"main"!=d.toLowerCase()&&(g+=d+"/"));b.shop.trace("update location.hash="+g);var h=/#\/product\/new\//,k="#/product/"+this.path.id+"/";b("#shop-productprofile, #s-product-edit-menu, #s-product-edit-forms, #s-product-profile-page").find('[href*="#/product/new/"]').each(function(){b(this).attr("href",b(this).attr("href").replace(h,k))});b("#s-product-edit-forms").find(':input[name="product\\[id\\]"]').val(this.path.id);
window.location.hash=g}c=b("#shop-productprofile h1");c.find(".s-product-name:first").text(a.name);c.find(".s-product-id").show().text("id: "+a.id);c=c.find(".s-new-product-editor-button");c.length&&(d=c.attr("href"),0<=d.indexOf("%product_id%")&&c.attr("href",d.replace("%product_id%",a.id)),c.show());document.title=a.name+b.product.options.title_suffix;b("#product-stock-stat").html(tmpl("template-product-stock-stat",a.raw));e.find(':input[name="product\\[type_id\\]"]').parents(".value").data("type",
a.type_id);if(b.isEmptyObject(a.frontend_urls))b(".s-product-frontend-url-not-empty").hide(),b(".s-product-frontend-url-empty").show();else{c=b("#s-product-frontend-url");c.text(a.url);b("#s-product-frontend-url-input").val(a.url);c.trigger("readable");d=c.parent();d.attr("href",a.frontend_urls[0].url);d.find("span:first").text(a.frontend_urls[0].base);c.closest("div.value").find(".s-product-frontend-url").each(function(c){a.frontend_urls[c+1]&&b(this).attr("href",a.frontend_urls[c+1].url).text(a.frontend_urls[c+
1].url)});b(".s-product-frontend-url-not-empty").show();b(".s-product-frontend-url-empty").hide();c="";d=0;for(g=a.frontend_urls.length;d<g;d+=1)c+=' <span class="s-product-frontend-url-not-empty"><a href="'+a.frontend_urls[d].url+'" target="_blank">'+a.frontend_urls[d].url+'</a><i class="icon10 new-window"></i></span> ';c&&b("#s-product-frontend-links").find(".s-product-frontend-url-not-empty").wrapAll("<div></div>").closest("div").replaceWith(c)}this.editTabMainUpdate(a.raw||{},a.features_selectable_strings,
f);e.find("select.s-product-categories").each(function(){var c=b(this),d=c.parent(),e=c.val();if("select"==e||"---"==e){var f=b("#s-product-edit-forms .s-product-form.main select.s-product-categories");1<f.length?d.remove():d.find(".s-product-delete-from-category").hide()}else if("create"==e){var g=d.find("input.val").val();"0"==g?(f=b("#s-product-edit-forms .s-product-form.main select.s-product-categories"),1<f.length?d.remove():(c.val(g),c.show().attr("disabled",!1),d.find("input.val").attr("disabled",
!0),d.find(".s-new-category").hide(),d.find(".s-product-delete-from-category").hide())):(c.val(g),c.show().attr("disabled",!1),d.find("input.val").attr("disabled",!0),d.find(".s-new-category").hide(),d.find(".s-product-delete-from-category").show())}e=parseInt(e,10);if(!b.isEmptyObject(a.storefront_map[e])){c="";e=a.storefront_map[e];for(f=0;f<e.length;f+=1)c+='<a class="hint" href="'+e[f]+'" target="_blank">'+e[f]+"</a> ";d.find(".s-storefront-map").html(c)}});b("#s-product-categories").html(tmpl("template-product-categories",
{categories:a.categories||[]}));b("#s-product-tags").html(tmpl("template-product-tags",{tags:a.tags||[]}));b("#s-product-save table.s-product-skus > tbody > tr").each(function(){var c=b(this).data("id");b(this).find("td.s-stock .s-product-stock-icon").each(function(){var d=b(this).data("stock");a.raw&&a.raw.skus&&a.raw.skus[c]&&a.raw.skus[c].stock_icon[d]&&b(this).html(a.raw.skus[c].stock_icon[d])})});e.find("tr.js-sku-settings").each(function(){var c=b(this),d=c.attr("data-id");(d=a.raw.skus[d])&&
