...
initCollapsible:function(){var a=function(a,e){b(a).removeClass("darr").addClass("rarr");var g=c(a).hide();e||b.storage.set("shop/products/"+a.id+"/collapse",1);g=g.closest("li");"category"==g.attr("data-type")&&g.trigger("count_subtree")},c=function(a){a=b(a).parent();return a.is("li")?a.find("ul:first"):a.next()};b(".collapse-handler").die("click").live("click",function(){if(b(this).hasClass("darr"))a(this);else{b(this).removeClass("rarr").addClass("darr");var d=c(this).show();b.storage.del("shop/products/"+
...
"error");e.find(".js-progressbar-container").hide();e.find(".fileupload:first").addClass("error").show()},formData:{product_id:b.product.path.id,sku_id:a,_csrf:g}})}catch(f){b.shop.error("Exception "+f.message,f)}}})})},editTabMainSkuImageSelect:function(a,c,d){d.parents("ul").find("li.selected").removeClass("selected");d.parents("li").addClass("selected");d.parents("div.value").find('input[name$="[image_id]"]').val(c);b.shop.trace("$.product.editTabMainSkuImageSelect",[d.parents("div.value"),d.parents("div.value").find(':input[name$="[image_id]"]')]);
b.product.helper.onChange(d.parents("div.s-product-form"))},editTabMainSkuEproductSave:function(a){a=b("#s-product-edit-forms .s-product-form.main tr.js-sku-settings"+(a&&0<a?'[data-id="'+a+'"]':"")+" > td:first .fileupload");b.shop.trace("$.product.editTabMainSkuEproductSave",a.length);a.length&&a.fileupload("start")},editTabMainSkuEproductDelete:function(){},editTabMainLinkEdit:function(){return!1},editTabMainTypeSelect:function(b){var c=b.parents("form").find(':input[name="product[type_id]"]').parents(".value");
"main"==this.path.tab?b.hide():(c.find(".js-action").hide(),window.location.hash="/product/"+this.path.id+"/"+this.path.mode+"/");c.find(".js-type-name").hide();c.find(":input").show().focus();setTimeout(function(){c.find(":input").focus()},100)},editTabMainTypeChange:function(a){var c=a.parents(".value"),d=a.find(":selected:first"),e=d.val();b.shop.trace("debug",a);e!=c.data("type")?(c.find(".js-type-icon").html(d.data("icon")),c.find(".js-type-name").html(d.text()),b("#s-product-edit-forms .s-product-form.features").length&&
"undefined"!=typeof this.editTabFeaturesReload?this.editTabFeaturesReload(e):(a=b("#s-product-edit-menu > li.features > a"),a.attr("href",a.attr("href").replace(/\/features\/.*$/,"/features/"+e+"/")))):(a=b("#s-product-edit-menu > li.features > a"),a.attr("href",a.attr("href").replace(/\/features\/.*$/,"/features/")));e=b(this.options.form_selector);a=e.find('input[name="product[sku_type]"]:first');sku_type=a.is(":radio")?e.find('input[name="product[sku_type]"]:checked').val():a.val();var e=b("#s-sku-type-field-group"),
g=e.find(".s-base-price-selectable-currency").contents();e.load("?module=product&action=featuresSelectable&id="+b.product.path.id+"&type_id="+d.val()+"&sku_type="+sku_type,function(){b("#s-sku-type-field-group").find(".s-base-price-selectable-currency").append(g);b.product.featureSelectableInit()})},editTabMainUpdate:function(a,c,d){var e=b("#s-product-edit-forms .s-product-form.main table.s-product-skus tbody"),g=b("#s-product-view table.s-product-skus tbody");e.find('tr[data-id^="-"]').remove();
g.find('tr[data-id^="-"]').remove();e.parents("table").find("tr:hidden").show();var f=[],h;for(h in a.skus||{})f.push(b.extend({id:h},a.skus[h]));f=f.sort(function(b,a){return b.sort-a.sort});h="";for(var j=0,k=f.length;j<k;j+=1)h+=tmpl("template-sku-edit",{sku_id:f[j].id,sku:f[j],stocks:this.getData("main","stocks"),checked:f[j].id==a.sku_id});e.html(h);e.find("select.s-product-currency").val(a.currency);e.parent().show();this.multiSkus(e.find("tr:not(.js-sku-settings)").length);h="";j=0;for(k=f.length;j<
k;j+=1)h+=tmpl("template-sku",{sku_id:f[j].id,sku:f[j],stocks:this.getData("main","stocks")});g.html(h);if(!d||"new"==d)d=b("#s-sku-type-field-group .s-sku-type-field-value"),e.parents("div.field:first").append(d);c&&(e=b("#s-product-feature-superposition .superposition-count"),e.find(".options").text(c.options),e.find(".skus").html('<span class="highlighted">'+c.skus+"</span>"));var m=/#\/product\/new\//,l="#/product/"+a.id+"/";b("#s-product-edit-forms .s-product-form.main table.s-product-skus tbody, #s-product-view table.s-product-skus tbody").find('[href*="#/product/new/"]').each(function(){b(this).attr("href",
b(this).attr("href").replace(m,l))})},editTabDescriptionsInit:function(){var a=[];a.push("wa-content/js/elrte/elrte.min.js");a.push("wa-content/js/elrte/elrte-wa.js");"en"!=wa_lang&&a.push("wa-content/js/elrte/i18n/elrte."+wa_lang+".js");for(var c=0;c<a.length;c++)b.product.helper.loadScript(a[c])},editTabPagesInit:function(){var a=[];a.push("wa-content/js/elrte/elrte.min.js");a.push("wa-content/js/elrte/elrte-wa.js");"en"!=wa_lang&&a.push("wa-content/js/elrte/i18n/elrte."+wa_lang+".js");for(var c=
0;c<a.length;c++)b.product.helper.loadScript(a[c])},editTabDescriptionsBlur:function(){waEditorUpdateSource({id:"s-product-description-content"})},editTabDescriptionsAction:function(){var a=10;(function d(){try{if(b.product.ajax.cached["elrte-wa.js"]&&b.product.ajax.cached["elrte.min.js"]){var e=b("#s-product-description-content");b("#s-product-description").html(tmpl("template-product-description",{description:e.val()}));e.remove();waEditorInit({id:"s-product-description-content",prefix:"s-product-description-",
upload_url:"",lang:wa_lang,save_button:"s-product-save-button",change_callback:function(){if("descriptions"==b.product.path.tab)b.product.helper.onTabChanged(b.product.path.tab,!0)}})}else a-=1,0<a&&(b.shop.trace("$.product.editTabDescriptionsAction wait while js are loading",b.product.ajax),setTimeout(function(){d()},500))}catch(g){console.error(g.message),console.dir(g.trace)}})()},profileInit:function(){wa_editor&&(wa_editor=void 0);if(this.options.edit_rights){var a=b("#shop-productprofile").find(".s-product-name");
parseInt(this.path.id,10)?a.addClass("editable"):a.removeClass("editable");a.inlineEditable({minSize:{width:150},maxSize:{width:550},inputClass:"s-title-h1-edit",beforeMakeEditable:function(){b("#s-edit-product").hide()},afterBackReadable:function(a,c){b("#s-edit-product").show();if(!c.changed)return!1;b.shop.jsonPost("?module=product&action=save&id="+b.product.path.id,{update:{name:b(a).val()}},function(){document.title=b(a).val()+b.product.options.title_suffix})},hold:function(){return!b(this).hasClass("editable")}})}var c=
this;setTimeout(function(){c.call("profileLazyInit",[])},2E3)},profileLazyInit:function(){b("#product-sales-plot").empty();b("#product-sku-plot").empty();sales_plot_data&&sales_plot_data.length?b.jqplot("product-sales-plot",sales_plot_data,{seriesColors:"#3b7dc0 #129d0e #a38717 #ac3562 #1ba17a #87469f #6b6b6b #686190 #b2b000 #00b1ab #76b300".split(" "),grid:{borderWidth:0,shadow:!1,background:"#ffffff",gridLineColor:"#eeeeee"},series:[{color:"#129d0e",yaxis:"y2axis",shadow:!1,lineWidth:3,fill:!0,
fillAlpha:0.1,fillAndStroke:!0,markerOptions:{show:!1},rendererOptions:{highlightMouseOver:!1}}],axes:{y2axis:{min:0,tickOptions:{markSize:0}},xaxis:{pad:1,showTicks:!1}},highlighter:{lineWidthAdjust:12.5}}):b("#product-sales-plot").remove();sku_plot_data&&sku_plot_data.length?b.jqplot("product-sku-plot",sku_plot_data,{seriesColors:"#0077CC #33BB11 #EE5500 #EEBB11 #44DDDD #6b6b6b #686190 #b2b000 #00b1ab #76b300".split(" "),grid:{borderWidth:0,background:"#ffffff",shadow:!1},legend:{show:!0,location:"s"},
seriesDefaults:{shadow:!1,renderer:b.jqplot.PieRenderer,rendererOptions:{padding:0,sliceMargin:1,showDataLabels:!1}}}):b("#product-sku-plot").remove()}}})(jQuery);(function(b){b.fn.rateWidget=function(a,c,d){function e(a){var d=0;this.find("i").removeClass("star star-hover").addClass("star-empty").each(function(){if(d==a)return!1;d++;b(this).removeClass("star-empty").addClass("star")});this.attr("data-rate",a)}function g(b){return function(){return b}}var f,h=this;if(this&&this.length){if("string"==typeof a){if("getOption"==a&&"rate"==c)return parseInt(this.attr("data-rate"),10);if("setOption"==a&&("rate"==c&&(e.call(this,parseInt(d,10)),c={rate:d}),"object"===
