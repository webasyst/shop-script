...
c.find(".s-product-id").show().text("id: "+a.id);c=c.find(".s-new-product-editor-button");c.length&&(d=c.attr("href"),0<=d.indexOf("%product_id%")&&c.attr("href",d.replace("%product_id%",a.id)),c.show());document.title=a.name+b.product.options.title_suffix;b("#product-stock-stat").html(tmpl("template-product-stock-stat",a.raw));e.find(':input[name="product\\[type_id\\]"]').parents(".value").data("type",a.type_id);if(b.isEmptyObject(a.frontend_urls))b(".s-product-frontend-url-not-empty").hide(),b(".s-product-frontend-url-empty").show();
else{c=b("#s-product-frontend-url");c.text(a.url);b("#s-product-frontend-url-input").val(a.url);c.trigger("readable");d=c.parent();d.attr("href",a.frontend_urls[0].url);d.find("span:first").text(a.frontend_urls[0].base);c.closest("div.value").find(".s-product-frontend-url").each(function(c){a.frontend_urls[c+1]&&b(this).attr("href",a.frontend_urls[c+1].url).text(a.frontend_urls[c+1].url)});b(".s-product-frontend-url-not-empty").show();b(".s-product-frontend-url-empty").hide();c="";d=0;for(g=a.frontend_urls.length;d<
g;d+=1)c+=' <span class="s-product-frontend-url-not-empty"><a href="'+a.frontend_urls[d].url+'" target="_blank">'+a.frontend_urls[d].url+'</a><i class="icon10 new-window"></i></span> ';c&&b("#s-product-frontend-links").find(".s-product-frontend-url-not-empty").wrapAll("<div></div>").closest("div").replaceWith(c)}this.editTabMainUpdate(a.raw||{},a.features_selectable_strings,f);e.find("select.s-product-categories").each(function(){var c=b(this),d=c.parent(),e=c.val();if("select"==e||"---"==e){var f=
b("#s-product-edit-forms .s-product-form.main select.s-product-categories");1<f.length?d.remove():d.find(".s-product-delete-from-category").hide()}else if("create"==e){var g=d.find("input.val").val();"0"==g?(f=b("#s-product-edit-forms .s-product-form.main select.s-product-categories"),1<f.length?d.remove():(c.val(g),c.show().attr("disabled",!1),d.find("input.val").attr("disabled",!0),d.find(".s-new-category").hide(),d.find(".s-product-delete-from-category").hide())):(c.val(g),c.show().attr("disabled",
!1),d.find("input.val").attr("disabled",!0),d.find(".s-new-category").hide(),d.find(".s-product-delete-from-category").show())}e=parseInt(e,10);if(!b.isEmptyObject(a.storefront_map[e])){c="";e=a.storefront_map[e];for(f=0;f<e.length;f+=1)c+='<a class="hint" href="'+e[f]+'" target="_blank">'+e[f]+"</a> ";d.find(".s-storefront-map").html(c)}});b("#s-product-categories").html(tmpl("template-product-categories",{categories:a.categories||[]}));b("#s-product-tags").html(tmpl("template-product-tags",{tags:a.tags||
[]}));b("#s-product-save table.s-product-skus > tbody > tr").each(function(){var c=b(this).data("id");b(this).find("td.s-stock .s-product-stock-icon").each(function(){var d=b(this).data("stock");a.raw&&a.raw.skus&&a.raw.skus[c]&&a.raw.skus[c].stock_icon[d]&&b(this).html(a.raw.skus[c].stock_icon[d])})});e.find("tr.js-sku-settings").each(function(){var c=b(this),d=c.attr("data-id");(d=a.raw.skus[d])&&void 0!==d.file_name&&!d.file_name&&(c.find(".s-sku-attachment").hide(),c.find(".fileupload").show())});
