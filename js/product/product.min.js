...
"function"&&d(a.data,b);location.href="#/products/"+b+"_id="+a.data.id;wa_editor=void 0;c.trigger("close")},k=function(a){if(a&&a.errors){var a=a.errors,b;for(b in a)c.find("input[name="+b+"]").addClass("error").parent().find(".errormsg").text(a[b]);return false}wa_editor=void 0};a("#s-category-description-content").length&&waEditorUpdateSource({id:"s-category-description-content"});if(e.find("input:file").length)a.products._iframePost(e,f,k);else{a.shop.jsonPost(e.attr("action"),e.serialize(),f,
k);return false}},onCancel:function(){wa_editor=void 0}})})},initCategoryDescriptionWysiwyg:function(a){a=a.find(".field.description");a.find("i").hide();a.find(".s-editor-core-wrapper").show();waEditorInit({id:"s-category-description-content",prefix:"s-category-description-",upload_url:"",lang:wa_lang})}}})(jQuery);(function(a){Array.prototype.filter||(Array.prototype.filter=function(a,c){var d=this.length;if("function"!=typeof a)throw new TypeError;for(var e=[],f=0;f<d;f++)if(f in this){var g=this[f];a.call(c,g,f,this)&&e.push(g)}return e});a.storage=new a.store;a.products={hash:"",list_hash:"",list_params:{},options:{view:"thumbs"},random:"",init:function(b){a.extend(this.options,b);this.initRouting();this.initSearch();a.product_sidebar.init();a.categories_tree.init();this.initCollapsible()},data:{prev_action:null},
...
this.options.view;a.storage.set("shop/products/view",b.view);this.list_hash=this.hash;this.list_params=b;if(a.product_list!==void 0&&a.product_list.fixed_blocks!==void 0){a.product_list.fixed_blocks.set&&a.product_list.fixed_blocks.set.unsetFixed();a.product_list.fixed_blocks.category&&a.product_list.fixed_blocks.category.unsetFixed()}this.load("?module=products&"+this.buildProductsUrlComponent(b))},productAction:function(b,c,d){var e=Array.prototype.slice.call(arguments).filter(function(a){return a.length}).join("/");
...
b.id&&a.shop.error("Abnormal product ID change",[this.path.id,b]);c=a("#shop-productprofile h1");c.find(".s-product-name:first").text(b.name);c.find(".s-product-id").show().text("id: "+b.id);document.title=b.name+a.product.options.title_suffix;a("#product-stock-stat").html(tmpl("template-product-stock-stat",b.raw));e.find(':input[name="product\\[type_id\\]"]').parents(".value").data("type",b.type_id);a.isEmptyObject(b.frontend_urls)?(a(".s-product-frontend-url-not-empty").hide(),a(".s-product-frontend-url-empty").show()):
(c=a("#s-product-frontend-url"),c.text(b.url),a("#s-product-frontend-url-input").val(b.url),c.trigger("readable"),d=c.parent(),d.attr("href",b.frontend_urls[0].url),d.contents(":eq(0)").replaceWith(b.frontend_urls[0].base),c.closest("div.value").find(".s-product-frontend-url").each(function(c){b.frontend_urls[c+1]&&a(this).attr("href",b.frontend_urls[c+1].url).text(b.frontend_urls[c+1].url)}),a(".s-product-frontend-url-not-empty").show(),a(".s-product-frontend-url-empty").hide(),a("#s-product-frontend-links").find("a").each(function(c){a(this).attr("href",
b.frontend_urls[c].url).text(b.frontend_urls[c].url)}));this.editTabMainUpdate(b.raw||{},b.features_selectable_strings,f);e.find("select.s-product-categories").each(function(){var c=a(this),d=c.parent(),e=c.val();if(e=="select"||e=="---"){var f=a("#s-product-edit-forms .s-product-form.main select.s-product-categories");f.length>1?d.remove():d.find(".s-product-delete-from-category").hide()}else if(e=="create"){var g=d.find("input.val").val();if(g=="0"){f=a("#s-product-edit-forms .s-product-form.main select.s-product-categories");
if(f.length>1)d.remove();else{c.val(g);c.show().attr("disabled",false);d.find("input.val").attr("disabled",true);d.find(".s-new-category").hide();d.find(".s-product-delete-from-category").hide()}}else{c.val(g);c.show().attr("disabled",false);d.find("input.val").attr("disabled",true);d.find(".s-new-category").hide();d.find(".s-product-delete-from-category").show()}}e=parseInt(e,10);if(!a.isEmptyObject(b.storefront_map[e])){c="";e=b.storefront_map[e];for(f=0;f<e.length;f=f+1)c=c+('<a class="hint" href="'+
e[f]+'" target="_blank">'+e[f]+"</a> ");d.find(".s-storefront-map").html(c)}});a("#s-product-save table.s-product-skus > tbody > tr").each(function(){var c=a(this).data("id");a(this).find("td.s-stock .s-product-stock-icon").each(function(){var d=a(this).data("stock");b.raw&&(b.raw.skus&&b.raw.skus[c]&&b.raw.skus[c].stock_icon[d])&&a(this).html(b.raw.skus[c].stock_icon[d])})});e.find("tr.js-sku-settings").each(function(){var c=a(this),d=c.attr("data-id");if((d=b.raw.skus[d])&&d.file_name!==void 0&&
!d.file_name){c.find(".s-sku-attachment").hide();c.find(".fileupload").show()}});this.helper.checkChanges(this.get("form"),!0);a("#s-product-edit-menu li a i.icon10.status-yellow-tiny").remove();a("#s-product-edit-save-panel :submit").removeClass("yellow").addClass("green");"0"==b.raw.sku_type&&(e=a("#s-product-feature-superposition"),e.find("input[type=checkbox]").attr("checked",!1),f=e.find("ul.features li"),f.find(".count").text(""),f.find("i").removeClass("status-blue-tiny status-gray-tiny").addClass("status-gray-tiny"),
...
b.tab).addClass("selected");this.refresh()},editTabBlur:function(b){if(b){var c=this,d=b.tab;-1!==this.standalone_tabs.indexOf(d)&&this.helper.checkChanges(this.get("form"),!0,function(){c.saveData(c.path.mode,d,function(){d&&c.call(b)})});a("#s-product-edit-forms .s-product-form").hide();a("#s-product-edit-menu li.selected").removeClass("selected");this.path.tab=null;this.path.tail=null}},editTabShow:function(b){var c=a("#s-product-edit-forms .s-product-form."+b.tab);c.length?(c.show(),a.shop.trace("$.product.showTab",
b),this.path.tab=b.tab):a.shop.error("$.product.showTab",b)},editClick:function(b){var c=b.attr("href").replace(/.*#\/product\/(\d+|new)\/edit\//,"").replace(/\/$/,"").split("/"),d=[],e;if(c.length){a.shop.trace("$.product.editClick",c);for(var f,g="editTab";f=c.shift();)g+=f.substr(0,1).toUpperCase()+f.substr(1),f="function"==typeof this[g],a.shop.trace("$.settings.featuresClick try",[g,f,c]),!0===f&&(e=g,d=c.slice(0))}if(e){if(a.shop.trace("$.product.editClick",[e,d]),!b.hasClass("js-confirm")||
confirm(b.attr("title")||"Are you sure?")){d.push(b);try{this[e].apply(this,d)}catch(h){a.shop.error("Error: "+h.message,h)}}}else a.shop.error("Not found js handler for link",[e,b]);return!1},switchSubMenu:function(b){b=b||"default";a("#mainmenu .s-submenu > *").hide();a("#mainmenu .s-submenu .s-submenu-"+b).show()},profileFocus:function(){var b=this.options.duration,c=this;a("#s-product-edit-menu, #s-product-edit-save-panel").hide();c.switchSubMenu();a("#s-sidebar").show().animate({width:a.product.options.sidebar_width});
...
var g=[],h;for(h in b.skus||{})g.push(a.extend({id:h},b.skus[h]));g=g.sort(function(a,b){return a.sort-b.sort});h=this.getData("main","stocks");for(var j=this.getData("main","stock_ids"),k="",m=0,l=g.length;m<l;m+=1)k+=tmpl("template-sku-edit",{sku_id:g[m].id,sku:g[m],stocks:h,stock_ids:j,checked:g[m].id==b.sku_id});e.html(k);e.find("select.s-product-currency").val(b.currency);e.parent().show();this.multiSkus(e.find("tr:not(.js-sku-settings)").length);k="";m=0;for(l=g.length;m<l;m+=1)k+=tmpl("template-sku",
{sku_id:g[m].id,sku:g[m],stocks:h,stock_ids:j,runout:b.runout||{}});f.html(k);(!d||"new"==d)&&e.parents("div.field-group:first").after(a("#s-sku-type-field-group"));c&&(d=a("#s-product-feature-superposition .superposition-count"),d.find(".options").text(c.options),d.find(".skus").html('<span class="highlighted">'+c.skus+"</span>"));var i=/#\/product\/new\//,p="#/product/"+b.id+"/";a("#s-product-edit-forms .s-product-form.main table.s-product-skus tbody, #s-product-view table.s-product-skus tbody").find('[href*="#/product/new/"]').each(function(){a(this).attr("href",
a(this).attr("href").replace(i,p))})},editorUpdateSource:function(a){this.editorActive&&(waEditorUpdateSource(a),wa_editor&&(this.description=wa_editor.getValue()))},editorActive:!1,editTabDescriptionsBlur:function(){this.editorUpdateSource({id:"s-product-description-content"});this.editorActive=!1},editTabDescriptionsAction:function(){this.editTabDescriptionsInitDelayed()},editTabDescriptionsInitDelayed:function(){if(!this.editorActive){var b=a("#s-product-description");if(a("#s-product-description-content").is(":visible"))try{waEditorInit({id:"s-product-description-content",
prefix:"s-product-description-",upload_url:"",lang:wa_lang,html_tab:"s-product-html",wysiwyg_tab:"s-product-wysiwyg",ace_editor_container:"s-product-ace-editor-container",save_button:"s-product-save-button",change_callback:function(){"descriptions"==a.product.path.tab&&(a.product.helper.options.defaultChangedStatus=!0,a.product.helper.onTabChanged(a.product.path.tab,!0))}});var c=b.find("iframe"),d=c.parents(".workzone"),e=d.height(),f=c.contents().find("body").prop("scrollHeight"),g=a(window).height()-
a("#mainmenu").height()-160;f>g&&(f=g);f>e&&(d.height(f),c.height(f));a(".ace_text-input").get(0).defaultValue="\u0001\u0001";this.editorActive=!0}catch(h){a.shop.error(h.message,h)}else{var j=this;a("#s-product-description-content").show();setTimeout(function(){j.editTabDescriptionsInitDelayed()},50)}}},profileInit:function(){void 0!==a.product_list&&void 0!==a.product_list.fixed_blocks&&(a.product_list.fixed_blocks.set&&a.product_list.fixed_blocks.set.unsetFixed(),a.product_list.fixed_blocks.category&&
a.product_list.fixed_blocks.category.unsetFixed());if(this.options.edit_rights){var b=a("#shop-productprofile").find(".s-product-name");parseInt(this.path.id,10)?b.addClass("editable"):b.removeClass("editable");var c=this.get("form");b.inlineEditable({minSize:{width:150},maxSize:{width:550},inputClass:"s-title-h1-edit",beforeMakeEditable:function(){a("#s-edit-product").hide()},afterBackReadable:function(b,d){a("#s-edit-product").show();if(!d.changed)return false;a.shop.jsonPost("?module=product&action=save&id="+
a.product.path.id,{update:{name:a(b).val()}},function(){var d=a(b).val();document.title=d+a.product.options.title_suffix;c.find('input[name="product[name]"]').val(d)})},hold:function(){return!a(this).hasClass("editable")}})}var d=this;setTimeout(function(){d.call("profileLazyInit",[])},2E3)},profileLazyInit:function(){a("#product-sales-plot").empty().length&&(sales_plot_data&&sales_plot_data.length?a.jqplot("product-sales-plot",sales_plot_data,{seriesColors:"#3b7dc0 #129d0e #a38717 #ac3562 #1ba17a #87469f #6b6b6b #686190 #b2b000 #00b1ab #76b300".split(" "),
...
b.removeClass("s-fixed")})},getSelectedProducts:function(b){var b=b||!1,c={count:0};if(this.container.find(".s-select-all").attr("checked")){var d=a.product_list.collection_hash.join("/")||"all";b?c.serialized=[{name:"hash",value:d}]:c.hash=d;c.count=this.total_count}else b?c.serialized=a.product_list.container.find(".product.selected").map(function(){c.count+=1;return{name:"product_id[]",value:a(this).attr("data-product-id")}}).toArray():c.product_id=a.product_list.container.find(".product.selected").map(function(){c.count+=
...
b.length&&(b.parent().remove(),b.remove());a("#s-product-list-settings-dialog").waDialog({disableButtonsOnSubmit:!0,onLoad:function(){a("#s-category-description-content").length&&a.product_sidebar.initCategoryDescriptionWysiwyg(a(this));a("#s-product-list-name").focus()},onSubmit:function(b){var c=a(this),f=function(c){var e=a.product_list.collection_hash,f=location.hash.replace(e[0]+"_id="+e[1],e[0]+"_id="+c.data.id),g=a("#"+e.join("-"));g.find(".name:first").html(c.data.name);a.isArray(c.data.routes)&&
c.data.routes.length?g.find(".routes:first").html(" "+c.data.routes.join(" ")):g.find(".routes:first").html(" ");c.data.status=="0"?g.children("a").addClass("gray"):c.data.status=="1"&&g.children("a").removeClass("gray");g.find(".id:first").html(c.data.id);g.attr("id",e[0]+"-"+c.data.id);g.find("a").attr("href",f);location.hash!=f?location.hash=f:a.products.dispatch();wa_editor=void 0;b.trigger("close")},g=function(a){if(a&&a.errors){var a=a.errors,c;for(c in a)b.find("input[name="+c+"]").addClass("error").parent().find(".errormsg").text(a[c]);
return false}wa_editor=void 0};a("#s-category-description-content").length&&waEditorUpdateSource({id:"s-category-description-content"});if(c.find("input:file").length)a.products._iframePost(c,f,g);else{a.shop.jsonPost(c.attr("action"),c.serialize(),f,g);return false}},onCancel:function(){wa_editor=void 0}})})},exportProducts:function(a,c){for(var d=[],e,f=!1;e=a.serialized.pop();)e.name&&("product_id[]"==e.name?d.push(parseInt(e.value,10)):"hash"==e.name&&(f=e.value));e="?action=importexport#/"+c;
f?window.location=e+("/hash/"+f):d.length&&(e+="/hash/id/"+d.join(","),window.location=e)},remove:function(b,c){var d={},e;b.product_id?e=function(){100>=b.product_id.length&&(d.get_lists=!0);d.product_id=b.product_id.splice(0,100);a.shop.jsonPost("?module=products&action=deleteList",d,function(a){b.product_id.length?e():typeof c==="function"&&c(a)})}:(d.hash=b.hash||this.collection_hash.join("/")||"all",d.remove=a.isArray(b.remove)&&b.remove.length?b.remove:["list"],1==d.remove.length&&"list"==d.remove[0]?
e=function(){a.shop.jsonPost("?module=products&action=deleteList",d,c)}:(d.count=100,e=function(){a.shop.jsonPost("?module=products&action=deleteList",d,function(a){0<a.data.rest_count&&null!=a.data.rest_count?e():"function"===typeof c&&c(a)})}));e()}}})(jQuery);
