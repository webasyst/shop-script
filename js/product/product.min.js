...
minLength:3,delay:300,select:function(d,e){c.val("");"new_editor"===a(this).attr("data-editor")?window.location.href=window.location.pathname+"products/"+e.item.id:a.wa.setHash("#/product/"+e.item.id);return!1}})},jsonPost:function(b,c,d,e){a.shop.jsonPost(b,c,d,e)},_iframePost:function(b,c,d){var e=b.attr("id")+"-iframe";a("#"+e).length||b.after("<iframe id="+e+" name="+e+" style='display:none;'></iframe>");var f=a("#"+e);b.attr("target",e);f.one("load",function(){try{var g=a(this).contents().find("body").html();
...
initInlineEditor:function(){function c(g,h){if(g.data("denied-reason")==h)g.data("denied-reason",""),alert(h);else{g.data("denied-reason",h);setTimeout(function(){g.data("denied-reason","")},3E3);g.wrapInner('<div class="animation-container" style="position:relative;"></div>');var k=g.find(".animation-container").animate({left:"3px"},100,function(){k.animate({left:"0px"},100,function(){k.animate({left:"3px"},100,function(){k.animate({left:"0px"},100,function(){k.children().first().unwrap()})})})})}}
function d(g){var h=[];a.product_list.container.find("tr.product").each(function(){var k=a(this);void 0===k.data("original-price")&&k.data("edit-rights")&&h.push(a(this).data("product-id"))});if(!h.length)return g();a.post("?module=products&action=inlineEditorData",{ids:h},function(k){k.data&&a.product_list.container.find("tr.product").each(function(){var l=a(this),m=k.data[parseInt(l.data("product-id"),10)];m&&(l.data("stocks",m.stocks||null),l.data("original-price",m.price.replace(/\.?0+$/g,"")))});
g()},"json")}var e=this,f=0;e.options.stocks&&e.options.stocks.length&&(f=e.options.stocks.length);a.product_list.highlightInlineEditorCells();a.product_list.container.on("append_product_list",function(){a.product_list.highlightInlineEditorCells()});a.product_list.container.on("click",".s-product-price,.s-product-stock",function(){function g(){if(l)h.html(tmpl("template-list-price-editor-one",{product_id:k.data("product-id"),currency:k.data("currency"),value:k.data("original-price")}));else{var m=
k.data("stocks");f?(m&&"object"===typeof m||(m={}),h.html(tmpl("template-list-stock-editor-many",{product_id:k.data("product-id"),values:m}))):(m&&"object"!==typeof m||(m=""),h.html(tmpl("template-list-stock-editor-one",{product_id:k.data("product-id"),value:m})))}h.find(".cancel").click(function(){var n=h;n.removeClass("editor-on").html(n.data("read-only-state-html"));n.data("read-only-state-html","");n.data("denied-reason","")});var p=h.find("form").submit(function(){if(p.find(".loading").length)return!1;
...
v.message)}},"json");return!1});p.on("dblclick",!1);setTimeout(function(){h.find(":input:visible:first").focus()},0)}var h=a(this);if(!h.data("plugin")){var k=h.closest("tr"),l=h.hasClass("s-product-price");if(l){if(k.data("min-price")!=k.data("max-price"))return c(h,$_("Inline editing is available only for products with a single SKU"))}else if("1"!=k.data("sku-count"))return c(h,$_("Inline editing is available only for products with a single SKU"));if(!k.data("edit-rights"))return c(h,$_("Insufficient access rights"));
h.hasClass("editor-on")||(h.addClass("editor-on").data("read-only-state-html",h.html()),a.product_list.container.removeClass("single-lined"),void 0!==k.data("original-price")?g():(h.html('<i class="icon16 loading"></i>'),d(g)))}})},initFixedBlocks:function(){var c=a("#mainmenu").height(),d=a("#s-sidebar").width(),e=a(window),f=a("#s-fixed-blocks");f.length||(f=a('<div id="s-fixed-blocks"></div>').css({display:"none",position:"fixed",background:"#fff",top:c,width:d,"float":"left"}).appendTo("#s-sidebar"));
var g=a("#s-category-list"),h=a("#s-category-list-block"),k=a("#s-set-list"),l=a("#s-set-list-block"),m=function(){function n(){if(!x){var u=a("#s-category-list-handler");t=a.categories_tree.isCollapsed(u);q=g.is(":hidden");var w=a('<div id="s-category-list-dummy" class="block"></div>').css({height:h.height()});h.replaceWith(w);f.show().append(h.hide());h.find(".heading").before('<span class="hint float-right" style="margin-top: 3px;">'+$_("Drag products here")+"</span>");t&&a.categories_tree.expand(u,
null,function(){a.categories_tree.collapse(u)});q||g.hide();h.show();a("i.add",h).hide();a("#s-category-list-handler").hide();h.css({"overflow-x":"hidden","overflow-y":"auto"});(function(){var A=g.is(":hidden"),y;h.bind("mousemove.fixed_blocks.category",function(z){document.getElementById("products-helper")&&A&&(y&&(clearTimeout(y),y=null),y=setTimeout(function(){if(A){h.css({height:""});g.show();var B=h.height(),D=(e.height()-c)/2;h.css({height:Math.min(B,D)});h.find(".heading").prev(".hint").hide();
A=!1}},250))});e.bind("mouseout.fixed_blocks.category",function(z){z.pageX>d+10&&(y&&clearTimeout(y),y=null,A||(g.hide(),h.css({height:""}),h.find(".heading").prev(".hint").show(),A=!0))})})();x=!0}}function r(){if(x){var u=a("#s-category-list-dummy");u.replaceWith(h);h.css({"overflow-x":"","overflow-y":""});var w=a("#s-category-list-handler");t&&a.categories_tree.collapse(w);q||g.show();h.find(".heading").prev(".hint").remove();a("#s-category-list-handler").show();a("i.add",h).show();u.remove();
h.unbind("mousemove.fixed_blocks.category");e.unbind("mouseout.fixed_blocks.category");f.children().length||f.hide();x=!1;a.categories_tree.init()}}var v=h.offset().top,x=!1,q,t;e.unbind("scroll.fixed_blocks.category").bind("scroll.fixed_blocks.category",function(){x||(v=h.offset().top);var u=v;u=document.getElementById("s-category-list-dummy")?u+a("#s-category-list-dummy").height():u+a("#s-category-list-block").height();e.scrollTop()+c+5>=u?n():r()});return{setFixed:n,unsetFixed:r}}(),p=function(){function n(){if(!q){x=
...
return{category:m,set:p}},initEditingControls:function(){var c=a("#s-product-list-title"),d=a.product_list.collection_hash[0],e="?module=category&action=save&";"set"===d&&(e="?module=set&action=save&");c.hasClass("editable")&&(c.inlineEditable({minSize:{width:350},maxSize:{width:600},size:{height:30},inputClass:"s-title-h1-edit",beforeMakeEditable:function(){a(".s-product-list-manage").hide()},afterBackReadable:function(f,g){a(".s-product-list-manage").show();if(!g.changed)return!1;a.shop.jsonPost(e+
a.product_list.collection_param+"&edit=name",{name:a(f).val()},function(h){a.product_list.sidebar.find("#"+d+"-"+h.data.id).find(".name:first").html(h.data.name)})}}),"name"==this.options.edit&&c.trigger("editable"));c=a("#s-product-list-settings");!c.length||"category"!=d&&"set"!=d||c.unbind("click").bind("click",function(){"category"==d?shopDialogProductsCategory.staticDialog(a.product_list.collection_hash[1],null,"edit"):shopDialogProductsSet.staticDialog(a.product_list.collection_hash[1],"edit");
return!1});c=a("#s-product-list-delete");!c.length||"category"!=d&&"set"!=d||c.unbind("click").bind("click",function(){a("#s-product-list-delete-dialog").waDialog({disableButtonsOnSubmit:!0,onLoad:function(){a(this).find(".dialog-buttons i.loading").hide()},onSubmit:function(){function f(p,n){if(!p.length)return n&&n();var r=p.shift();g(["category",r],function(){f(p,n)})}function g(p,n){var r={hash:p.join("/")||"all",remove:["list"]};m&&r.remove.push("products");var v=function(){a.product_list.remove(r,
function(q,t){h(p,q);a.product_list.sidebar.trigger("update",q.data.lists);n&&n()})},x=a("#"+p.join("-")+"-handler");x.length?a.categories_tree.expand(x,function(q){q.parent().find("ul:first").hide();v()}):v();return!1}function h(p,n){var r=p[0],v=a("#"+p.join("-")),x=v.prev(".drag-newposition");p=a("#s-"+p[0]+"-list");var q=p.find("ul:first");"set"==r?(v.remove(),x.remove()):(r=v.find("ul:first"),r.length&&(r.find(">li.drag-newposition:first").remove(),v.hide(),r.find(">li").each(function(){v.after(this)})),
