...
return!1});b.off("click",".s-new-list").on("click",".s-new-list",function(){var b=a(this),d=b.attr("id"),c=0;d?b=d.replace("s-new-",""):(c=b.parents("li.dr:first").attr("id").split("-"),b=c[0],c=c[1]);a.product_sidebar.createListDialog(b,c,function(b,c){var d=a("#s-"+c+"-list"),e=d.find("ul:first");e.length||(d.prepend('<ul class="menu-v with-icons"><li class="drag-newposition" data-type="'+c+'"></li></ul>'),d.find(".drag-newposition").mouseover(),e=d.find("ul:first"));var f=parseInt(b.parent_id,
...
c.data("type")!=b)c={base_price_selectable:c.find(':input[name="product[base_price_selectable]"]').val(),currency:c.find("select.s-product-currency").val()},parseInt(c.base_price_selectable)||(c.base_price_selectable=a(':input[name^="skus"][name$="[price]"]:first').val()),a.product.skuTypeFeaturesSelectableLoad(b,c);else a.product.onSkuTypeEnabled();else c.hide(),c.find("input").attr("disabled",!0),c=a(this.options.form_selector).find(".s-product-skus"),"new"==a.product.path.id?a.product.disableSkus(!1,
!1):(b=c.find("tbody"),a.product.disableSkus(!0,!0),b.find(">tr:not(.s-sku-virtual)").length||(a.product.editTabMainSkuAdd(),b.find(">tr:not(.s-sku-virtual):first").find('input[name="product[sku_id]"]').attr("checked",!0))),c.closest(".field").find(".name").show()},featureSelectableInit:function(){var b=a(this.options.form_selector),c=a("#s-product-feature-superposition"),e=b.find(".s-product-skus"),b=b.find(':input[name="product[base_price_selectable]"]'),d=this;c.on("click","ul.features>li",function(){var b=
a(this);b.parent().find("li.selected").removeClass("selected");b.addClass("selected");b=b.attr("data-feature-id");b=c.find(".feature-values[data-feature-id="+b+"]");c.find(".feature-values").hide();b.show();return!1});c.on("change","ul.values>li input",function(){var b=a(this).parents("li:first").parent().find(">li input:checked").length,e=c.find("ul.features li.selected");e.find(".count").text(b||"");b=b?"status-blue-tiny":"status-gray-tiny";e.find("i").removeClass("status-blue-tiny status-gray-tiny").addClass(b);
d.featureSelectableCount(c)});a.shop.changeListener(b,function(){e.find(".s-sku-virtual .s-price input").val(a(this).val())});a.shop.changeListener(e,function(){var b=a(this);if("product[sku_id]"!=b.attr("name")&&(b=b.parents("tr:first"),b.length)){var c=parseInt(b.attr("data-id"),10);if(c){if(b.hasClass("js-sku-settings")&&(b=e.find("tr[data-id="+c+"]:first"),!b.length))return;b.find("input.s-input-virtual").val(0);b.removeClass("s-sku-virtual");a("#s-product-view").find("tr[data-id="+c+"]").removeClass("s-sku-virtual")}}},
"input, textarea");c.find("i.status-blue-tiny:first").parents("li").trigger("click");return c},featureSelectableCount:function(b){b||(b=a("#s-product-feature-superposition"));var c=[],e=a("#s-product-edit-forms .s-product-form.features");b.find("ul.features>li").each(function(){var d=a(this),f=parseInt(d.find(".count").text(),10);f&&c.push(f);if(e.length){var g=e.find('div[data-code="'+d.data("feature-code")+'"] div.value');if(f){var d=d.data("feature-id"),h=b.find('div.feature-values[data-feature-id="'+
d+'"] ul.values input:checked');g.each(function(){var b=a(this),c=h.filter('[value="'+b.data("value-id")+'"]').length;b.find("input:checkbox").attr("checked",c?"checked":null).attr("disabled","disabled");c?b.show():b.hide()})}else g.show(),g.find("input:checkbox").each(function(a,b){b.checked=b.defaultChecked;b.disabled=null})}});var d=b.find(".superposition-count");if(c.length){d.find(".options").text(c.join(" x ")+" "+$_("options"));for(var f=1,g=0,h=c.length;g<h;g+=1)f*=c[g];d.find(".skus").html('<span class="highlighted">'+
$_("%d SKUs in total").replace("%d",f)+"</span>")}else d.find(".options").text(""),d.find(".skus").html("")},editInit:function(){a("#s-product-edit-forms").unbind("keydown.product").bind("keydown.product",function(b){if(a(b.target).is(":input")&&(b.ctrlKey||b.metaKey)&&83==b.keyCode)return a("#s-product-save-button").click(),!1});a.shop.makeFlexibleInput("s-product-meta-title");if(!this.options.edit_rights)return window.location.hash="#/product/"+this.path.id+"/",!1;var b=a(this.options.form_selector);
b.bind("submit.product",function(b){return a.product.saveData("profile",null)});b.on("change.product, keyup.product","div.s-product-form:not(.ajax) :input",function(b){a.product.helper.onChange(a(this).parents("div.s-product-form"))});b.on("change.product, keyup.product, keypress.product",':input[name="product[name]"]',function(b){a.product.helper.onNameChange(a(this),!1,a.product.options.update_delay||500)});a.shop.changeListener(b,':input[name="product[url]"]',function(){var b=a(this);a.getJSON("?module=product&action=checkUrlInUse",
{id:a.product.path.id,url:b.val()||""},function(b){a.product.informAboutUrlInUse("ok"===b.status&&b.data.url_in_use)})});var c=a("#product-tags");c.data("tags_input_init")||(c.tagsInput({autocomplete_url:"?module=product&action=tagsAutocomplete",height:120,onChange:function(){a.product.updateMetaFields()},defaultText:""}).data("tags_input_init",!0),a("#s-product-popular-tags").off("click.product","a").on("click.product","a",function(){var b=a(this).text();c.removeTag(b);c.addTag(b)}));b.on("change.product",
".s-product-currency",function(){var c=a(this).val();b.find(".s-product-currency").val(c);b.find(".s-product-currency-readonly").text(c);a("#s-product-currency-code").val(c)});b.on("change.product",':input[name="product[type_id]"]',function(){return a.product.editTabMainTypeChange(a(this))});b.on("change",'select[name="product[status]"]',function(){"1"==a(this).val()?(a(this).prev().removeClass("no-bw").addClass("yes"),a(".s-product-status-text").hide()):(a(this).prev().removeClass("yes").addClass("no-bw"),
a(".s-product-status-text").show())});b.on("change",'input[name="product[sku_type]"]',function(){a.product.onSkuTypeChange(this.value)}).change();a.product.featureSelectableInit()},informAboutUrlInUse:function(b){b?a("#s-product-url-in-use-block").show().find(".s-text").html(b):a("#s-product-url-in-use-block").hide()},editFocus:function(){var b=this.options.duration,c=this;a("h1 .s-product-edit-link, #mainmenu .s-level2, #s-product-frontend-links").hide();a("#s-sidebar, #s-toolbar").animate({width:0},
b).queue(function(){a(this).hide();a(this).dequeue();a("#s-product-edit-menu, #s-product-edit-forms, #s-product-edit-save-panel").show();"new"!=c.path.id?c.get("container").find(".back").attr("href","#/product/"+c.path.id+"/"):c.get("container").find(".back").attr("href","#/"+a.products.list_hash);a("#s-product-profile-page").hide();c.switchSubMenu("productprofile")});a("#maincontent").animate({"margin-top":"40px"},b);a("#s-content").animate({"margin-left":0},b);a("#shop-productprofile").off("click.edit-product",
"a.js-action").on("click.edit-product","a.js-action",function(){return c.editClick(a(this))})},editBlur:function(){this.path.tab=!1;this.path.mode=!1;a(a.product.options.form_selector).off("submit.product");a(this.options.form_selector).off("change.product, keyup.product");a("#s-product-edit-forms").off("click.edit-product")},editAction:function(a){this.path.mode=a.mode},editTabMainData:{sku_id:-1,stocks:{}},editTabMainInit:function(b){a("#s-product-type").each(function(){a(this).parent().get(0)!=
...
a.product.helper.onChange(c))},keydownCallback:function(b){if("115"!=b.which&&"83"!=b.which||!b.ctrlKey&&!b.metaKey)a.product.helper.onChange(c);else return b.preventDefault(),a("#s-product-save-button").click(),!1}})}},profileInit:function(){void 0!==a.product_list&&void 0!==a.product_list.fixed_blocks&&(a.product_list.fixed_blocks.set&&a.product_list.fixed_blocks.set.unsetFixed(),a.product_list.fixed_blocks.category&&a.product_list.fixed_blocks.category.unsetFixed());if(this.options.edit_rights){var b=
a("#shop-productprofile").find(".s-product-name");parseInt(this.path.id,10)?b.addClass("editable"):b.removeClass("editable");var c=this.get("form");b.inlineEditable({minSize:{width:150},maxSize:{width:550},inputClass:"s-title-h1-edit",beforeMakeEditable:function(){a("#s-edit-product").hide()},afterBackReadable:function(b,d){a("#s-edit-product").show();if(!d.changed)return!1;a.shop.jsonPost("?module=product&action=save&id="+a.product.path.id,{update:{name:a(b).val()}},function(d){var g=a(b).val();
document.title=g+a.product.options.title_suffix;"ok"===d.status&&d.data&&a("#s-product-meta-title").attr("placeholder",d.data.default_meta_title);c.find('input[name="product[name]"]').val(g)})},hold:function(){return!a(this).hasClass("editable")}})}this.call("profileLazyInit",[])},profileLazyInit:function(){var b=a("#product-sales-plot");if(window.sales_data&&sales_data.length){if(!b.data("graph_rendered")){var c=function(){b.length&&0<b.width()?(showSalesGraph(sales_data,"undefined"==typeof cash_type?
null:cash_type),b.data("graph_rendered",1)):setTimeout(c,1E3)};c()}}else b.remove();if(sku_plot_data&&sku_plot_data.length&&sku_plot_data[0]&&sku_plot_data[0].length){if(!a("#product-sku-plot").data("graph_rendered")){var e=[];a.each(sku_plot_data[0],function(a,b){e.push({label:b[0],value:b[1]})});showPieGraph(e,{color_type:"products"});a("#product-sku-plot").data("graph_rendered",1)}}else a("#product-sku-plot").remove()},updateMetaFields:function(){var b=a(".s-product-name-input").val();a("#s-product-meta-title").attr("placeholder",
