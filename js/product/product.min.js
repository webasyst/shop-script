...
"green":"yellow").addClass(e?"yellow":"green");if(e)d.checkChanges(b,false,function(a){d.onTabChanged(c,a)});else d.onTabChanged(c,e)})},urlHelper:function(b,c){this.data.url_helper.timer&&clearTimeout(this.data.url_helper.timer);var d={str:a(b).val()};a.shop.trace("$.product.urlHelper ",d);if(d.url!=this.data.url_helper.name){var e=this;this.data.url_helper.url=d.url;a.ajax({url:"?action=transliterate",dataType:"html",data:d}).done(function(b){if((b=a.parseJSON(b))&&"ok"==b.status)e.data.url_helper.url=
...
this.initSelecting();this.initView(this.options.view);this.initDragndrop();this.initSortingNotice();this.rubberTable();"table"==this.options.view&&this.initInlineEditor()}catch(f){a.shop.error("$.product_list.init error: "+f.message,f)}return this},initSortingNotice:function(){if(!a.storage.get("shop/list-sorting-notice-closed")){var b=a("#custom-backend-order-notice");if(b.length)b.show().on("click",".close",function(){a.storage.set("shop/list-sorting-notice-closed","1");b.remove()})}},initLazyLoad:function(b){var c=
b.count,d=this.total_count;a(window).lazyLoad("stop");if(c<d){var e=this,f=e.container.find("tr.header th[data-additional-column]").map(function(){return a(this).data("additional-column")}).get().join(",");a(window).lazyLoad({container:e.container,state:"undefined"===typeof b.auto||b.auto?"wake":"stop",hash:["","products"],load:function(){a(window).lazyLoad("sleep");a(".lazyloading-link").hide();a(".lazyloading-progress").show();a.get("?module=products&action=loadList&offset="+c+("&total_count="+
d)+(e.collection_param?"&"+e.collection_param:"")+(e.sort?"&sort="+e.sort:"")+(e.order?"&order="+e.order:"")+("&enabled_columns="+(f||"")),function(b){if("ok"==b.status&&b.data.count){c+=b.data.count;var f=e.container;try{e.container.append(tmpl("template-product-list-"+e.options.view,{products:b.data.products,check_all:"table"==e.options.view?f.find(".s-select-all:first").attr("checked"):!1,sort:a.product_list.sort})),e.container.trigger("append_product_list",[b.data.products])}catch(j){console.log("Error: "+
j.message);a(window).lazyLoad("stop");return}f.find(".last").removeClass("last");f.find(".product:last").addClass("last");a(".lazyloading-progress-string").text(b.data.progress.loaded+" "+b.data.progress.of);a(".lazyloading-progress").hide();a(".lazyloading-chunk").text(b.data.progress.chunk);c>=d?(a(window).lazyLoad("stop"),a(".lazyloading-link").hide()):(a(".lazyloading-link").show(),a(window).lazyLoad("wake"))}else b.data&&!b.data.count?console&&(console.log("Count of again loaded products is 0"),
a(".lazyloading-progress").hide()):(b.errors&&console&&console.log("Error when loading products: "+b.errors),a(window).lazyLoad("stop"))},"json")}});a(".lazyloading-link").die("click").live("click",function(){a(window).lazyLoad("force");return!1})}},initSelecting:function(){var b=this.container,c=this.toolbar.find(".count"),d=b.find(".s-select-all:first"),e=function(){var a=parseInt(b.find("> .product.selected").length,10);a?c.text(a).show():c.text("").hide()};a(document).keydown(function(a){16==
a.keyCode&&(document.body.onselectstart=function(){return!1})}).keyup(function(a){16==a.keyCode&&(document.body.onselectstart=null)});b.off("select",".product").on("select",".product",function(b,c,h){h=void 0!==h?h:!0;(void 0!==c?c:1)?a(this).addClass("selected").find("input:first").attr("checked",!0):(a(this).removeClass("selected").find("input:first").attr("checked",!1),d.is(":checked")&&d.attr("checked",!1));h&&e();return!1});d.unbind("select").bind("select",function(d,e){var e=void 0!==e?e:!0,
h=a(this);e?(h.attr("checked",!0),b.find("> .product").trigger("select",!0,!1),c.text(h.attr("data-count")).show()):(h.attr("checked",!1),b.find("> .product").trigger("select",!1,!1),c.text("").hide())});b.off("click",".product input").on("click",".product input",function(c,g){var h=c.shiftKey,j=this.checked;g&&(h=g.shiftKey,j=g.checked);var k=a(this).parents(".product:first");j?k.addClass("selected"):(d.is(":checked")&&d.attr("checked",!1),k.removeClass("selected"));if(h&&j){(h=b.data("last_checked"))||
(h=b.find("> .product:first").trigger("select",!0));var m=k.prevAll(".selected[data-product-id="+h.attr("data-product-id")+"]"),l;if(m.length){l=k.prev();for(h=h.get(0);l.length&&h!=l.get(0);)l.addClass("selected").find("input").attr("checked",!0),l=l.prev()}else if(m=k.nextAll(".selected[data-product-id="+h.attr("data-product-id")+"]"),m.length){l=k.next();for(h=h.get(0);l.length&&h!=l.get(0);)l.addClass("selected").find("input").attr("checked",!0),l=l.next()}!b.data("last_checked")&&!m.length&&
h.trigger("selected",!1)}j&&b.data("last_checked",k);e()});if(a.browser.mozilla)b.off("click",".product label").on("click",".product label",function(b){if(b.shiftKey){var c=a(b.target);a(c).is("label")||(c=a(c).parents("label:first"));c=a("#"+c.attr("for"));c.trigger("click",{shiftKey:b.shiftKey,checked:!c.attr("checked")})}})},initView:function(b){try{var c=this.container,d=this.sidebar;"table"==b&&(c.find(".s-select-all:first").click(function(){a(this).trigger("select",this.checked)}),a("#s-content .list-view-mode-table").click(function(){c.toggleClass("single-lined");
a.storage.set("shop/product_list/multi-lined",!c.hasClass("single-lined"));return!1}),a.storage.get("shop/product_list/multi-lined")&&c.removeClass("single-lined"));var e="view="+b;d.find(".s-collection-list li.dr").each(function(){var b=a(this);b.find("a:first").attr("href","#/products/"+b.attr("id").replace("-","_id=")+"&"+e)});a("#s-products-search").autocomplete("disable").val(this.options.text||"").autocomplete("enable");var f="s-all-products";a.product_list.collection_hash.length&&("search"!==
a.product_list.collection_hash[0]&&"tag"!==a.product_list.collection_hash[0])&&(f=a.product_list.collection_hash.join("-"));a.shop.trace("$.product_list.initView",[b,f]);d.find("li.selected").removeClass("selected");var g=d.find("#"+f);0>=g.length&&window.location.hash&&(g=d.find('a[href="'+window.location.hash+'"]').parent());0>=g.length&&(g=d.find("#s-all-products"));g.addClass("selected");a.product_list.collection_hash.length&&("search"!==a.product_list.collection_hash[0]&&"tag"!==a.product_list.collection_hash[0])&&
g.find(".count:first").text(this.category_count);a("#s-content").find(".sort").unbind("click").bind("click",function(){location.href=a(this).find("a").attr("href");return false});a.product_list.fixed_blocks=a.product_list.initFixedBlocks();this.initEditingControls();this.initToolbar();a(document).trigger("product_list_init_view")}catch(h){a.shop.error("$.product_list.initView error: "+h.message,h)}},highlightInlineEditorCells:function(){a.product_list.container.find("tr").each(function(){var b=a(this),
...
"")},3E3);a.wrapInner('<div class="animation-container" style="position:relative;"></div>');var e=a.find(".animation-container").animate({left:"3px"},100,function(){e.animate({left:"0px"},100,function(){e.animate({left:"3px"},100,function(){e.animate({left:"0px"},100,function(){e.children().first().unwrap()})})})})}}a.product_list.highlightInlineEditorCells();a.product_list.container.on("append_product_list",function(){a.product_list.highlightInlineEditorCells()});a.product_list.container.on("click",
".s-product-price,.s-product-stock",function(){function c(){if(f)d.html(tmpl("template-list-price-editor-one",{product_id:e.data("product-id"),currency:e.data("currency"),value:e.data("original-price")}));else{var b=e.data("stocks");!b||"object"!=typeof b?d.html(tmpl("template-list-stock-editor-one",{product_id:e.data("product-id"),value:b||""})):d.html(tmpl("template-list-stock-editor-many",{product_id:e.data("product-id"),values:b}))}d.find(".cancel").click(function(){var a=d;a.removeClass("editor-on").html(a.data("read-only-state-html"));
...
sort:a.product_list.sort}));e.replaceWith(d.find("tr"));self.container.trigger("append_product_list",[b.data])}catch(f){console.log("Error: "+f.message)}},"json");return!1});c.on("dblclick",!1);setTimeout(function(){d.find(":input:visible:first").focus()},0)}var d=a(this);if(!d.data("plugin")){var e=d.closest("tr"),f=d.hasClass("s-product-price");if(f){if(e.data("min-price")!=e.data("max-price"))return b(d,$_("Inline editing is available only for products with a single SKU"))}else if("1"!=e.data("sku-count"))return b(d,
$_("Inline editing is available only for products with a single SKU"));if(!e.data("edit-rights"))return b(d,$_("Insufficient access rights"));if(!d.hasClass("editor-on"))if(d.addClass("editor-on").data("read-only-state-html",d.html()),a.product_list.container.removeClass("single-lined"),void 0!==e.data("original-price"))c();else{d.html('<i class="icon16 loading"></i>');var g=[];a.product_list.container.find("tr.product").each(function(){var b=a(this);void 0===b.data("original-price")&&b.data("edit-rights")&&
g.push(a(this).data("product-id"))});g.length?a.post("?module=products&action=inlineEditorData",{ids:g},function(b){b.data&&a.product_list.container.find("tr.product").each(function(){var c=a(this),d=b.data[parseInt(c.data("product-id"),10)];d&&(c.data("stocks",d.stocks||null),c.data("original-price",d.price.replace(/\.?0+$/g,"")))});c()},"json"):c()}}})},initFixedBlocks:function(){var b=a("#mainmenu").height(),c=a("#s-sidebar").width(),d=a(window),e=a("#s-fixed-blocks");e.length||(e=a('<div id="s-fixed-blocks"></div>').css({display:"none",
position:"fixed",background:"#fff",top:b,width:c,"float":"left"}).appendTo("#s-sidebar"));var f=a("#s-category-list"),g=a("#s-category-list-block"),h=a("#s-set-list"),j=a("#s-set-list-block"),k,m=function(){if(!n){var h=a("#"+o);s=a.categories_tree.isCollapsed(h);q=f.is(":hidden");var j=a('<div id="'+p+'" class="block"></div>').css({height:g.height()});g.replaceWith(j);e.show().append(g.hide());g.find(".heading").before('<span class="hint float-right" style="margin-top: 3px;">'+$_("Drag products here")+
"</span>");s&&a.categories_tree.expand(h,null,function(){a.categories_tree.collapse(h)});q||f.hide();g.show();a("i.add",g).hide();a("#"+o).hide();g.css({"overflow-x":"hidden","overflow-y":"auto"});var k=f.is(":hidden"),i;g.bind("mousemove.fixed_blocks.category",function(){document.getElementById("products-helper")&&k&&(i&&(clearTimeout(i),i=null),i=setTimeout(function(){if(k){g.css({height:""});f.show();var a=g.height(),c=(d.height()-b)/2;g.css({height:Math.min(a,c)});g.find(".heading").prev(".hint").hide();
k=false}},250))});d.bind("mouseout.fixed_blocks.category",function(a){a.pageX>c+10&&(i&&clearTimeout(i),i=null,k||(f.hide(),g.css({height:""}),g.find(".heading").prev(".hint").show(),k=!0))});n=!0}},l=function(){if(n){var b=a("#"+p);b.replaceWith(g);g.css({"overflow-x":"","overflow-y":""});var c=a("#"+o);s&&a.categories_tree.collapse(c);q||f.show();g.find(".heading").prev(".hint").remove();a("#"+o).show();a("i.add",g).show();b.remove();g.unbind("mousemove.fixed_blocks.category");d.unbind("mouseout.fixed_blocks.category");
e.children().length||e.hide();n=!1;a.categories_tree.init()}},i=g.offset().top,p="s-category-list-dummy",o="s-category-list-handler",n=!1,q,s;d.unbind("scroll.fixed_blocks.category").bind("scroll.fixed_blocks.category",function(){n||(i=g.offset().top);var c=i,c=document.getElementById(p)?c+a("#"+p).height():c+a("#s-category-list-block").height();d.scrollTop()+b+5>=c?m():l()});k={setFixed:m,unsetFixed:l};var t=function(){if(!u){r=h.is(":hidden");var f=a('<div id="'+v+'" class="block"></div>').css({height:j.height()});
j.replaceWith(f);e.show().append(j);j.find(".heading").before('<span class="hint float-right" style="margin-top: 3px;">'+$_("Drag products here")+"</span>");r||h.hide();a("i.add",j).hide();a("i.collapse-handler",j).hide();j.css({"overflow-x":"hidden","overflow-y":"auto"});var g=h.is(":hidden"),k;j.bind("mousemove.fixed_blocks.set",function(){document.getElementById("products-helper")&&g&&(k&&(clearTimeout(k),k=null),k=setTimeout(function(){if(g){j.css({height:""});h.show();var a=j.height(),c=(d.height()-
b)/2;j.css({height:Math.min(a,c)});j.find(".heading").prev(".hint").hide();g=false}},250))});d.bind("mouseout.fixed_blocks.set",function(a){a.pageX>c+10&&(k&&clearTimeout(k),k=null,g||(h.hide(),j.css({height:""}),j.find(".heading").prev(".hint").show(),g=!0))});u=!0}},w=function(){if(u){var b=a("#"+v);b.replaceWith(j);j.css({"overflow-x":"","overflow-y":""});j.find(".heading").prev(".hint").remove();r||h.show();a("i.add",j).show();a("i.collapse-handler",j).show();b.remove();j.unbind("mousemove.fixed_blocks.category");
d.unbind("mouseout.fixed_blocks.category");e.children().length||e.hide();u=!1}},x=j.offset().top,v="s-set-list-dummy",r,u=!1;d.unbind("scroll.fixed_blocks.set").bind("scroll.fixed_blocks.set",function(){u||(x=j.offset().top);var c=x,c=document.getElementById(v)?c+a("#"+v).height():c+a("#s-set-list-block").height();d.scrollTop()+b+5>=c?t():w()});return{category:k,set:{setFixed:t,unsetFixed:w}}},initEditingControls:function(){var b=a("#s-product-list-title");b.hasClass("editable")&&(b.inlineEditable({minSize:{width:350},
...
b.trigger("editable"));var c=a.product_list.collection_hash[0],b=a("#s-product-list-settings");b.length&&("category"==c||"set"==c)&&b.unbind("click").bind("click",function(){a.product_list.settingsListDialog();return false});b=a("#s-product-list-delete");b.length&&("category"==c||"set"==c)&&b.unbind("click").bind("click",function(){a("#s-product-list-delete-dialog").waDialog({disableButtonsOnSubmit:true,onLoad:function(){a(this).find(".dialog-buttons i.loading").hide()},onSubmit:function(){function b(a,
c){if(!a.length)return c&&c();var f=a.shift();e(["category",f],function(){b(a,c)})}function e(b,c){var d={hash:b.join("/")||"all",remove:["list"]};h&&d.remove.push("products");var e=function(){a.product_list.remove(d,function(d){var e=b[0],f=a("#"+b.join("-")),g=f.prev(".drag-newposition"),h=a("#s-"+b[0]+"-list"),i=h.find("ul:first");if(e=="set"){f.remove();g.remove()}else{e=f.find("ul:first");if(e.length){e.find(">li.drag-newposition:first").remove();f.hide();e.find(">li").each(function(){f.after(this)})}e=
f.parents("ul:first");f.remove();g.remove();if(!e.find("li.dr").length)if(i.get(0)!=e.get(0)){e.parents("li:first").find("i.collapse-handler").remove();e.closest("li").find(".collapse-handler-ajax:first").remove();e.remove()}else i.closest(".block").find(".collapse-handler-ajax:first").remove()}if(i.find("li.dr:first").length){i.show();h.find(".s-empty-list").hide()}else{i.hide();h.find(".s-empty-list").show()}d.data.old_parent_category&&(typeof d.data.old_parent_category.children_count!=="undefined"&&
!parseInt(d.data.old_parent_category.children_count,10))&&a("#category-"+d.data.old_parent_category.id+"-handler").remove();a.product_list.sidebar.trigger("update",d.data.lists);c&&c()})},f=a("#"+b.join("-")+"-handler");f.length?a.categories_tree.expand(f,function(a){a.parent().find("ul:first").hide();e()}):e();return false}var f=a(this),g=parseInt(a.product_list.collection_hash[1]);f.find(".dialog-buttons i.loading").show();var h=f.find("input[name=s-delete-products]:checked").val()=="1";c=="category"&&
f.find('input[name="s-delete-sub"]').is(":checked")?a.get("?action=categoryExpand&tree=1&recurse=1&id="+g,function(c){c=a("<div>").html(c).find('li[id^="category-"]').map(function(){return parseInt(this.id.substr(9),10)}).get().reverse();c.push(g);b(c,function(){location.href="#/"})}):e(a.product_list.collection_hash,function(){location.href="#/"});return false}});return false});"set"===c&&a("#s-embed-code").click(function(){a("#s-product-list-embed-dialog").waDialog();return false})},initDragndrop:function(){a.product_dragndrop.init({products:!0,
...
d,f=a(document),g=a(window);f.bind("scroll",c=function(){jQuery.contains(document.documentElement,b[0])?f.scrollTop()>e&&b.height()+d<g.height()?b.addClass("s-fixed").css({top:d}):b.removeClass("s-fixed"):(f.off("scroll",c),g.off("resize",c))});g.bind("resize",c)},getSelectedProducts:function(b){var b=b||!1,c={count:0};if(this.container.find(".s-select-all:first").attr("checked")){var d=a.product_list.collection_hash.join("/")||"all";b?c.serialized=[{name:"hash",value:d}]:c.hash=d;c.count=this.total_count}else b?
...
if(g.length){var h=jQuery.Event("keypress",{which:13});g.trigger(h)}d.find(".dialog-buttons i.loading").show();var j=d.attr("action");setTimeout(function(){var g=d.serializeArray().concat(b.serialized);a.shop.jsonPost(j,g,function(b){b.data.cloud&&a("#s-tag-cloud").trigger("update",[b.data.cloud]);c.trigger("close");a.products.dispatch()},function(){c.trigger("close")})},10);return!1}})})},typesDialog:function(b){var c=a("#s-types"),d=this.container,e=this.sidebar,f=function(){a("#s-types").waDialog({disableButtonsOnSubmit:!0,
...
b.count,function(){a("#s-product-list-delete-products-dialog").waDialog({disableButtonsOnSubmit:!0,onLoad:function(){a(this).find(".dialog-buttons i.loading").hide()},onSubmit:function(c){a(this).find(".dialog-buttons i.loading").show();a.product_list.remove(a.extend(b,{remove:["products"]}),function(b,d){a.product_list.sidebar.trigger("update",b.data.lists);a.products.dispatch();c.trigger("close");d.length&&alert((c.data("not-allowed-string")||"").replace("%d",d.length))});return!1}})})},settingsListDialog:function(){var b=
...
50)},onSubmit:function(b){var c=a(this),f=function(c){var e=a.product_list.collection_hash,f=location.hash.replace(e[0]+"_id="+e[1],e[0]+"_id="+c.data.id),g=a("#"+e.join("-"));g.find(".name:first").html(c.data.name);a.isArray(c.data.routes)&&c.data.routes.length?g.find(".routes:first").html(" "+c.data.routes.join(" ")):g.find(".routes:first").html(" ");c.data.status=="0"?g.children("a").addClass("gray"):c.data.status=="1"&&g.children("a").removeClass("gray");g.find(".id:first").html(c.data.id);g.attr("id",
e[0]+"-"+c.data.id);g.find("a").attr("href",f);if("sort_products"in c.data){f=f.replace(/&sort=[^&]*/,"&sort=");f=f.replace(/&order=[^&]*/,"&order=")}location.hash!=f?location.hash=f:a.products.dispatch();b.trigger("close")},g=function(a){if(a&&a.errors){var a=a.errors,c;for(c in a)b.find("input[name="+c+"]").addClass("error").parent().find(".errormsg").text(a[c]);return false}};a("#s-category-description-content").length&&a("#s-category-description-content").waEditor("sync");if(c.find("input:file").length)a.products._iframePost(c,
...
a(this),d=a(this).data("product-id");d&&b[d]&&c.addClass("highlighted")})});a.products.dispatch()},error:function(){}}))},setBadge:function(b,c){function d(b){var d="delete-badge"==e?null:b.data;a("#product-list [data-product-id]").each(function(){var b=a(this);if(g||f[b.data("product-id")]){if(b.is("tr")){var c=b.find(".s-image a");c.find(".s-image-corner").remove();d&&c.prepend(a('<div class="s-image-corner"></div>').html(d))}else c=b.find(".s-product-image a"),c.find(".s-image-corner.top.right").remove(),
d&&c.append(a('<div class="s-image-corner top right"></div>').html(d));b.trigger("badge",[d])}});"set-custom-badge"==e&&c.parent().find(".textarea-wrapper").slideUp("fast");c.find(".loading").remove();c.find("a").append(a('<span class="count"><i class="icon16 yes"></i></span>').animate({opacity:0},function(){a(this).remove()}));a.product_list.container.find(".s-select-all:first").trigger("select",!1)}var e=c.data("action");"set-custom-badge"!=e&&c.parent().find(".textarea-wrapper").slideUp("fast");
c.find(".loading").remove();c.find("a").append('<span class="count"><i class="icon16 loading"></i></span>');var f={},g=!1;a.each(b.serialized,function(){g=g||"hash"==this.name;f[this.value]=1});if("delete-badge"==e)c.find(".loading").remove(),a.shop.jsonPost("?module=product&action=badgeDelete",b.serialized,d);else{var h;h="set-custom-badge"==e?c.find("textarea").val():c.data("type");var j=b.serialized;j.push({name:"code",value:h});a.shop.jsonPost("?module=product&action=badgeSet",j,d)}},duplicate:function(b,
c){var d=this,e=[];a.shop.jsonPost("?module=products&action=duplicate",{hash:b,limit:50,offset:c.offset||0},function(a){if("ok"==(a.status||"error"))e=e.concat(a.data.new_ids||[]),a.data.offset<a.data.total_count?(c.offset=a.data.offset,d.duplicate(b,c),c.progress(a.data||{})):c.finish(a.data||{},e)})},remove:function(b,c){var d={},e=[],f;b.product_id?f=function(){100>=b.product_id.length&&(d.get_lists=!0);d.product_id=b.product_id.splice(0,100);a.shop.jsonPost("?module=products&action=deleteList",
d,function(a){a.data.not_allowed&&a.data.not_allowed.length&&(e=e.concat(a.data.not_allowed));b.product_id.length?f():typeof c==="function"&&c(a,e)})}:(d.hash=b.hash||this.collection_hash.join("/")||"all",d.remove=a.isArray(b.remove)&&b.remove.length?b.remove:["list"],1==d.remove.length&&"list"==d.remove[0]?f=function(){a.shop.jsonPost("?module=products&action=deleteList",d,c)}:(d.count=100,f=function(){a.shop.jsonPost("?module=products&action=deleteList",d,function(a){a.data.not_allowed&&a.data.not_allowed.length&&
(e=e.concat(a.data.not_allowed));0<a.data.rest_count&&null!=a.data.rest_count?f():"function"===typeof c&&c(a,e)})}));f()},rubberTable:function(){var b=a("#wa"),c=a("#s-content"),d=a("#s-content > .content"),c=parseInt(c.css("margin-left")),e=parseInt(d.css("margin-right")),f=a("#s-product-list-table-container table").width(),g=b.attr("style");g&&b.data("style",g);d=parseInt(d.width()-a("#s-product-list-table-container").width())+f+c+e;b.width()<d&&b.css({"min-width":d+"px"});a(window).resize()}}})(jQuery);
