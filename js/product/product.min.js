...
0,d;for(d in a)a.hasOwnProperty(d)&&c++;return c}},multiSkus:function(b){var c=a("#s-product-edit-forms .s-product-form.main table.s-product-skus:first");1<b?(c.find("thead tr th.s-sku-sort").show(),c.find(".s-name,.s-sku-sort").show("slow"),c.find(".delete").parent("a").show(),2==b?c.find("> tbody:first > tr:first .s-name :input").focus():c.find("> tbody:first > tr:last .s-name :input").focus()):(c.find("thead tr th.s-sku-sort").hide(),c.find(".s-name,.s-sku-sort").hide(),c.find(".delete").parent("a").hide())},
editTabMainSkuAdd:function(){try{var b=a("#s-product-edit-forms .s-product-form.main table.s-product-skus:first").find("tbody:first"),c=0,d="0";b.find(':input[name$="[price]"]').each(function(){var b=a(this),e=b.val()?b.val().replace(",","."):0,e=parseFloat(e);e>c&&(c=e,d=b.val()||"0")});var e=0;b.find(':input[name$="[purchase_price]"]').each(function(){e=Math.max(e,parseFloat(a(this).val())||0)});a.product.multiSkus(b.find("tr:not(.js-sku-settings)").length+1);var f={id:--a.product.editTabMainData.sku_id,
...
data:{sku_id:b,product_id:this.path.id},dataType:"json",type:"post",success:function(c){"fail"==c.status?f.refresh("error",c.errors):c.data.redirect?window.location.href=c.data.redirect:(f.refresh("success",c.data.message||"Success"),e.hide("normal",function(){e.remove();a('#s-product-view table.s-product-skus > tbody > tr[data-id="'+b+'"]').remove();a.product.multiSkus(d.find("tr:not(.js-sku-settings)").length)}),a("#s-product-edit-forms .s-product-form.main").find("input[name=product\\[sku_id\\]][value="+
c.data.sku_id+"]").attr("checked",!0))},error:function(a,b){f.refresh("error",b)}}):f.refresh("error",$_("A product must have at least one SKU.")):e.hide("normal",function(){e.remove();a('#s-product-view table.s-product-skus > tbody >tr[data-id="'+b+'"]').remove();a.product.multiSkus(d.find("tr:not(.js-sku-settings)").length)})},editTabMainCategoriesAdd:function(){var b=a("#s-product-edit-forms .s-product-form.main select.s-product-categories:last").parent("div"),c=b.clone(!1);c.find("select").val("select").attr("disabled",
