...
b.shop.jsonPost(k,e,function(a){a.data.denied_message&&alert(a.data.denied_message);"search"===a.data.cloud?c.trigger("close"):a.data.cloud&&b("#s-tag-cloud").trigger("update",[a.data.cloud]);c.trigger("close");b.products.dispatch()},function(){c.trigger("close")})},10);return!1}})};c.length&&c.remove();b.post("?module=dialog&action=assignTags",a.serialized,function(a){b("body").append(a);d()})},typesDialog:function(a){var c=b("#s-types"),d=this.container,e=this.sidebar,f=function(){b("#s-types").waDialog({disableButtonsOnSubmit:!0,
onLoad:function(){b(this).find(".dialog-buttons i.loading").hide()},onSubmit:function(c){var f=b(this);f.find(".dialog-buttons i.loading").show();b.shop.jsonPost(f.attr("action"),f.serializeArray().concat(a.serialized),function(a){e.trigger("update",{type:a.data.types});d.find(".s-select-all:first").trigger("select",!1);f.find("input:checked").attr("checked",!1);c.trigger("close");b.products.dispatch()});return!1}})},g=c.parent();c.length?f():(g=b("<div></div>").appendTo("body"),g.load("?module=dialog&action=types",
f))},deleteFromSet:function(a){0<a.count&&b.shop.jsonPost("?module=products&action=deleteFromSet&id="+this.collection_hash[1],a.serialized,function(){b.products.dispatch()})},deleteFromCategory:function(a){0<a.count&&b.shop.jsonPost("?module=products&action=deleteFromCategory&id="+this.collection_hash[1],a.serialized,function(){b.products.dispatch()})},deleteProductsDialog:function(a){var c=b("#s-product-list-delete-products-dialog"),d=c.parent();c.length||(d=b("<div></div>").appendTo("body"));d.load("?module=dialog&action=productsDelete&count="+
a.count,function(){b("#s-product-list-delete-products-dialog").waDialog({disableButtonsOnSubmit:!0,onLoad:function(){b(this).find(".dialog-buttons i.loading").hide()},onSubmit:function(c){b(this).find(".dialog-buttons i.loading").show();b.product_list.remove(b.extend(a,{remove:["products"]}),function(a,d){b.product_list.sidebar.trigger("update",a.data.lists);b.products.dispatch();c.trigger("close");d.length&&alert((c.data("not-allowed-string")||"").replace("%d",d.length))});return!1}})})},exportProducts:function(a,
b){for(var c=[],e,f=!1;e=a.serialized.pop();)e.name&&("product_id[]"==e.name?c.push(parseInt(e.value,10)):"hash"==e.name&&(f=e.value));a="?action=importexport#/"+b;f?window.location=a+("/hash/"+f):c.length&&(a+="/hash/id/"+c.join(","),window.location=a)},createCoupon:function(a){for(var b=[],d,e=!1;d=a.serialized.pop();)d.name&&("product_id[]"==d.name?b.push(parseInt(d.value,10)):"hash"==d.name&&(e=d.value));a="?action=orders#/coupons/";e?window.location=a+("hash/"+e):b.length&&(a+="hash/id/"+b.join(","),
window.location=a)},duplicateProducts:function(a,c){for(var d=[],e,f=!1;e=a.serialized.pop();)e.name&&("product_id[]"==e.name?d.push(parseInt(e.value,10)):"hash"==e.name&&(f=e.value));!f&&d.length&&(f="id/"+d.join(","));f&&(c.find("i.icon16").removeClass("split").addClass("loading"),this.duplicate(f,{progress:function(a){c.attr("title",Math.round(100*a.offset/a.total_count)+"%")},finish:function(a,d){c.attr("title",null);var e=c.find("i.icon16");e.removeClass("loading").addClass("yes");setTimeout(function(){e.removeClass("yes").addClass("split")},
3E3);b(document).one("product_list_init_view",function(){var a={};b.each(d,function(b,c){a[c]=1});b("#product-list [data-product-id]").each(function(){var c=b(this),d=b(this).data("product-id");d&&a[d]&&c.addClass("highlighted")})});b.products.dispatch()},error:function(a){}}))},setBadge:function(a,c){function d(a){var d="delete-badge"==e?null:a.data;b("#product-list [data-product-id]").each(function(){var a=b(this);if(g||f[a.data("product-id")]){if(a.is("tr")){var c=a.find(".s-image a");c.find(".s-image-corner").remove();
d&&c.prepend(b('<div class="s-image-corner"></div>').html(d))}else c=a.find(".s-product-image a"),c.find(".s-image-corner.top.right").remove(),d&&c.append(b('<div class="s-image-corner top right"></div>').html(d));a.trigger("badge",[d])}});"set-custom-badge"==e&&c.parent().find(".textarea-wrapper").slideUp("fast");c.find(".loading").remove();c.find("a").append(b('<span class="count"><i class="icon16 yes"></i></span>').animate({opacity:0},function(){b(this).remove()}));b.product_list.container.find(".s-select-all:first").trigger("select",
!1)}var e=c.data("action");"set-custom-badge"!=e&&c.parent().find(".textarea-wrapper").slideUp("fast");c.find(".loading").remove();c.find("a").append('<span class="count"><i class="icon16 loading"></i></span>');var f={},g=!1;b.each(a.serialized,function(){g=g||"hash"==this.name;f[this.value]=1});if("delete-badge"==e)c.find(".loading").remove(),b.shop.jsonPost("?module=product&action=badgeDelete",a.serialized,d);else{var h="set-custom-badge"==e?c.find("textarea").val():c.data("type");a=a.serialized;
a.push({name:"code",value:h});b.shop.jsonPost("?module=product&action=badgeSet",a,d,function(a){"object"===typeof a.errors&&"string"===typeof a.errors[0]&&alert(a.errors[0]);c.find(".loading").remove()})}},duplicate:function(a,c){var d=this,e=[];b.shop.jsonPost("?module=products&action=duplicate",{hash:a,limit:50,offset:c.offset||0},function(b){"ok"==(b.status||"error")&&(e=e.concat(b.data.new_ids||[]),b.data.offset<b.data.total_count?(c.offset=b.data.offset,d.duplicate(a,c),c.progress(b.data||{})):
c.finish(b.data||{},e))},function(a){a.errors&&alert(a.errors)})},remove:function(a,c){var d={},e=[];if(a.product_id)var f=function(){100>=a.product_id.length&&(d.get_lists=!0);d.product_id=a.product_id.splice(0,100);b.shop.jsonPost("?module=products&action=deleteList",d,function(b){b.data.not_allowed&&b.data.not_allowed.length&&(e=e.concat(b.data.not_allowed));a.product_id.length?f():"function"===typeof c&&c(b,e)})};else d.hash=a.hash||this.collection_hash.join("/")||"all",d.remove=b.isArray(a.remove)&&
a.remove.length?a.remove:["list"],1==d.remove.length&&"list"==d.remove[0]?f=function(){b.shop.jsonPost("?module=products&action=deleteList",d,c)}:(d.count=100,f=function(){b.shop.jsonPost("?module=products&action=deleteList",d,function(a){a.data.not_allowed&&a.data.not_allowed.length&&(e=e.concat(a.data.not_allowed));0<a.data.rest_count&&null!=a.data.rest_count?f():"function"===typeof c&&c(a,e)})});f()},rubberTable:function(){var a=b("#wa"),c=b("#s-content"),d=b("#s-content > .content");c=parseInt(c.css("margin-left"));
var e=parseInt(d.css("margin-right")),f=b(".s-product-list-table-container table").width(),g=a.attr("style");g&&a.data("style",g);d=parseInt(d.width()-b(".s-product-list-table-container").width())+f+c+e;a.width()<d&&a.css({"min-width":d+"px"});b(window).resize()}}})(jQuery);var shopDialogProductsSet=function(b){shopDialogProductsSet=function(a){this.$wrapper=a.$wrapper;this.is_new=a.is_new;this.initClass()};shopDialogProductsSet.prototype.initClass=function(){this.is_new&&this.initTransliterateSetID();this.initTypeSwitch()};shopDialogProductsSet.prototype.initTypeSwitch=function(){var a=this.$wrapper;a.find("input[name=type]").on("click",function(){"0"==b(this).val()?a.find(".js-set-dynamic-settings").hide():a.find(".js-set-dynamic-settings").show()})};shopDialogProductsSet.prototype.initTransliterateSetID=
