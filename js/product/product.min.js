...
"&sort="+e.sort:"")+(e.order?"&order="+e.order:"")+("&enabled_columns="+(f||"")),function(b){if("ok"==b.status&&b.data.count){c+=b.data.count;var f=e.container;try{e.container.append(tmpl("template-product-list-"+e.options.view,{products:b.data.products,check_all:"table"==e.options.view?f.find(".s-select-all:first").attr("checked"):!1,sort:a.product_list.sort})),e.container.trigger("append_product_list",[b.data.products])}catch(j){console.log("Error: "+j.message);a(window).lazyLoad("stop");return}f.find(".last").removeClass("last");
...
function(){location.href=a(this).find("a").attr("href");return false});a.product_list.fixed_blocks=a.product_list.initFixedBlocks();this.initEditingControls();this.initToolbar();a(document).trigger("product_list_init_view")}catch(h){a.shop.error("$.product_list.initView error: "+h.message,h)}},initFixedBlocks:function(){var b=a("#mainmenu").height(),c=a("#s-sidebar").width(),d=a(window),e=a("#s-fixed-blocks");e.length||(e=a('<div id="s-fixed-blocks"></div>').css({display:"none",position:"fixed",background:"#fff",
...
null,function(){a.categories_tree.collapse(h)});q||f.hide();g.show();a("i.add",g).hide();a("#"+o).hide();g.css({"overflow-x":"hidden","overflow-y":"auto"});var k=f.is(":hidden"),i;g.bind("mousemove.fixed_blocks.category",function(){document.getElementById("products-helper")&&k&&(i&&(clearTimeout(i),i=null),i=setTimeout(function(){if(k){g.css({height:""});f.show();var a=g.height(),c=(d.height()-b)/2;g.css({height:Math.min(a,c)});g.find(".heading").prev(".hint").hide();k=false}},250))});d.bind("mouseout.fixed_blocks.category",
function(a){a.pageX>c+10&&(i&&clearTimeout(i),i=null,k||(f.hide(),g.css({height:""}),g.find(".heading").prev(".hint").show(),k=!0))});n=!0}},l=function(){if(n){var b=a("#"+p);b.replaceWith(g);g.css({"overflow-x":"","overflow-y":""});var c=a("#"+o);s&&a.categories_tree.collapse(c);q||f.show();g.find(".heading").prev(".hint").remove();a("#"+o).show();a("i.add",g).show();b.remove();g.unbind("mousemove.fixed_blocks.category");d.unbind("mouseout.fixed_blocks.category");e.children().length||e.hide();n=
!1;a.categories_tree.init()}},i=g.offset().top,p="s-category-list-dummy",o="s-category-list-handler",n=!1,q,s;d.unbind("scroll.fixed_blocks.category").bind("scroll.fixed_blocks.category",function(){n||(i=g.offset().top);var c=i,c=document.getElementById(p)?c+a("#"+p).height():c+a("#s-category-list-block").height();d.scrollTop()+b+5>=c?m():l()});k={setFixed:m,unsetFixed:l};var t=function(){if(!u){r=h.is(":hidden");var f=a('<div id="'+v+'" class="block"></div>').css({height:j.height()});j.replaceWith(f);
e.show().append(j);j.find(".heading").before('<span class="hint float-right" style="margin-top: 3px;">'+$_("Drag products here")+"</span>");r||h.hide();a("i.add",j).hide();a("i.collapse-handler",j).hide();j.css({"overflow-x":"hidden","overflow-y":"auto"});var g=h.is(":hidden"),k;j.bind("mousemove.fixed_blocks.set",function(){document.getElementById("products-helper")&&g&&(k&&(clearTimeout(k),k=null),k=setTimeout(function(){if(g){j.css({height:""});h.show();var a=j.height(),c=(d.height()-b)/2;j.css({height:Math.min(a,
c)});j.find(".heading").prev(".hint").hide();g=false}},250))});d.bind("mouseout.fixed_blocks.set",function(a){a.pageX>c+10&&(k&&clearTimeout(k),k=null,g||(h.hide(),j.css({height:""}),j.find(".heading").prev(".hint").show(),g=!0))});u=!0}},w=function(){if(u){var b=a("#"+v);b.replaceWith(j);j.css({"overflow-x":"","overflow-y":""});j.find(".heading").prev(".hint").remove();r||h.show();a("i.add",j).show();a("i.collapse-handler",j).show();b.remove();j.unbind("mousemove.fixed_blocks.category");d.unbind("mouseout.fixed_blocks.category");
e.children().length||e.hide();u=!1}},x=j.offset().top,v="s-set-list-dummy",r,u=!1;d.unbind("scroll.fixed_blocks.set").bind("scroll.fixed_blocks.set",function(){u||(x=j.offset().top);var c=x,c=document.getElementById(v)?c+a("#"+v).height():c+a("#s-set-list-block").height();d.scrollTop()+b+5>=c?t():w()});return{category:k,set:{setFixed:t,unsetFixed:w}}},initEditingControls:function(){var b=a("#s-product-list-title");b.hasClass("editable")&&(b.inlineEditable({minSize:{width:350},maxSize:{width:600},
size:{height:30},inputClass:"s-title-h1-edit",beforeMakeEditable:function(){a(".s-product-list-manage").hide()},afterBackReadable:function(b,c){a(".s-product-list-manage").show();if(!c.changed)return!1;a.shop.jsonPost("?module=products&action=saveListSettings&"+a.product_list.collection_param+"&edit=name",{name:a(b).val()},function(b){a.product_list.sidebar.find("#"+a.product_list.collection_hash[0]+"-"+b.data.id).find(".name:first").html(b.data.name)})}}),"name"==this.options.edit&&b.trigger("editable"));
var c=a.product_list.collection_hash[0],b=a("#s-product-list-settings");b.length&&("category"==c||"set"==c)&&b.unbind("click").bind("click",function(){a.product_list.settingsListDialog();return false});b=a("#s-product-list-delete");b.length&&("category"==c||"set"==c)&&b.unbind("click").bind("click",function(){a("#s-product-list-delete-dialog").waDialog({disableButtonsOnSubmit:true,onLoad:function(){a(this).find(".dialog-buttons i.loading").hide()},onSubmit:function(){function b(a,c){if(!a.length)return c&&
c();var f=a.shift();e(["category",f],function(){b(a,c)})}function e(b,c){var d={hash:b.join("/")||"all",remove:["list"]};h&&d.remove.push("products");var e=function(){a.product_list.remove(d,function(d){var e=b[0],f=a("#"+b.join("-")),g=f.prev(".drag-newposition"),h=a("#s-"+b[0]+"-list"),i=h.find("ul:first");if(e=="set"){f.remove();g.remove()}else{e=f.find("ul:first");if(e.length){e.find(">li.drag-newposition:first").remove();f.hide();e.find(">li").each(function(){f.after(this)})}e=f.parents("ul:first");
f.remove();g.remove();if(!e.find("li.dr").length)if(i.get(0)!=e.get(0)){e.parents("li:first").find("i.collapse-handler").remove();e.closest("li").find(".collapse-handler-ajax:first").remove();e.remove()}else i.closest(".block").find(".collapse-handler-ajax:first").remove()}if(i.find("li.dr:first").length){i.show();h.find(".s-empty-list").hide()}else{i.hide();h.find(".s-empty-list").show()}d.data.old_parent_category&&(typeof d.data.old_parent_category.children_count!=="undefined"&&!parseInt(d.data.old_parent_category.children_count,
10))&&a("#category-"+d.data.old_parent_category.id+"-handler").remove();a.product_list.sidebar.trigger("update",d.data.lists);c&&c()})},f=a("#"+b.join("-")+"-handler");f.length?a.categories_tree.expand(f,function(a){a.parent().find("ul:first").hide();e()}):e();return false}var f=a(this),g=parseInt(a.product_list.collection_hash[1]);f.find(".dialog-buttons i.loading").show();var h=f.find("input[name=s-delete-products]:checked").val()=="1";c=="category"&&f.find('input[name="s-delete-sub"]').is(":checked")?
...
b.error)})},initToolbar:function(){var b=this.toolbar;b.find("li").unbind("click.product_list").bind("click.product_list",function(){var b=a(this).attr("data-action");if(b){var c=a.product_list.getSelectedProducts("delete"!=b);if(!c.count)return alert($_("Please select at least one product")),!1;switch(b){case "new-set":break;case "category":a.product_list.categoriesDialog(c);break;case "set":a.product_list.setsDialog(c);break;case "type":a.product_list.typesDialog(c);break;case "assign-tags":a.product_list.assignTagsDialog(c);
break;case "delete":a.product_list.deleteProductsDialog(c);break;case "delete-from-set":"set"==a.product_list.collection_hash[0]&&a.product_list.deleteFromSet(c);break;case "delete-from-category":"category"==a.product_list.collection_hash[0]&&a.product_list.deleteFromCategory(c);break;case "export":a.product_list.exportProducts(c,a(this).attr("data-plugin"));break;case "duplicate":a.product_list.duplicateProducts(c,a(this));break;case "set-badge":case "delete-badge":a.product_list.setBadge(c,a(this));
break;default:return}return!1}});b.find('li[data-action="set-custom-badge"] a').click(function(){a(this).parent().find(".textarea-wrapper").slideToggle("fast");return!1}).closest("li").find(".textarea-wrapper").click(function(){return!1}).find("input").click(function(){var b=a.product_list.getSelectedProducts(!0);b.count?a.product_list.setBadge(b,a(this).closest("li")):alert($_("Please select at least one product"));return!1});var c,d=a("#mainmenu").height(),e=b.find(":first").offset().top-d,f=a(document),
g=a(window);f.bind("scroll",c=function(){jQuery.contains(document.documentElement,b[0])?f.scrollTop()>e&&b.height()+d<g.height()?b.addClass("s-fixed").css({top:d}):b.removeClass("s-fixed"):(f.off("scroll",c),g.off("resize",c))});g.bind("resize",c)},getSelectedProducts:function(b){var b=b||!1,c={count:0};if(this.container.find(".s-select-all:first").attr("checked")){var d=a.product_list.collection_hash.join("/")||"all";b?c.serialized=[{name:"hash",value:d}]:c.hash=d;c.count=this.total_count}else b?
