...
this.id+'-hint">Ctrl+Enter</em>'),a("#"+d).click(function(){c.trigger("readable")}));a("#"+this.id+"-hint").show();e.show()},afterBackReadable:function(b,c){var d=a(this);d=parseInt(d.parents("li:first").attr("data-image-id"),10);b=a(b).val();var e="#"+this.id+"-";a(e+"button").hide();a(e+"hint").hide();c.changed&&a.products.jsonPost("?module=product&action=imageSave",{id:d,data:{description:b}})}}).trigger("editable")})}}})(jQuery);(function(a){a.product_stocks={options:{},container:null,products_ids:[],stock_id:null,waLoading:a.waLoading(),init:function(b){this.options=b;this.stocks=b.stocks;this.stock_id=b.stock_id;this.container=a("#s-product-stocks");this.products_ids=[];this.initView();this.options.lazy_loading&&0<this.options.product_stocks.length&&this.initLazyLoad(this.options.lazy_loading);this.initFixRetrespectiveTable(this.container.closest(".s-table-scrollable-x"))},initLazyLoad:function(b){var c=b.count,d=b.total_count,
e=b.sort;a(window).lazyLoad("stop");if(c<d){var f=this;a(window).lazyLoad({container:f.container.find("tbody"),state:"undefined"===typeof b.auto||b.auto?"wake":"stop",hash:"stocks",load:function(){a(window).lazyLoad("sleep");a(".lazyloading-link").hide();a(".lazyloading-progress").show();var g=function(k){console&&(k&&k.errors?console.error("Error when loading products: "+k.errors):k&&k.responseText?console.error("Error when loading products: "+k.responseText):console.error(k));a(window).lazyLoad("stop")},
h=["module=stocksBalance","offset="+c,"total_count="+d];f.stock_id&&h.push("stock_id="+f.stock_id);f.options.order&&h.push("order="+f.options.order);e&&h.push("sort="+e);a.get("?"+h.join("&"),function(k){k&&"ok"===k.status?(c+=k.data.count,f.append({product_stocks:k.data.product_stocks,stocks:f.stocks,is_single_stock:!!f.stock_id})?(a(".lazyloading-progress-string").text(k.data.progress.loaded+" "+k.data.progress.of),a(".lazyloading-progress").hide(),a(".lazyloading-chunk").text(k.data.progress.chunk),
c>=d||1>k.data.count?(a(window).lazyLoad("stop"),a(".lazyloading-link").hide()):(a(".lazyloading-link").show(),a(window).lazyLoad("wake"))):a(window).lazyLoad("stop")):g(k)},"json").error(g)}});a(".lazyloading-link").off("click").on("click",function(){a(window).lazyLoad("force");return!1})}},initView:function(){if(!this.append({product_stocks:this.options.product_stocks,stocks:this.stocks,is_single_stock:!!this.stock_id}))return this},append:function(b){var c=this;b=function(d){var e=[];a.each(d.product_stocks,
function(f,g){0>c.products_ids.indexOf(g.id)&&(e.push(g),c.products_ids.push(g.id))});d.product_stocks=e;return d}(b);try{this.container.find("tbody").append(tmpl("template-product-stocks",b))}catch(d){return console.error("Error: "+d.message),!1}return!0},showLoading:function(){this.waLoading.show();this.waLoading.animate(1E4,95,!1)},doneLoading:function(){this.waLoading.done()},initFixRetrespectiveTable:function(b){if(-1!==navigator.userAgent.indexOf("AppleWebKit")){b.css("perspective","0");var c=
null,d=function(){c&&clearTimeout(c);c=setTimeout(function(){b.css("perspective","0")},50)};b.on("scroll",function(){b.css("perspective","none");d()})}}}})(jQuery);(function(a){a.product_stocks_log={product_id:null,container:null,options:{},init:function(b){this.options=b;this.container=b.container?a(b.container):a("body");this.options.lazy_loading&&this.initLazyLoad(this.options.lazy_loading)},initLazyLoad:function(b){var c=b.count,d=this.options.total_count,e=b.url;a(window).lazyLoad("stop");if(c<d){var f=this;a(window).lazyLoad({container:f.container,state:"undefined"===typeof b.auto||b.auto?"wake":"stop",load:function(){if(f.container.is(":visible"))a(window).lazyLoad("sleep"),
