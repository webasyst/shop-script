...
j.length;g.css("width",m);"prepend"==b.type&&(g.is(":animated")&&g.stop(!1,!0),g.css("left",parseInt(g.css("left"))-h));g.find("li.selected").hasClass("visible")&&e.trigger("home",[null,!1])});e.unbind("forward backward").bind("forward backward",function(a,d){b({direction:a.type,steps:d.steps,animate:"undefined"!==typeof d.animate?d.animate:!0,fn:d.fn});var c=g.find("li.selected"),e="forward"==a.type?c.next("li:not(.dummy)"):c.prev("li:not(.dummy)");e.length&&(c.removeClass("selected"),e.addClass("selected"))});
e.unbind("home").bind("home",function(d,c,e){var d=parseInt(h.length/2),d=h.filter(":eq("+d+")"),g=d.nextAll(".selected:first"),j=d.prevAll(".selected:first"),i=0,m="";"function"!==typeof c&&(e=c);e="undefined"!==typeof e?e:!0;g.length?(m="forward",d.nextAll().each(function(){++i;if(a(this).hasClass("selected"))return!1})):j.length&&(m="backward",d.prevAll().each(function(){++i;if(a(this).hasClass("selected"))return!1}));m&&i?b({direction:m,steps:i,fn:c,animate:e}):"function"==typeof c&&c();return!1});
...
d){b=a(b);b.hasClass("darr")?f(b,d):"function"===typeof d&&d(b)},expand:function(b,d){b=a(b);b.hasClass("rarr")?g(b,d):"function"===typeof d&&d(b)},setExpanded:function(b){a.get("?action=categoryExpand&id="+b)},setCollapsed:function(b){a.get("?action=categoryExpand&id="+b+"&collapsed=1")},getHandlerByCategoryId:function(b){var d=a();return d=(b=parseInt(b,10)||0)?a("#category-"+b+"-handler"):a("#s-category-list-handler")}}})(jQuery);(function(a){Array.prototype.filter||(Array.prototype.filter=function(a,c){var d=this.length;if("function"!=typeof a)throw new TypeError;for(var e=[],f=0;f<d;f++)if(f in this){var g=this[f];a.call(c,g,f,this)&&e.push(g)}return e});a.storage=new a.store;a.products={hash:"",list_hash:"",list_params:{},options:{view:"thumbs"},random:"",init:function(b){a.extend(this.options,b);this.initRouting();this.initSearch();this.initSidebar();a.categories_tree.init();this.initCollapsible()},data:{prev_action:null},
...
false){var f=b.id.replace(/\-handler$/,""),f=a(b).parents("div.block:first").find("#"+f+":first");if(f.length)(f=f.data("on-collapse-url"))&&a.get(f)}}},c=function(b){b=a(b).parent();return b.is("li")?b.find("ul:first"):b.next()};a(".collapse-handler").die("click").live("click",function(){if(a(this).hasClass("darr"))b(this);else{c(this).show();var d=a(this);d.removeClass("rarr").addClass("darr");a.storage.del("shop/products/"+this.id+"/collapse");var e=this.id.replace(/\-handler$/,""),f=d.parents("div.block:first").find("#"+
e+"-placeholder:first");if(f.length){var g=a(this).parents("div.block:first").find(".count:first");g.find("i.icon16.loading").remove();g.find("i.icon16").hide();g.prepend('<i class="icon16 loading"></i>');a.get(f.data("url"),function(b){f.replaceWith(a(b));g.find("i.icon16.loading").remove();g.find("i.icon16").show();f.remove()})}else{d=a(this).parents("div.block:first").find("#"+e+":first");if(d.length)(d=d.data("on-expand-url"))&&a.get(d)}}}).each(function(){var d="shop/products/"+this.id+"/collapse",
c=a(this).hasClass("rarr");(a.storage.get(d)||c)&&b(this,!c)})},initSearch:function(){var b=function(){var b=this.value,c=a.products.hash.match(/[&\/](text=.*?&|text=.*)/),f=a.products.hash;if(c){c=c[1];if(b){b=c.substr(-1)=="&"?"text="+encodeURIComponent(b)+"&":"text="+encodeURIComponent(b);f=f.replace(c,b)}else{c.substr(-1)!="&"&&(c="[&/]"+c);f=f.replace(RegExp(c),"")}}else if(b){delete a.products.list_params.text;f="products/"+a.products.buildProductsUrlComponent(a.products.list_params)+"&text="+
encodeURIComponent(b)}else f="products/"+a.products.buildProductsUrlComponent(a.products.list_params);location.hash="#/"+f},c=a("#s-products-search");c.unbind("keydown").bind("keydown",function(d){if(d.keyCode==13){b.call(this);a(this).autocomplete("close");return false}});c.unbind("search").bind("search",function(){b.call(this);return false});c.autocomplete({source:"?action=autocomplete",minLength:3,delay:300,select:function(b,c){a.wa.setHash("#/product/"+c.item.id);return false}})},initSidebar:function(){var b=
a("#s-sidebar");a.product_dragndrop.init({collections:true}).bind("move_list",function(b){if(b.type){var c={id:b.id,type:b.type,parent_id:b.parent_id||0};if(b.before_id)c.before_id=b.before_id;a.products.jsonPost("?module=products&action=moveList",c,b.success,b.error)}else typeof b.error==="function"&&b.error("Unknown list type")});b.off("add",".s-collection-list ul").on("add",".s-collection-list ul",function(b,c,f,g){var b=a(this),h=b.parents(".s-collection-list:first"),j=a("<ul></ul>");j.append(tmpl("template-sidebar-list-item",
{type:f,item:c}));var c=j.children(":not(.drag-newposition):first"),f=c.attr("id"),f=b.find("#"+f),i=j.children();f.length?g&&f.replaceWith(c):b.prepend(i).show();i.each(function(){var b=a(this);b.hasClass("dr")?b.find("a").mouseover():b.mouseover()});b.find(".drag-newposition").css({height:"2px"}).removeClass("dragging");h.find(".s-empty-list").hide();j.remove();return false});b.unbind("update").bind("update",function(b,c){for(var f in c)if(f=="all")a("#s-all-products").find(".count:first").text(c[f].count);
else{var g="#"+f+"-",h;for(h in c[f])a(g+h).find(".count:first").text(c[f][h].count)}return false});a("#s-tag-cloud").unbind("update").bind("update",function(b,c){var f='<ul class="tags"><li class="block align-center">',g;for(g in c)var h=c[g],f=f+('<a href="#/products/tag='+h.uri_name+'/" style="font-size: '+h.size+"%; opacity: "+h.opacity+'"  data-id="'+h.id+'"  class="s-product-list">'+h.name+"</a>");a("#s-tag-cloud").html(f+"</li></ul>").parents(".block:first").show();return false});b.off("count_subtree",
".s-collection-list li").on("count_subtree",".s-collection-list li",function(b,c){var f=a(this);typeof c==="undefined"&&(c=f.find("i.collapse-handler-ajax").hasClass("rarr"));var g=f.find(">.counters .count:not(.subtree)"),f=f.find(">.counters .subtree");if(!f.length){f=g.clone().addClass("subtree").hide();g.after(f)}if(c){g.hide();f.show()}else{f.hide();g.show()}return false});b.off("update_counters",".s-collection-list li").on("update_counters",".s-collection-list li",function(b,c){var f=a(this),
...
b.remove());a("#s-product-list-settings-dialog").waDialog({disableButtonsOnSubmit:!0,onLoad:function(){a("#s-category-description-content").length&&a.product_list.initCategoryDescriptionWysiwyg(a(this));a("#s-product-list-name").focus()},onSubmit:function(b){var c=a(this),f=function(c){var e=a.product_list.collection_hash,f=location.hash.replace(e[0]+"_id="+e[1],e[0]+"_id="+c.data.id),g=a("#"+e.join("-"));g.find(".name:first").html(c.data.name);c.data.status=="0"?g.children("a").addClass("gray"):
c.data.status=="1"&&g.children("a").removeClass("gray");g.find(".id:first").html(c.data.id);g.attr("id",e[0]+"-"+c.data.id);g.find("a").attr("href",f);location.hash!=f?location.hash=f:a.products.dispatch();b.trigger("close")},g=function(a){if(a&&a.errors){var a=a.errors,c;for(c in a)b.find("input[name="+c+"]").addClass("error").parent().find(".errormsg").text(a[c]);return false}};a("#s-category-description-content").length&&waEditorUpdateSource({id:"s-category-description-content"});if(c.find("input:file").length)a.product_list._iframePost(c,
f,g);else{a.shop.jsonPost(c.attr("action"),c.serialize(),f,g);return false}}})})},createListDialog:function(b,c,d){var e=a("#s-product-list-create-dialog");p=e.length?e.parent():a("<div></div>").appendTo("body");p.load("?module=dialog&action=productListCreate&type="+b+"&parent_id="+c,function(){var c=a("#s-product-list-settings-dialog");c.length&&(c.parent().remove(),c.remove());a("#s-product-list-create-dialog").waDialog({disableButtonsOnSubmit:!0,onLoad:function(){a("#s-category-description-content").length&&
a.product_list.initCategoryDescriptionWysiwyg(a(this));a("#s-c-product-list-name").focus()},onSubmit:function(c){var e=a(this),f=function(e){typeof d==="function"&&d(e.data,b);location.href="#/products/"+b+"_id="+e.data.id+"&view="+a.product_list.options.view;c.trigger("close")},i=function(a){if(a&&a.errors){var a=a.errors,b;for(b in a)c.find("input[name="+b+"]").addClass("error").parent().find(".errormsg").text(a[b]);return false}};a("#s-category-description-content").length&&waEditorUpdateSource({id:"s-category-description-content"});
if(e.find("input:file").length)a.product_list._iframePost(e,f,i);else{a.shop.jsonPost(e.attr("action"),e.serialize(),f,i);return false}}})})},exportProducts:function(a,c){for(var d=[],e,f=!1;e=a.serialized.pop();)e.name&&("product_id[]"==e.name?d.push(parseInt(e.value,10)):"hash"==e.name&&(f=e.value));e="?action=importexport#/"+c;f?window.location=e+("/hash/"+f):d.length&&(e+="/hash/id/"+d.join(","),window.location=e)},remove:function(b,c){var d={},e;b.product_id?e=function(){100>=b.product_id.length&&
